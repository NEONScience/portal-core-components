...
var ALL_POSSIBLE_VALID_SITES = Object.keys(_sites.default);
...
var DEFAULT_STATE = {
  ...
  s3Files: {
    ...
    urlsById: {},
    // A mapping of id to url for building the final file list download request
    totalSize: 0,
    estimatedPostSize: 0,
  ...
  sites: {
    value: [],
    validValues: _toConsumableArray(ALL_POSSIBLE_VALID_SITES),
    isValid: false
  },
  dateRange: {
...
var VALIDATABLE_STATE_KEYS = ['sites', 'dateRange', 'documentation', 'packageType', 's3Files', 'policies']; // State keys that can be transfered between contexts through higher order state
// (must be a subset of VALIDATABLE_STATE_KEYS)

var HIGHER_ORDER_TRANSFERABLE_STATE_KEYS = ['sites', 'dateRange']; // State keys that should trigger a new manifest (file size estimate) request when updated
// (must be a subset of VALIDATABLE_STATE_KEYS)

var MANIFEST_TRIGGERING_STATE_KEYS = ['sites', 'dateRange', 'documentation', 'packageType']; // Regexes and associated capture group names for parse s3 file names and URLs
...
var getInitialStateFromProps = function getInitialStateFromProps(props) {
  ...
  }) && (productData.productPublicationFormatType || '').includes('AOP'); // Set required steps and data download origin booleans
...
var regenerateS3FilesFiltersAndValidValues = function regenerateS3FilesFiltersAndValidValues(state) {
  ...
  updated.s3Files.validValues = updated.s3Files.cachedValues.filter(function (file) {
    ...
    return _extends({}, file, {
      tableData: {
        checked: updated.s3Files.value.includes(file.id)
      }
    });
    ...
    case 'setS3FileFetchesCompleted':
      ...
      action.value.forEach(function (response) {
        ...
        var files = response.files.map(function (fileObj) {
          ...
          newState.s3Files.urlsById[file.id] = file.url; // If new unique values are present add them to the s3File value lookups
...
var Provider = function Provider(props) {
  ...
  var _useReducer = (0, _react.useReducer)(reducer, initialState),
      ...
      dispatch = _useReducer2[1]; // Create an observable for manifests requests and subscribe to it to execute
  ...
  var handleFetchS3Files = function handleFetchS3Files(currentState) {
    ...
    var observable = forkJoinWithProgress(keys.map(function (key) {
      var site = key.substr(0, 4);
      var yearMonth = key.substr(5, 7);
      return _NeonApi.default.getJsonObservable((0, _manifestUtil.buildS3FilesRequestUrl)(productCode, site, yearMonth)).pipe((0, _operators.map)(function (response) {
        return {
          status: 'fetched',
...
Provider.propTypes = {
  ...
  sites: _propTypes.default.arrayOf(_propTypes.default.oneOf(ALL_POSSIBLE_VALID_SITES)),
