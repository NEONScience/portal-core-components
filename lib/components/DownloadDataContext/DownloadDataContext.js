...
var Provider = function Provider(props) {
  ...
  }, [stateObservable]); // Set the authentication status exactly once

  (0, _react.useEffect)(function () {
    if (state.auth.fetchStatus !== 'awaitingFetchCall') {
      return;
    }
  ...
  }, [state.auth.fetchStatus]); // When the state has changed such that it needs to broadcast itself to anything
