...
var SITE_MANAGEMENT_DATA_PRODUCT_CODE = 'DP1.10111.001';
...
var ALL_POSSIBLE_VALID_BUNDLED_PRODUCTS = [SITE_MANAGEMENT_DATA_PRODUCT_CODE];
...
var DEFAULT_STATE = {
  ...
  bundledProducts: {
    value: [],
    validValues: [].concat(ALL_POSSIBLE_VALID_BUNDLED_PRODUCTS),
    isValid: false
  },
  packageType: {
    value: null,
...
var VALIDATABLE_STATE_KEYS = ['release', 'sites', 'dateRange', 'documentation', 'bundledProducts', 'packageType', 's3Files', 'policies']; // State keys that can be transfered between contexts through higher order state
...
var MANIFEST_TRIGGERING_STATE_KEYS = ['release', 'sites', 'dateRange', 'documentation', 'bundledProducts', 'packageType']; // Regexes and associated capture group names for parse s3 file names and URLs
  ...
  switch (key) {
    ...
    case 'bundledProducts':
      return Array.isArray(value) && value.every(function (p) {
        return typeof p === 'string' && /^DP[0-9]{1}[.][0-9]{5}[.][0-9]{3}$/.test(p);
      });
...
var newStateIsValid = function newStateIsValid(key, value) {
  ...
  switch (key) {
    ...
    case 'bundledProducts':
      return newStateIsAllowable(key, value) && Array.isArray(validValues) && (value.length <= 0 || value.length > 0 && value.every(function (p) {
...
var mutateNewStateIntoRange = function mutateNewStateIntoRange(key, value) {
  ...
  switch (key) {
    ...
    case 'bundledProducts':
      return valueIsAllowable ? value.filter(function (p) {
        return validValues.includes(p);
      }) : [];
...
var getValidValuesFromProductData = function getValidValuesFromProductData(productData, key) {
  switch (key) {
    ...
    case 'bundledProducts':
      return [].concat(ALL_POSSIBLE_VALID_BUNDLED_PRODUCTS);
...
Provider.propTypes = {
  ...
  bundledProducts: _propTypes.default.arrayOf(_propTypes.default.string),
...
var DownloadDataContext = {
  ...
  ALL_STEPS: ALL_STEPS,
  SITE_MANAGEMENT_DATA_PRODUCT_CODE: SITE_MANAGEMENT_DATA_PRODUCT_CODE,
...
var getTestableItems = function getTestableItems() {
  return process.env.NODE_ENV !== 'test' ? {} : {
    ...
    ALL_POSSIBLE_VALID_BUNDLED_PRODUCTS: ALL_POSSIBLE_VALID_BUNDLED_PRODUCTS
  };
};
