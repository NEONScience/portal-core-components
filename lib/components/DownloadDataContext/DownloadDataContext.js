...
import { of, merge, map, mergeMap, switchMap, catchError, tap, ignoreElements, takeUntil, Subject } from 'rxjs';
import NeonApi from '../NeonApi/NeonApi';
import NeonContext from '../NeonContext/NeonContext';
import ExternalHost, { HOST_TYPES } from '../ExternalHost/ExternalHost';
import { buildManifestConfig, buildS3FilesRequestUrl, buildManifestRequestUrl, buildManifestRequestBody, getSizeEstimateFromManifestRollupResponse, MAX_POST_BODY_SIZE } from '../../util/manifestUtil';
...
const ALL_POSSIBLE_VALID_DOCUMENTATION = [
    'include',
    'exclude'
];
const ALL_POSSIBLE_VALID_PACKAGE_TYPE = [
    'basic',
    'expanded'
];
const ALL_POSSIBLE_VALID_PROVISIONAL_DATA = [
    'include',
    'exclude'
];
const AVAILABILITY_VIEW_MODES = [
    'summary',
    'sites',
    'states',
    'domains'
...
const HIGHER_ORDER_TRANSFERABLE_STATE_KEYS = [
    'release',
    'sites',
    'dateRange'
];
...
const productDataIsValid = (productData)=>{
    ...
    if (!hasSiteCodes) {
        ...
        if (exists(externalHost)) {
            ...
            return exists(externalHostProduct) && externalHostProduct.allowNoAvailability === true;
        }
    }
    return hasSiteCodes;
};
...
const getInitialStateFromProps = (props)=>{
    ...
    const { productData, availabilityView, release } = props;
    ...
    ].some((key)=>typeof productData[key] === 'string' && productData[key].includes('AOP')) && (productData.productPublicationFormatType || '').includes('AOP');
    // Pull the latest release from productData
    if (productData.releases && productData.releases.length) {
        const sortedReleases = [
            ...productData.releases
    ...
    if (externalHost) {
        ...
        const useExternalExclusiveData = externalHost.hostType === HOST_TYPES.EXCLUSIVE_DATA || allowNoAvailability && !existsNonEmpty(productData.siteCodes);
    ...
    const packageTypeStepNotRequired = productData.productHasExpanded === false && requiredSteps.some((step)=>step.key === 'packageType');
    ...
    if (exists(productData) && existsNonEmpty(productData.siteCodes)) {
        hasProvisionalData = productData.siteCodes.some((siteCode)=>{
            ...
            return siteCode.availableReleases.some((availableRelease)=>ReleaseService.isProv(availableRelease.release) && existsNonEmpty(availableRelease.availableMonths));
        });
    }
...
const calcS3FilesSummaryState = (previousState, action)=>{
    ...
    newState.s3Files.isValid = newState.s3Files.value.length > 0 && newState.s3Files.estimatedPostSize < MAX_POST_BODY_SIZE;
...
const regenerateS3FilesFiltersAndValidValues = (state)=>{
    ...
    updated.s3Files.validValues = updated.s3Files.cachedValues.filter((file)=>updated.sites.value.includes(file.site) && updated.dateRange.value[0] <= file.yearMonth && file.yearMonth <= updated.dateRange.value[1]).map((file, index)=>({
            ...file,
            tableData: {
...
const getAndValidateNewState = (previousState, action, broadcast = false)=>{
    ...
    if ([
        'sites',
        'dateRange'
    ].includes(action.key) && previousState.fromAOPManifest) {
        Object.keys(previousState.s3FileFetches).filter((key)=>[
                'notRequested',
                'error'
            ].includes(previousState.s3FileFetches[key])).filter((key)=>{
...
// CONTEXT
const Context = /*#__PURE__*/ createContext(DEFAULT_STATE);
// HOOK
...
const getManifestAjaxObservable = (request)=>NeonApi.postJsonObservable(request.url, request.body, null, false);
const defaultProps = {
...
const Provider = (inProps)=>{
    ...
    const { downloadDataContextUniqueId, stateObservable, children } = props;
    const neonContextSessionState = NeonContext.useNeonContextSessionState();
    // Check preconditions for initial status
    const { ready: preconditionsSatisfied, canAccessData } = neonContextSessionState;
    ...
    useEffect(()=>{
        ...
        const newStatus = canAccessData ? DOWNLOAD_STATUS.ALLOW_DOWNLOAD : DOWNLOAD_STATUS.DISALLOW_DOWNLOAD;
        dispatch({
            type: 'setDownloadStatus',
    ...
    manifestRequest$.subscribe((request)=>getManifestAjaxObservable(request).pipe(switchMap((resp)=>of(request.body ? resp.response : resp)), takeUntil(manifestCancelation$)).subscribe({
            next: (resp)=>dispatch({
    ...
    const handleFetchS3Files = (currentState, headers)=>{
        ...
        const observable = forkJoinWithProgress(keys.map((key)=>{
            const site = key.slice(0, 4);
            const yearMonth = key.slice(5, 12);
            const release = currentState.release && currentState.release.value ? currentState.release.value : null;
            return NeonApi.getJsonObservable(buildS3FilesRequestUrl(productCode, site, yearMonth, release), headers).pipe(map((response)=>({
                    status: 'fetched',
        ...
        observable.pipe(mergeMap(([finalResult, progress])=>merge(progress.pipe(tap((value)=>dispatch({
    ...
    useEffect(()=>{
        ...
        const config = buildManifestConfig(state, ALL_POSSIBLE_VALID_PACKAGE_TYPE[0]);
...
Provider.propTypes = {
    ...
    productData: PropTypes.shape({
        ...
        siteCodes: PropTypes.arrayOf(PropTypes.shape({
    ...
    /* eslint-disable react/no-unused-prop-types */ availabilityView: PropTypes.oneOf(AVAILABILITY_VIEW_MODES),
    ...
    /* eslint-enable react/no-unused-prop-types */ children: PropTypes.oneOfType([
