...
var DEFAULT_STATE = {
  ...
  s3Files: {
    value: [],
    validValues: [], // Where we cache fetched file objects
    isValid: false,
    idxById: {}, // A cached mapping of id to index in validValues to use when starting with ids
    bytesById: {}, // A cached mapping of id to file size for fast summing of total download size
    urlsById: {}, // A cached mapping of id to url for building the final file list download request
    totalSize: 0,
...
var getAndValidateNewS3FilesState = function getAndValidateNewS3FilesState(previousState, action) {
  ...
  // Set value and reflect back onto validValues as "checked" for material table.
  switch (action.type) {
    case 'setValidatableValue':
      if (!newStateIsValid(action.key, action.value, previousState[action.key].validValues)) {
        return newState;
      }
...
var getAndValidateNewState = function getAndValidateNewState(previousState, action) {
  ...
  // step then trigger s3 file fetches as needed
  if (['sites', 'dateRange'].includes(action.key) && previousState.fromAOPManifest) {
    Object.keys(previousState.s3FileFetches).filter(function (key) {
      ...
      return newState.sites.value.includes(site) && newState.dateRange.value[0] <= yearMonth && newState.dateRange.value[1] >= yearMonth;
...
var reducer = function reducer(state, action) {
  ...
  switch (action.type) {
    ...
    case 'setS3FilesFilterValue':
      ...
      newState.s3Files.filteredFileCount = newState.s3Files.validValues.filter(function (row) {
        return Object.keys(newState.s3Files.filters).every(function (col) {
          return !newState.s3Files.filters[col].length || newState.s3Files.filters[col].includes(row[col]);
        });
      }).length;
    ...
    // Note we only append to validValues in the s3Files key.
    // This can't flip a valid value to invalid as we're only
    // expanding what's valid.
    case 'setS3FileFetchesCompleted':
      ...
      action.value.forEach(function (response) {
        ...
        newState.s3Files.validValues = newState.s3Files.validValues.concat(files);
      });
      // Cache index of validValues by file id to avoid needing to search when the set is large
      newState.s3Files.idxById = newState.s3Files.validValues.reduce(function (acc, file, idx) {
        acc[file.id] = idx;
        return acc;
      }, {});
      return newState;
...
var Provider = function Provider(props) {
  ...
  // in through props: subscribe to that observable.
