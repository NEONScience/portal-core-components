...
function BasicAvailabilityGrid(config) {
  ...
  */
  ...
  rowKeys.forEach(function (rowKey, rowIdx) {
    ...
    var mask = rowLabelG.append('rect').attr('x', 0).attr('y', -1 * (_AvailabilityUtils.SVG.CELL_PADDING / 2)).attr('width', getLabelWidth()).attr('height', _AvailabilityUtils.SVG.CELL_HEIGHT + _AvailabilityUtils.SVG.CELL_PADDING).on('mouseover', function () {
  ...
  */
  ...
  var redrawData = function redrawData() {
    ...
    dataG.selectAll('g').data(rowKeys).join('g').attr('transform', getRowTranslation).each(function (rowKey, rowIdx, gNodes) {
      ...
      }).attr('y', 0).attr('width', "".concat(_AvailabilityUtils.SVG.CELL_WIDTH, "px")).attr('height', "".concat(_AvailabilityUtils.SVG.CELL_HEIGHT, "px")).attr('rx', "".concat(_AvailabilityUtils.SVG.CELL_RX, "px")).attr('cursor', 'pointer').attr('fill', function (d) {
        switch (rowData[d]) {
          case 'available':
            return _Theme.default.palette.secondary.main;

          default:
            return _Theme.default.palette.grey[200];
        }
      });
    });
  };
