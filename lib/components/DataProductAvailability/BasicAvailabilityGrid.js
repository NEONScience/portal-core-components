...
function BasicAvailabilityGrid(config) {
  ...
  */
  ...
  if (svg.attr('id') === null) {
    svg.attr('id', `availability-${(0, _uniqueId.default)()}`);
  }
  ...
  */
  ...
  const rowHighlightReset = (0, _lodash.debounce)((clearRowHoverKey = true) => {
    if (clearRowHoverKey) {
      rowHoverKey = null;
    }
  ...
  */
  ...
  const redrawDateRangeHandles = () => {
    ...
    dateRangeHandlesG.selectAll('rect').data([0, 1]).join('rect').attr('class', d => `dateRange${d === 0 ? 'Start' : 'End'}HandleRect`).attr('x', d => {
  ...
  */
  ...
  const redrawDateRangeHandleMasks = () => {
    ...
    dateRangeMasksG.selectAll('rect').data([0, 1]).join('rect').attr('class', d => `dateRange${d === 0 ? 'Start' : 'End'}MaskRect`).attr('x', d => getYearMonthGutterX(dateRange.value[d] || _AvailabilityUtils.TIME.MIN_YEAR_MONTH, d === 0 ? 'left' : 'right') - _AvailabilityUtils.SVG.DATE_RANGE_MASK_WIDTH / 2 + getTimeOffset()).attr('y', 0).attr('width', _AvailabilityUtils.SVG.DATE_RANGE_MASK_WIDTH).attr('height', svgHeight).style('cursor', 'crosshair') // 'ew-resize'
