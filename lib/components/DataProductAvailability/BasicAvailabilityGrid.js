...
import { SVG, TIME, SVG_STYLES, calcBasicRollupStatus } from './AvailabilityUtils';
/**
 * BasicAvailabilityGrid generator function
 * @param {object} config - all arguments, see below:
 * * @param {object} theme The theme object to use to apply to the grid
 ...
 */ export default function BasicAvailabilityGrid(config) {
    /**
     Extract Config
  */ const { theme, svgRef, view, data, sites = {
        value: [],
        validValues: []
    }, allSites = {}, sortedSites = [], setSitesValue = null, dateRange = {
        value: [],
        validValues: [
            TIME.MIN_YEAR_MONTH,
            TIME.MAX_YEAR_MONTH
        ]
    }, setDateRangeValue = ()=>{}, selectionEnabled = true } = config;
    /**
  ...
  */ const svg = select(svgRef.current);
    if (svg.attr('id') === null) {
        svg.attr('id', `availability-${uniqueId()}`);
    }
  ...
  */ const getLabelWidth = ()=>{
    ...
    const getYearStartX = (year)=>{
        ...
        return getLabelWidth() + SVG.END_PADDING + TIME.YEARS.indexOf(intYear) * (SVG.YEAR_WIDTH + SVG.YEAR_PADDING);
    };
    ...
    const getYearMonthsInView = (totalSvgWidth, dragOffset)=>TIME.YEAR_MONTHS.filter((yearMonth)=>{
    ...
    const getYearsInView = (totalSvgWidth, dragOffset)=>TIME.YEARS.filter((year)=>{
            const tX = getYearStartX(year) + dragOffset;
            const lower = -2 * SVG.YEAR_WIDTH;
            const upper = totalSvgWidth - getLabelWidth() + 2 * SVG.YEAR_WIDTH;
  ...
  */ const getInitialTimeOffset = ()=>{
        ...
        const finalMonth = latestAvailableMonthInt <= 6 ? `${latestAvailableYearInt}-08` : `${latestAvailableYearInt + 1}-08`;
        return Math.max(0 - getYearMonthStartX(finalMonth) - SVG.YEAR_MONTH_WIDTH + svgWidth, minTimeOffset);
    };
    /**
  ...
  */ svg.selectAll('*').remove();
    ...
    const clipPath = defs.append('clipPath').attr('id', `${svgId}-clip`);
    clipPath.append('rect').attr('x', getLabelWidth()).attr('y', 0).attr('width', clipWidth).attr('height', svgHeight);
    const clipG = svg.append('g').attr('class', 'clipG').attr('clip-path', `url(#${svgId}-clip)`);
  ...
  */ // Get the string for the yearmonth closest to a given pixel x-offset on either side.
    ...
    const getXOffsetYearMonth = (xOffset, dragOffset, side = 'left', selectable = true)=>{
        ...
        } else if (basis > yearStartX + SVG.YEAR_WIDTH && side === 'right' && boundedYearIdx !== TIME.YEARS.length - 1) {
            year = parseInt(year, 10) + 1;
            yearStartX = getYearStartX(year);
        }
        const roundFunc = side === 'left' ? 'floor' : 'ceil';
        const month = Math[roundFunc]((basis - yearStartX) / SVG.YEAR_MONTH_WIDTH) + 1;
        const boundedMonth = Math.max(Math.min(month, 12), 1);
        const yearMonth = `${year}-${boundedMonth.toString().padStart(2, '0')}`;
        const bounds = selectable ? dateRange.validValues : [
            TIME.MIN_YEAR_MONTH,
            TIME.MAX_YEAR_MONTH
        ];
   ...
   */ const rowHover = svg.append('rect').attr('class', 'rowHover');
    rowHover.attr('x', 1).attr('y', -2 * SVG.CELL_HEIGHT).attr('width', svgWidth - 1).attr('height', SVG.CELL_HEIGHT + SVG.CELL_PADDING).attr('fill', 'none').attr('stroke', COLORS.LIGHT_BLUE[700]).attr('stroke-width', '1.5px').style('opacity', 0);
    /**
     SVG: Left and Right bounds
  */ const lboundOffset = getLabelWidth() + SVG.CELL_PADDING / 2 - 1;
  ...
  */ const viewSelections = {};
        ...
        switch(view.view){
            ...
            default:
                Object.keys(siteViewMaps[view.view]).forEach((entry)=>{
                    const viewSites = new Set(siteViewMaps[view.view][entry].filter((s)=>validSitesSet.has(s)));
                    const intersection = new Set([
                        ...viewSites
                    ].filter((s)=>sitesSet.has(s)));
    ...
    const rowHighlightHover = (key)=>{
        ...
        if (offset !== -1 && !draggingCells) {
            ...
            const y = SVG.CELL_PADDING / 2 + (rowCount - offset) * (SVG.CELL_HEIGHT + SVG.CELL_PADDING);
  ...
  */ const rowLabelsG = svg.append('g').attr('class', 'rowLabelsG');
    rowKeys.forEach((rowKey, rowIdx)=>{
        ...
        const fill = selectionEnabled && setSitesValue && viewSelections[rowKey] ? theme.palette.primary.contrastText : theme.palette.grey[700];
        const text = rowLabelG.append('text').attr('x', labelX).attr('y', SVG.LABEL_FONT_SIZE - SVG.CELL_PADDING / 2).attr('fill', fill).text(view.getLabel.text(rowKey));
        SVG_STYLES.apply(text, 'rowLabel');
        const mask = rowLabelG.append('rect').attr('x', 0).attr('y', -1 * (SVG.CELL_PADDING / 2)).attr('width', getLabelWidth()).attr('height', SVG.CELL_HEIGHT + SVG.CELL_PADDING).on('mouseover', (event, d)=>rowHighlightHover(rowKey)).on('focus', (event, d)=>rowHighlightHover(rowKey)).on('mouseout', (event, d)=>rowHighlightReset(true)).on('blur', (event, d)=>rowHighlightReset(true));
  ...
  */ const timeAxis = {};
    ...
    const redrawTimeAxis = ()=>{
        if (timeAxis.innerG.selectAll('*').empty()) {
            timeAxis.highlight.attr('x', getLabelWidth()).attr('y', 0).attr('width', SVG.ABS_MAX_DATA_WIDTH).attr('height', SVG.CELL_HEIGHT + 1.5 * SVG.CELL_PADDING);
            SVG_STYLES.apply(timeAxis.highlight, 'timeHighlight');
            timeAxis.mask.attr('x', getLabelWidth()).attr('y', 0).attr('width', SVG.ABS_MAX_DATA_WIDTH).attr('height', SVG.CELL_HEIGHT + SVG.CELL_PADDING);
            SVG_STYLES.apply(timeAxis.mask, 'timeHighlightMask');
        }
        timeAxis.innerG.selectAll('text').data(()=>getYearsInView(svgWidth, getTimeOffset())).join('text').attr('x', (year)=>getYearCenterX(year)).attr('y', SVG.LABEL_FONT_SIZE + 1).text((year)=>year).each((year, idx, labelNodes)=>{
        ...
        timeAxis.innerG.selectAll('line').data(()=>{
            ...
            return lineYears[0] === TIME.START_YEAR ? lineYears.slice(1) : lineYears;
        }).join('line').attr('x1', (year)=>getYearStartX(year) - SVG.YEAR_PADDING / 2).attr('y1', 0).attr('x2', (year)=>getYearStartX(year) - SVG.YEAR_PADDING / 2).attr('y2', svgHeight).each((year, idx, lineNodes)=>{
  ...
  */ const dateRangeHandlesG = selectionEnabled ? dragG.append('g').attr('class', 'dateRangeHandlesG') : null;
    const redrawDateRangeHandles = ()=>{
        ...
        dateRangeHandlesG.selectAll('rect').data([
            0,
            1
        ]).join('rect').attr('class', (d)=>`dateRange${d === 0 ? 'Start' : 'End'}HandleRect`).attr('x', (d)=>{
            const useWidth = isHighlighted(d) ? SVG.DATE_RANGE_HANDLE_WIDTH + 2 : SVG.DATE_RANGE_HANDLE_WIDTH;
        ...
        }).attr('width', (d)=>isHighlighted(d) ? SVG.DATE_RANGE_HANDLE_WIDTH + 2 : SVG.DATE_RANGE_HANDLE_WIDTH).attr('y', (d)=>{
            if (isHighlighted(d)) {
                return getRowY(rowKeys.length - 1) - SVG.CELL_PADDING / 2;
            }
            return yBounds[0];
        }).attr('height', (d)=>{
            if (isHighlighted(d)) {
                return svgHeight - getRowY(rowKeys.length - 1) + SVG.CELL_PADDING / 2 - 0.5;
            }
            return yBounds[1] - yBounds[0];
        }).attr('fill', (d)=>isHighlighted(d) ? COLORS.LIGHT_BLUE[100] : COLORS.LIGHT_BLUE[300]).attr('stroke', theme.palette.primary.main).style('stroke-width', '1.5px').style('display', sites.value.length ? null : 'none');
    };
    /**
  ...
  */ const dateRangeMasksG = selectionEnabled ? svg.append('g').attr('class', 'dateRangeMasksG') : null;
    const redrawDateRangeHandleMasks = ()=>{
        ...
        dateRangeMasksG.selectAll('rect').data([
            0,
            1
        ]).join('rect').attr('class', (d)=>`dateRange${d === 0 ? 'Start' : 'End'}MaskRect`).attr('x', (d)=>getYearMonthGutterX(dateRange.value[d] || TIME.MIN_YEAR_MONTH, d === 0 ? 'left' : 'right') - SVG.DATE_RANGE_MASK_WIDTH / 2 + getTimeOffset()).attr('y', 0).attr('width', SVG.DATE_RANGE_MASK_WIDTH).attr('height', svgHeight).style('cursor', 'crosshair') // 'ew-resize'
  ...
  */ const redrawSelections = ()=>{
        ...
        const fill = (d)=>viewSelections[d] === 'full' ? theme.palette.primary.main : COLORS.LIGHT_BLUE[200];
  ...
  */ const timeHighlightReset = debounce(()=>{
    ...
    const dragCells = drag().on('start', (event, d)=>{
        draggingCells = true;
    ...
    if (selectionEnabled) {
        ...
        const dragDateRangeStart = drag().on('start', (event, d)=>{
            ...
            const maskX = getYearMonthGutterX(dateRange.value[0], 'left') + SVG.DATE_RANGE_MASK_WIDTH / 2 + getTimeOffset();
        ...
        const dragDateRangeEnd = drag().on('start', (event, d)=>{
            ...
            // Recenter mask as it is likely off a few pixels due to snap-to-gutter behavior
            const maskX = getYearMonthGutterX(dateRange.value[1], 'right') - SVG.DATE_RANGE_MASK_WIDTH / 2 + getTimeOffset();
