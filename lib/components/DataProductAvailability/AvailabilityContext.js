...
*/ const calculateRows = (state)=>{
    ...
    const { sites, breakouts, reference, sortDirection, tables: knownTables } = state;
    if (!breakouts.length) {
        newState.rowLabels = [
            'ALL'
        ];
...
*/ const Provider = (inProps)=>{
    ...
    const stateStorage = useMemo(()=>makeStateStorage(key), [
        key
    ]);
    ...
    const appliedInitialState = initState.restoredStateFromStorage ? initState.initialState : contextState;
