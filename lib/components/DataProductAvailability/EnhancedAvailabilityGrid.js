...
function EnhancedAvailabilityGrid(config) {
  ...
  */
  ...
  rowLabels.forEach(function (label, rowIdx) {
    ...
    var mask = rowLabelG.append('rect').attr('x', 0).attr('y', -1 * (_AvailabilityUtils.SVG.CELL_PADDING / 2)).attr('width', labelWidth).attr('height', _AvailabilityUtils.SVG.CELL_HEIGHT + _AvailabilityUtils.SVG.CELL_PADDING).on('mouseover', function (event, d) {
  ...
  */
  ...
  var redrawData = function redrawData() {
    ...
    }).attr('width', svgWidth - labelWidth).attr('height', _AvailabilityUtils.SVG.CELL_HEIGHT + _AvailabilityUtils.SVG.CELL_PADDING).attr('fill', 'transparent').style('cursor', selectionEnabled && setSitesValue ? 'pointer' : 'grab').style('outline', 'none').on('mouseover', function (event, d) {
  ...
  */
  ...
  if (selectionEnabled) {
    ...
    var dragDateRangeStart = (0, _d3Drag.drag)().on('start', function (event, d) {
      ...
      var adjacentYearMonth = getAdjacentYearMonth(dateRange.value[0], event.dx > 0 ? 'right' : 'left');
    ...
    var dragDateRangeEnd = (0, _d3Drag.drag)().on('start', function (event, d) {
      ...
      var adjacentYearMonth = getAdjacentYearMonth(dateRange.value[1], event.dx > 0 ? 'right' : 'left');
