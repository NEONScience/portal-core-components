...
import { SVG, TIME, SVG_STYLES, calcRollupStatus } from './AvailabilityUtils';
 ...
 */ export default function EnhancedAvailabilityGrid(config) {
    /**
     Extract Config
  */ const { theme, svgRef, rows = {}, rowLabels = [], rowTitles = {}, sites = {
        value: [],
        validValues: []
    }, // allSites = {},
    setSitesValue = null, dateRange = {
        value: [],
        validValues: [
            TIME.MIN_YEAR_MONTH,
            TIME.MAX_YEAR_MONTH
  ...
  */ const svg = select(svgRef.current);
    ...
    const labelWidth = rowLabels[0].length * SVG.LABEL_WIDTH_MULTIPLIER + SVG.LABEL_WIDTH_RIGHT_BUFFER;
    /**
  ...
  */ const getMinTimeOffset = ()=>0 - (SVG.ABS_MAX_DATA_WIDTH - (svgWidth - labelWidth));
    const getYearStartX = (year)=>{
        ...
        return labelWidth + SVG.END_PADDING + TIME.YEARS.indexOf(intYear) * SVG.YEAR_WIDTH + TIME.YEARS.indexOf(intYear) * SVG.YEAR_PADDING;
    };
  ...
  */ const getInitialTimeOffset = ()=>{
        ...
        const ret = Math.max(0 - getYearMonthStartX(finalMonth) - SVG.YEAR_MONTH_WIDTH + svgWidth, minTimeOffset);
        return ret;
    };
    /**
     Functions to get a translate() string for a data row by index
     in the sorted rowLabels array
  */ const getRowY = (idx)=>SVG.CELL_PADDING + (rowLabels.length - idx) * (SVG.CELL_HEIGHT + SVG.CELL_PADDING);
    ...
    */ };
    ...
    const rowHighlightHover = (key)=>{
        ...
        if (offset !== -1 && !draggingCells) {
            ...
            const y = SVG.CELL_PADDING / 2 + (rowLabels.length - offset) * (SVG.CELL_HEIGHT + SVG.CELL_PADDING);
  ...
  */ const rowLabelsG = svg.append('g').attr('class', 'rowLabelsG');
    rowLabels.forEach((label, rowIdx)=>{
        ...
        const fill = selectionEnabled && setSitesValue && viewSelections[label] ? theme.palette.primary.contrastText : theme.palette.grey[700];
        const text = rowLabelG.append('text').attr('x', labelX).attr('y', SVG.LABEL_FONT_SIZE - SVG.CELL_PADDING / 2).attr('fill', fill).text(label);
        SVG_STYLES.apply(text, 'rowLabel');
        const mask = rowLabelG.append('rect').attr('x', 0).attr('y', -1 * (SVG.CELL_PADDING / 2)).attr('width', labelWidth).attr('height', SVG.CELL_HEIGHT + SVG.CELL_PADDING).on('mouseover', (event, d)=>rowHighlightHover(label)).on('focus', (event, d)=>rowHighlightHover(label)).on('mouseout', (event, d)=>rowHighlightReset(true)).on('blur', (event, d)=>rowHighlightReset(true));
  ...
  */ const timeAxis = {};
    ...
    const redrawTimeAxis = ()=>{
        if (timeAxis.innerG.selectAll('*').empty()) {
            timeAxis.highlight.attr('x', labelWidth).attr('y', 0).attr('width', SVG.ABS_MAX_DATA_WIDTH).attr('height', SVG.CELL_HEIGHT + 1.5 * SVG.CELL_PADDING);
            SVG_STYLES.apply(timeAxis.highlight, 'timeHighlight');
            timeAxis.mask.attr('x', labelWidth).attr('y', 0).attr('width', SVG.ABS_MAX_DATA_WIDTH).attr('height', SVG.CELL_HEIGHT + SVG.CELL_PADDING);
  ...
  */ const dataG = dragContentG.append('g').attr('class', 'dataG');
    ...
    const redrawData = ()=>{
        // Click/drag masks
        dataMasksG.selectAll('rect').data(rowLabels).join('rect').attr('x', labelWidth).attr('y', (d, idx)=>SVG.CELL_PADDING / 2 + (rowLabels.length - idx) * (SVG.CELL_HEIGHT + SVG.CELL_PADDING)).attr('width', svgWidth - labelWidth).attr('height', SVG.CELL_HEIGHT + SVG.CELL_PADDING).attr('fill', 'transparent').style('cursor', selectionEnabled && setSitesValue ? 'pointer' : 'grab').style('outline', 'none').on('mouseover', (event, d)=>rowHighlightHover(d)).on('focus', (event, d)=>rowHighlightHover(d)).on('mouseout', (event, d)=>rowHighlightReset(true)).on('blur', (event, d)=>rowHighlightReset(true)).on('click', selectionEnabled && setSitesValue ? (event, d)=>{
  ...
  */ const dateRangeMasksG = selectionEnabled ? svg.append('g').attr('class', 'dateRangeMasksG') : null;
    const redrawDateRangeHandleMasks = ()=>{
        ...
        dateRangeMasksG.selectAll('rect').data([
            0,
            1
        ]).join('rect').attr('class', (d)=>`dateRange${d === 0 ? 'Start' : 'End'}MaskRect`).attr('x', (d)=>getYearMonthGutterX(dateRange.value[d] || TIME.MIN_YEAR_MONTH, d === 0 ? 'left' : 'right') - SVG.DATE_RANGE_MASK_WIDTH / 2 + getTimeOffset()).attr('y', 0).attr('width', SVG.DATE_RANGE_MASK_WIDTH).attr('height', svgHeight).style('cursor', 'ew-resize').style('outline', 'none').attr('fill', 'red').style('opacity', 0).style('display', sites.value.length ? null : 'none');
    };
    /**
     SVG: Selections
  */ const redrawSelections = ()=>{
        ...
        if (setSitesValue) {
            labelSelectionsG.selectAll('rect').data(Object.keys(viewSelections)).join('rect').attr('x', 0).attr('y', y).attr('width', labelWidth).attr('height', SVG.CELL_HEIGHT + SVG.CELL_PADDING).attr('fill', fill);
        }
        // Date range handles
