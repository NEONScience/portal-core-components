...
function EnhancedAvailabilityGrid(config) {
  ...
  */
  var svgRef = config.svgRef,
      ...
      rows = _config$rows === void 0 ? {} : _config$rows,
      _config$rowLabels = config.rowLabels,
      rowLabels = _config$rowLabels === void 0 ? [] : _config$rowLabels,
      _config$rowTitles = config.rowTitles,
      rowTitles = _config$rowTitles === void 0 ? {} : _config$rowTitles,
  ...
  */
  ...
  var labelWidth = rowLabels[0].length * _AvailabilityUtils.SVG.LABEL_WIDTH_MULTIPLIER + _AvailabilityUtils.SVG.LABEL_WIDTH_RIGHT_BUFFER;
  /**
  ...
  */
  ...
  var getMinTimeOffset = function getMinTimeOffset() {
    return 0 - (_AvailabilityUtils.SVG.ABS_MAX_DATA_WIDTH - (svgWidth - labelWidth));
  };
  ...
  var getYearMonthsInView = function getYearMonthsInView(totalSvgWidth, dragOffset) {
    return _AvailabilityUtils.TIME.YEAR_MONTHS.filter(function (yearMonth) {
      ...
      var lower = Math.max(0, labelWidth - margin);
  ...
  var getYearsInView = function getYearsInView(totalSvgWidth, dragOffset) {
    return _AvailabilityUtils.TIME.YEARS.filter(function (year) {
      ...
      var upper = totalSvgWidth - labelWidth + 2 * _AvailabilityUtils.SVG.YEAR_WIDTH;
  ...
  */
  ...
  var getInitialTimeOffset = function getInitialTimeOffset() {
    ...
    var ret = Math.max(0 - getYearMonthStartX(finalMonth) - _AvailabilityUtils.SVG.YEAR_MONTH_WIDTH + svgWidth, minTimeOffset);
    return ret;
  };
  /**
  ...
  */
  ...
  var getRowY = function getRowY(idx) {
    return _AvailabilityUtils.SVG.CELL_PADDING + (rowLabels.length - idx) * (_AvailabilityUtils.SVG.CELL_HEIGHT + _AvailabilityUtils.SVG.CELL_PADDING);
  };
  ...
  */
  ...
  var clipWidth = svgWidth - labelWidth;
  ...
  */
  ...
  var lboundOffset = labelWidth + _AvailabilityUtils.SVG.CELL_PADDING / 2 - 1;
  ...
  */
  ...
  var viewSelections = {}; // const sitesSet = new Set(sites.value);
  ...
  */
  ...
  var toggleSelection = function toggleSelection() {// console.log(key);
  ...
  var rowHighlightHover = function rowHighlightHover(key) {
    rowHoverKey = key;
    var offset = rowLabels.indexOf(key);

    if (offset !== -1 && !draggingCells) {
      rowHighlightReset.cancel();
      var y = _AvailabilityUtils.SVG.CELL_PADDING / 2 + (rowLabels.length - offset) * (_AvailabilityUtils.SVG.CELL_HEIGHT + _AvailabilityUtils.SVG.CELL_PADDING);
  ...
  */
  ...
  rowLabels.forEach(function (label, rowIdx) {
    var transform = getRowTranslation(label, rowIdx);
    var labelX = labelWidth - _AvailabilityUtils.SVG.CELL_PADDING;
    var rowLabelG = rowLabelsG.append('g').attr('transform', transform);
    var fill = selectionEnabled && setSitesValue && viewSelections[label] ? _Theme.default.palette.primary.contrastText : _Theme.default.palette.grey[700];
    var text = rowLabelG.append('text').attr('x', labelX).attr('y', _AvailabilityUtils.SVG.LABEL_FONT_SIZE - _AvailabilityUtils.SVG.CELL_PADDING / 2).attr('fill', fill).text(label);

    _AvailabilityUtils.SVG_STYLES.apply(text, 'rowLabel');

    var mask = rowLabelG.append('rect').attr('x', 0).attr('y', -1 * (_AvailabilityUtils.SVG.CELL_PADDING / 2)).attr('width', labelWidth).attr('height', _AvailabilityUtils.SVG.CELL_HEIGHT + _AvailabilityUtils.SVG.CELL_PADDING).on('mouseover', function () {
      return rowHighlightHover(label);
    }).on('focus', function () {
      return rowHighlightHover(label);
    }).on('mouseout', rowHighlightReset).on('blur', rowHighlightReset);

    _AvailabilityUtils.SVG_STYLES.apply(mask, 'rowLabelMask'); // Fill the mask and delay the selection to emulate a touch ripple.
  ...
  */
  ...
  var redrawData = function redrawData() {
    // Click/drag masks
    dataMasksG.selectAll('rect').data(rowLabels).join('rect').attr('x', labelWidth).attr('y', function (d, idx) {
      return _AvailabilityUtils.SVG.CELL_PADDING / 2 + (rowLabels.length - idx) * (_AvailabilityUtils.SVG.CELL_HEIGHT + _AvailabilityUtils.SVG.CELL_PADDING);
    }).attr('width', svgWidth - labelWidth).attr('height', _AvailabilityUtils.SVG.CELL_HEIGHT + _AvailabilityUtils.SVG.CELL_PADDING).attr('fill', 'transparent').style('cursor', selectionEnabled && setSitesValue ? 'pointer' : 'grab').style('outline', 'none').on('mouseover', rowHighlightHover).on('focus', rowHighlightHover).on('mouseout', rowHighlightReset).on('blur', rowHighlightReset).on('click', selectionEnabled && setSitesValue ? function (label, idx, nodes) {
      _AvailabilityUtils.SVG_STYLES.touchRipple((0, _d3Selection.select)(nodes[idx]), 15);
    ...
    dataG.selectAll('g').data(rowLabels).join('g').attr('transform', getRowTranslation).each(function (label, idx, gNodes) {
      var rowData = rows[label];

      var getDefaultStatus = function getDefaultStatus(month) {
        return month >= _AvailabilityUtils.TIME.CURRENT_MONTH ? 'not expected' : 'not available';
      };

      var getCellAttr = function getCellAttr(month, attr) {
        var status = (0, _AvailabilityUtils.calcRollupStatus)(rowData[month]);
      ...
      (0, _d3Selection.select)(gNodes[idx]).selectAll('rect').data(function () {
        return getYearMonthsInView(svgWidth, getTimeOffset());
      }).join('rect').attr('x', function (month) {
  ...
  */
  ...
  var redrawDateRangeHandles = function redrawDateRangeHandles() {
    ...
    var yBounds = Object.keys(viewSelections).reduce(function (acc, key) {
      var y = getRowY(rowLabels.indexOf(key));
  ...
  */
  ...
  var redrawSelections = function redrawSelections() {
    ...
    var y = function y(d) {
      return yOffset + (rowLabels.length - rowLabels.indexOf(d)) * yMultiplier;
    };
  ...
  */
  ...
  var dragCells = (0, _d3Drag.drag)().on('start', function () {
    ...
    if (selectionEnabled && setSitesValue && cellDragTime < 100) {
      _AvailabilityUtils.SVG_STYLES.touchRipple(dataMasksG.selectAll('rect').filter(function (d) {
  ...
  if (selectionEnabled) {
    ...
    var flipDateRangeSelectionIfNeeded = function flipDateRangeSelectionIfNeeded() {
      if (dateRange.value[1] < dateRange.value[0]) {
        dateRange.value = [_AvailabilityUtils.TIME.getNextMonth(dateRange.value[1]), _AvailabilityUtils.TIME.getPreviousMonth(dateRange.value[0])]; // Only if the handles are atop each other will this still be true after the last flip.
