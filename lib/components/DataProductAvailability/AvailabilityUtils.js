...
*/ /* eslint-disable max-len, @stylistic/max-len */ export const VALID_ENHANCED_STATUSES = {
    expected: {
        title: 'Expected',
...
/* eslint-enable max-len, @stylistic/max-len */ export const calcBasicRollupStatus = (statuses)=>{
    if (!statuses) {
        return null;
    }
...
*/ export const AvailabilityPropTypes = {
    basicSiteCodes: PropTypes.arrayOf(PropTypes.shape({
    ...
    enhancedSites: PropTypes.arrayOf(PropTypes.shape({
        siteCode: PropTypes.string.isRequired,
        tables: PropTypes.arrayOf(PropTypes.shape({
            name: PropTypes.string.isRequired,
            description: PropTypes.string.isRequired,
            waitInterval: PropTypes.string.isRequired,
            months: PropTypes.objectOf(PropTypes.oneOf(Object.keys(VALID_ENHANCED_STATUSES))).isRequired
        })).isRequired
    })),
...
*/ export const TIME = {
    getYearMonthMoment: (yearMonth)=>moment(`${yearMonth}-01`),
    getNextMonth: (month)=>moment.utc(`${month}-15T00:00:00Z`).add(1, 'month').format('YYYY-MM'),
    getPreviousMonth: (month)=>moment.utc(`${month}-15T00:00:00Z`).subtract(1, 'month').format('YYYY-MM'),
...
TIME.YEAR_MONTHS = TIME.YEARS.flatMap((year)=>TIME.MONTHS.map((month)=>`${year}-${month}`));
...
*/ SVG_STYLES.touchRipple = (selection, duration = 15)=>{
    selection.style('fill', 'rgba(199, 110, 0, 0.75)').transition(transition().duration(duration)).style('fill', 'rgba(199, 110, 0, 0.25)');
};
