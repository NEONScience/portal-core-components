...
function AvailabilityGrid(config) {
  ...
  */
  ...
  rowKeys.forEach(function (rowKey, rowIdx) {
    ...
    var maskClick = selectionEnabled && setSitesValue ? function () {
      touchRipple(mask, 15);
      setTimeout(function () {
  ...
  */
  ...
  var redrawData = function redrawData() {
    ...
    }).attr('width', svgWidth - getLabelWidth()).attr('height', SVG.CELL_HEIGHT + SVG.CELL_PADDING).attr('fill', 'transparent').style('cursor', selectionEnabled && setSitesValue ? 'pointer' : 'grab').style('outline', 'none').on('mouseover', rowHighlightHover).on('focus', rowHighlightHover).on('mouseout', rowHighlightReset).on('blur', rowHighlightReset).on('click', selectionEnabled && setSitesValue ? function (rowKey, idx, nodes) {
  ...
  */
  ...
  var redrawSelections = function redrawSelections() {
    ...
    var fill = function fill(d) {
      return viewSelections[d] === 'full' ? _Theme.default.palette.secondary.main : _Theme.COLORS.LIGHT_BLUE[200];
    };
  ...
  if (selectionEnabled) {
    ...
    var flipDateRangeSelectionIfNeeded = function flipDateRangeSelectionIfNeeded() {
      if (dateRange.value[1] < dateRange.value[0]) {
        dateRange.value = [getNextMonth(dateRange.value[1]), getPreviousMonth(dateRange.value[0])]; // Only if the handles are atop each other will this still be true after the last flip.
