...
function AvailabilityGrid(config) {
  ...
  */
  var svgRef = config.svgRef,
      ...
      sortedSites = _config$sortedSites === undefined ? [] : _config$sortedSites,
  ...
  */
  ...
  var getYearMonthsInView = function getYearMonthsInView(totalSvgWidth, dragOffset) {
    return yearMonths.filter(function (yearMonth) {
      ...
      var lower = 0;
      var upper = totalSvgWidth - getLabelWidth() + 8 * yearMonthWidth;
  ...
  */
  var redrawSelections = function redrawSelections() {
    ...
    labelSelectionsG.selectAll('rect').data(Object.keys(viewSelections)).join('rect').attr('x', 0).attr('y', y).attr('width', getLabelWidth()).attr('height', SVG.CELL_HEIGHT + SVG.CELL_PADDING).attr('fill', fill);
