...
exports.yearMonths = exports.SVG = undefined;
...
Object.keys(_sites2.default).forEach(function (site) {
  var domain = _sites2.default[site].domainCode;
  if (!siteViewMaps.domains[domain]) {
    siteViewMaps.domains[domain] = [];
  }
  siteViewMaps.domains[domain].push(site);
  var state = _sites2.default[site].stateCode;
...
/**
   Setup: Derived SVG display values
*/
// This gets used a lot!
var halfCellPad = SVG.CELL_PADDING / 2;
var getDateRangeHandleMaskWidth = function getDateRangeHandleMaskWidth() {
  var dateRangeValue = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : [null, null];

  if (dateRangeValue[0] === dateRangeValue[1] && dateRangeValue[0] !== null) {
    return SVG.CELL_WIDTH + SVG.CELL_PADDING;
  }
  return SVG.CELL_WIDTH * 2 + SVG.CELL_PADDING;
};
...
var minYearMonth = startYear + '-01';
var maxYearMonth = endYear + '-12';
...
var yearMonths = exports.yearMonths = years.flatMap(function (year) {
...
/**
   ...
   be visible.
*/
var getMinTimeOffset = function getMinTimeOffset(svgWidth) {
  return 0 - (absMaxDataWidth - (svgWidth - SVG.LABEL_WIDTH));
};
var getYearStartX = function getYearStartX(year) {
  var intYear = parseInt(year, 10);
  return SVG.LABEL_WIDTH + SVG.END_PADDING + years.indexOf(intYear) * yearWidth + years.indexOf(intYear) * SVG.YEAR_PADDING;
};
...
var getYearMonthsInView = function getYearMonthsInView(totalSvgWidth, dragOffset) {
  return yearMonths.filter(function (yearMonth) {
    ...
    var lower = 0;
    var upper = totalSvgWidth - SVG.LABEL_WIDTH + 8 * yearMonthWidth;
...
var getYearsInView = function getYearsInView(totalSvgWidth, dragOffset) {
  return years.filter(function (year) {
    ...
    var upper = totalSvgWidth - SVG.LABEL_WIDTH + 2 * yearWidth;
...
var styles = {
  ...
  timeDivider: {
    stroke: _Theme2.default.palette.grey[400],
    strokeDasharray: 3,
    strokeWidth: '1.2px'
  },
  timeBound: {
    stroke: _Theme2.default.palette.grey[700],
    strokeWidth: '2px'
  },
  timeLabel: {
...
function AvailabilityGrid(config) {
  ...
  */
  var svgRef = config.svgRef,
      ...
      sites = _config$sites === undefined ? { value: [], validValues: [] } : _config$sites,
      _config$setSitesValue = config.setSitesValue,
      setSitesValue = _config$setSitesValue === undefined ? function () {} : _config$setSitesValue,
      _config$dateRange = config.dateRange,
      dateRange = _config$dateRange === undefined ? { value: [], validValues: [minYearMonth, maxYearMonth] } : _config$dateRange,
      _config$setDateRangeV = config.setDateRangeValue,
      setDateRangeValue = _config$setDateRangeV === undefined ? function () {} : _config$setDateRangeV,
      _config$selectionEnab = config.selectionEnabled,
      selectionEnabled = _config$selectionEnab === undefined ? true : _config$selectionEnab;
  ...
  */
  ...
  var rowKeys = Object.keys(data.rows).sort().reverse();
  ...
  */
  ...
  var draggingDateRange = [{ dragging: false, centerDragX: 0 }, { dragging: false, centerDragX: 0 }];
  ...
  */
  ...
  var getRowTranslation = function getRowTranslation(d, idx) {
    return 'translate(0,' + getRowY(idx) + ')';
  };
  ...
  */
  ...
  var clipWidth = svgWidth - SVG.LABEL_WIDTH;
  var defs = svg.append('defs');
  var labelSelectionsG = svg.append('g').attr('class', 'labelSelectionsG');
  var clipPath = defs.append('clipPath').attr('id', svgId + '-clip');
  clipPath.append('rect').attr('x', SVG.LABEL_WIDTH).attr('y', 0).attr('width', clipWidth).attr('height', svgHeight);
  var clipG = svg.append('g').attr('class', 'clipG').attr('clip-path', 'url(#' + svgId + '-clip)');
  ...
  */
  var prevMinTimeOffset = getMinTimeOffset(svgWidth);
  ...
  var getTimeOffset = function getTimeOffset() {
    var minTimeOffset = getMinTimeOffset(svgWidth);
  ...
  var setTimeOffset = function setTimeOffset(timeOffset) {
    var boundedTimeOffset = Math.min(Math.max(getMinTimeOffset(svgWidth), timeOffset), 0);
  ...
  */
  var lboundOffset = SVG.LABEL_WIDTH + halfCellPad - 1;
  ...
  */
    ...
    switch (data.view) {
      ...
      default:
        ...
        Object.keys(siteViewMaps[data.view]).forEach(function (entry) {
          ...
          var intersection = new Set([].concat(_toConsumableArray(viewSites)).filter(function (s) {
  ...
  var toggleSelection = function toggleSelection(key) {
    ...
    var newSelectedSitesSet = void 0;
    if (!viewSelections[key] || viewSelections[key] === 'partial') {
      ...
      newSelectedSitesSet = new Set([].concat(_toConsumableArray(sitesSet)).filter(function (s) {
        return !allSitesForKey.has(s);
      }));
    }
    setSitesValue([].concat(_toConsumableArray(newSelectedSitesSet)));
  };
  ...
  */
  ...
  rowKeys.forEach(function (rowKey, rowIdx) {
    ...
    var labelX = SVG.LABEL_WIDTH - SVG.CELL_PADDING;
    var rowLabelG = rowLabelsG.append('g').attr('transform', transform);
    var fill = viewSelections[rowKey] ? _Theme2.default.palette.secondary.contrastText : _Theme2.default.palette.grey[700];
    ...
    var mask = rowLabelG.append('rect').attr('x', 0).attr('y', -1 * halfCellPad).attr('width', SVG.LABEL_WIDTH).attr('height', SVG.CELL_HEIGHT + SVG.CELL_PADDING).on('mouseover', function () {
  ...
  */
  ...
  var redrawTimeAxis = function redrawTimeAxis() {
    if (timeAxis.innerG.selectAll('*').empty()) {
      timeAxis.highlight.attr('x', SVG.LABEL_WIDTH).attr('y', 0).attr('width', absMaxDataWidth).attr('height', SVG.CELL_HEIGHT + 1.5 * SVG.CELL_PADDING);
      applyStyles(timeAxis.highlight, 'timeHighlight');
      timeAxis.mask.attr('x', SVG.LABEL_WIDTH).attr('y', 0).attr('width', absMaxDataWidth).attr('height', SVG.CELL_HEIGHT + SVG.CELL_PADDING);
  ...
  */
  ...
  var redrawData = function redrawData() {
    ...
    dataG.selectAll('g').data(rowKeys).join('g').attr('transform', getRowTranslation).each(function (rowKey, rowIdx, gNodes) {
      ...
      }).attr('y', 0).attr('width', SVG.CELL_WIDTH + 'px').attr('height', SVG.CELL_HEIGHT + 'px').attr('rx', SVG.CELL_RX + 'px').attr('cursor', 'pointer').attr('fill', function (d) {
        switch (rowData[d]) {
          case 'available':
            return _Theme2.default.palette.primary.main;
          default:
            return _Theme2.default.palette.grey[100];
        }
      });
    });
  };
  ...
  */
  ...
  var redrawDateRangeHandles = function redrawDateRangeHandles() {
    ...
    dateRangeHandlesG.selectAll('rect').data([0, 1]).join('rect').attr('class', function (d) {
      return 'dateRange' + (d === 0 ? 'Start' : 'End') + 'HandleRect';
    }).attr('x', function (d) {
    ...
    }).attr('stroke', _Theme2.default.palette.secondary.main).style('stroke-width', '1.5px').style('display', sites.value.length ? null : 'none');
  };
  ...
  */
  ...
  var redrawDateRangeHandleMasks = function redrawDateRangeHandleMasks() {
    ...
    dateRangeMasksG.selectAll('rect').data([0, 1]).join('rect').attr('class', function (d) {
      return 'dateRange' + (d === 0 ? 'Start' : 'End') + 'MaskRect';
    }).attr('x', function (d) {
      return getYearMonthGutterX(dateRange.value[d] || minYearMonth, d === 0 ? 'left' : 'right') - getDateRangeHandleMaskWidth(dateRange.value) / 2 + getTimeOffset();
    }).attr('y', 0).attr('width', getDateRangeHandleMaskWidth(dateRange.value)).attr('height', svgHeight).style('cursor', 'ew-resize').style('outline', 'none').attr('fill', 'red').style('opacity', 0).style('display', sites.value.length ? null : 'none');
  };
  ...
  */
  var redrawSelections = function redrawSelections() {
    ...
    var fill = function fill(d) {
      return viewSelections[d] === 'full' ? _Theme2.default.palette.secondary.main : _Theme.COLORS.SECONDARY_BLUE[200];
    };
    ...
    labelSelectionsG.selectAll('rect').data(Object.keys(viewSelections)).join('rect').attr('x', 0).attr('y', y).attr('width', SVG.LABEL_WIDTH).attr('height', SVG.CELL_HEIGHT + SVG.CELL_PADDING).attr('fill', fill);
  ...
  */
  ...
  if (selectionEnabled) {
    ...
    var dragDateRangeStart = (0, _d3Drag.drag)().on('start', function () {
      ...
      draggingDateRange[0].centerDragX = parseFloat(dragDateRangeStartMask.attr('x'), 10) + getDateRangeHandleMaskWidth(dateRange.value) / 2;
    }).on('drag', function () {
      draggingDateRange[0].centerDragX += _d3Selection.event.dx;
      var maskX = draggingDateRange[0].centerDragX - getDateRangeHandleMaskWidth(dateRange.value) / 2;
      dragDateRangeStartMask.attr('x', maskX);
      ...
      if (adjacentYearMonth !== dateRange.value[0] && distanceToAdjacent < distanceToCurrent && adjacentYearMonth <= dateRange.value[1]) {
      ...
      var maskX = getYearMonthGutterX(dateRange.value[0], 'left') + getDateRangeHandleMaskWidth(dateRange.value) / 2 + getTimeOffset();
      dateRangeMasksG.select('.dateRangeStartMaskRect').attr('x', maskX);
      setDateRangeValue([].concat(_toConsumableArray(dateRange.value)));
      redrawSelections();
    });
    ...
    var dragDateRangeEnd = (0, _d3Drag.drag)().on('start', function () {
      ...
      draggingDateRange[1].centerDragX = parseFloat(dragDateRangeEndMask.attr('x'), 10) + getDateRangeHandleMaskWidth(dateRange.value) / 2;
    }).on('drag', function () {
      draggingDateRange[1].centerDragX += _d3Selection.event.dx;
      var maskX = draggingDateRange[1].centerDragX - getDateRangeHandleMaskWidth(dateRange.value) / 2;
      dragDateRangeEndMask.attr('x', maskX);
      ...
      if (adjacentYearMonth !== dateRange.value[1] && distanceToAdjacent < distanceToCurrent && adjacentYearMonth >= dateRange.value[0]) {
      ...
      // Recenter mask as it is likely off a few pixels due to snap-to-gutter behavior
      var maskX = getYearMonthGutterX(dateRange.value[1], 'right') - getDateRangeHandleMaskWidth(dateRange.value) / 2 + getTimeOffset();
