...
const applyDoiStatusReleaseGlobally = (state, productCode, release, doiStatus) => {
  ...
  const appliedDoiStatus = _BundleService.default.determineAppliedBundleRelease((updatedState.neonContextState?.data || {}).bundles, release, productCode, doiStatus);
...
const buildCitationDownloadKey = (citationProduct, releaseCb, formatCb, provisionalCb = true) => {
  ...
  if ((0, _typeUtil.isStringNonEmpty)(releaseCb)) {
    ...
    key = `${key}-${_ReleaseService.PROVISIONAL_RELEASE}`;
  } else {
    key = `${key}-RELEASE`;
  }
