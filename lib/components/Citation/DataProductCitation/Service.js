...
import { FetchStatus, ContextStatus } from './State';
...
const fetchIsAwaitingCall = (fetchObject)=>fetchIsInStatus(fetchObject, FetchStatus.AWAITING_CALL);
...
const calculateFetches = (state)=>{
    ...
    const { productCode, release, bundle: { parentCodes } } = state;
    const { releases } = state.data;
    if (!productCode) {
        return state;
    }
    // Find the latest non-prov release definition
    const latestRelease = existsNonEmpty(releases) ? releases.find((r)=>!ReleaseService.isLatestNonProv(r.release)) : null;
...
const applyDoiStatusReleaseGlobally = (state, productCode, release, doiStatus)=>{
    ...
    const appliedDoiStatus = BundleService.determineAppliedBundleRelease((updatedState.neonContextState?.data || {}).bundles, release, productCode, doiStatus);
    ...
    const hasRelease = updatedState.data.releases.some((value)=>exists(value) && isStringNonEmpty(value.release) && isStringNonEmpty(citationRelease.release) && value.release.localeCompare(citationRelease.release) === 0);
    if (!hasRelease) {
 ...
 */ const calculateContextState = (newState, neonContextState, release, productCode)=>{
    ...
    if (isStringNonEmpty(productCode)) {
        bundles = BundleService.determineCitationBundle(neonContextState.data.bundles, release, productCode);
    }
...
const getReleaseObject = (releases, release)=>!isStringNonEmpty(release) || release === PROVISIONAL_RELEASE ? null : releases.find((r)=>r.release === release);
const getReleaseDoi = (releases, release)=>{
    const releaseObject = getReleaseObject(releases, release);
    if (!exists(releaseObject) || !exists(releaseObject.productDoi) || !isStringNonEmpty(releaseObject.productDoi.url)) {
        return null;
    }
    return releaseObject.productDoi.url;
};
const buildCitationDownloadKey = (citationProduct, releaseCb, formatCb, provisionalCb = true)=>{
...
const hasCitationDownloadStatus = (citationDownloadsFetchStatus, provisionalCb, productCode, statusCb)=>Object.keys(citationDownloadsFetchStatus).some((k)=>{
        if (citationDownloadsFetchStatus[k]) {
            ...
            if (!k.includes(productCode) || !provisionalCb && k.includes(PROVISIONAL_RELEASE) || provisionalCb && !k.includes(PROVISIONAL_RELEASE)) {
...
const useViewState = (state, props)=>{
    const { release: specifiedReleaseTag, bundle, component: { status }, fetches: { citationDownloads: citationDownloadsFetchStatus }, data: { product: baseProduct, productReleases, productReleaseDois, bundleParents, bundleParentReleases, releases }, neonContextState: { data: neonContextStateData } } = state;
    const bundlesContext = neonContextStateData.bundles;
    const { disableConditional } = props;
    const hasReleases = existsNonEmpty(releases);
    // Identify the latest release state.
    const latestReleaseObject = hasReleases ? releases.find((r)=>r.showCitation === true) : null;
    const hasLatestRelease = exists(latestReleaseObject) && isStringNonEmpty(latestReleaseObject.release);
    ...
    const hasSpecifiedRelease = isStringNonEmpty(specifiedReleaseTag);
    const specifiedReleaseObject = hasSpecifiedRelease && hasReleases ? getReleaseObject(releases, specifiedReleaseTag) : null;
    const isSpecifiedReleaseValid = exists(specifiedReleaseObject) && isStringNonEmpty(specifiedReleaseObject.release);
    const isSpecifiedReleaseLatestNonProv = hasSpecifiedRelease ? ReleaseService.isLatestNonProv(specifiedReleaseTag) : false;
    ...
    const hasAppliedRelease = exists(appliedReleaseObject);
    const isAppliedReleaseLatestNonProv = isStringNonEmpty(appliedRenderedReleaseTag) ? ReleaseService.isLatestNonProv(appliedRenderedReleaseTag) : false;
    ...
    const hideAppliedReleaseCitation = exists(appliedReleaseObject) && appliedReleaseObject.showCitation === false;
    // Determine tombstoned state for entire citation
    const hasProductReleaseDois = exists(productReleaseDois) && isStringNonEmpty(appliedRenderedReleaseTag) && exists(productReleaseDois[appliedRenderedReleaseTag]);
    let dataProductDoiStatus = null;
    let isTombstoned = false;
    const determineDoiStatusTombstone = (dpds)=>{
        let tsResult = false;
        if (!Array.isArray(dpds)) {
            const coercedDpds = dpds;
            const doiStatusType = coercedDpds?.status;
    ...
    const hasBundleCode = existsNonEmpty(bundle.parentCodes) && (isStringNonEmpty(bundle.doiProductCode) || Array.isArray(bundle.doiProductCode) && existsNonEmpty(bundle.doiProductCode));
    ...
    const buildCitationItem = (bundleParentCode, bundleDpds)=>{
        ...
        if (hasBundleCode && isStringNonEmpty(bundleParentCode) && exists(bundleParents[bundleParentCode])) {
            bundleProduct = bundleParents[bundleParentCode];
        }
        const hasBundleProduct = exists(bundleProduct);
        ...
        if (hasLatestRelease && exists(baseProduct)) {
            hasLatestReleaseBundle = BundleService.isProductInBundle(bundlesContext, latestReleaseObject.release, baseProduct.productCode);
        }
        ...
        if (hasBundleProduct && hasSpecifiedRelease && !isAppliedReleaseLatestNonProv) {
            const bundleHasRelease = bundleProduct?.releases.find((r)=>r.release === appliedRenderedReleaseTag);
        ...
        if (hasBundleCode && isStringNonEmpty(bundleParentCode) && exists(bundleDpds)) {
            const bundleCitationRelease = ReleaseService.transformDoiStatusRelease(bundleDpds);
    ...
    const hasValidProduct = items.some((item)=>exists(item.citableBaseProduct));
    const hasValidReleaseProduct = items.some((item)=>exists(item.citableReleaseProduct));
    ...
    if (isAppliedReleaseLatestNonProv) {
        ...
        const productHasRelease = items.some((item)=>{
            const dpr = item.citableReleaseProduct?.releases.find((r)=>r.release === appliedRenderedReleaseTag);
    ...
    let displayType = DisplayType.CONDITIONAL;
    const isReady = status === ContextStatus.READY;
    const isError = status === ContextStatus.ERROR;
