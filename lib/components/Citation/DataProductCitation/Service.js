...
var applyDoiStatusReleaseGlobally = function applyDoiStatusReleaseGlobally(state, doiStatus) {
  var updatedState = _extends({}, state);

  var transformedRelease = _ReleaseService.default.transformDoiStatusRelease(doiStatus);
...
var hasCitationDownloadStatus = function hasCitationDownloadStatus(citationDownloadsFetchStatus, provisionalCb, statusCb) {
  return Object.keys(citationDownloadsFetchStatus).some(function (k) {
    if (citationDownloadsFetchStatus[k]) {
      var shouldConsider = true;

      if (!provisionalCb && k.includes(_ReleaseService.PROVISIONAL_RELEASE)) {
        shouldConsider = false;
      } else if (provisionalCb && !k.includes(_ReleaseService.PROVISIONAL_RELEASE)) {
        shouldConsider = false;
      }
...
var handleResetCitationDownloads = function handleResetCitationDownloads(citationDownloadsFetchStatus, provisionalCb, dispatch) {
...
var useViewState = function useViewState(state, props) {
  ...
  var hideLatestReleaseCitation = hasLatestRelease ? latestReleaseObject.showCitation === false : false; // Identify state of specified release.
  ...
  var isSpecifiedReleaseLatestNonProv = hasSpecifiedRelease ? _ReleaseService.default.isLatestNonProv(specifiedReleaseTag) : false; // Determine the release tag to render for the citation, either for the
  ...
  var hideAppliedReleaseCitation = (0, _typeUtil.exists)(appliedReleaseObject) && appliedReleaseObject.showCitation === false; // Determine tombstoned state

  var isTombstoned = false;

  if ((0, _typeUtil.exists)(productReleaseDois) && (0, _typeUtil.isStringNonEmpty)(appliedRenderedReleaseTag) && (0, _typeUtil.exists)(productReleaseDois[appliedRenderedReleaseTag])) {
    var dataProductDoiStatus = productReleaseDois[appliedRenderedReleaseTag];
    var doiStatusType = dataProductDoiStatus === null || dataProductDoiStatus === void 0 ? void 0 : dataProductDoiStatus.status;
    isTombstoned = (0, _typeUtil.exists)(doiStatusType) && doiStatusType === _neonApi.DoiStatusType.TOMBSTONED || false;
  ...
  if (isAppliedReleaseLatestNonProv) {
    ...
    isCitableReleaseProductInRelease = (0, _typeUtil.exists)(productHasRelease) || isTombstoned;
