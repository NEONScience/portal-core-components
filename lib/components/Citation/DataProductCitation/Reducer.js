...
var reinitialize = function reinitialize(state, action) {
  var defaultState = (0, _State.getDefaultState)();
...
var resolveError = function resolveError(action, state) {
  ...
  var fetchProductReleaseDoiFailedAction;
  var fetchBundleParentFailedAction;
  var fetchBundleParentReleaseFailedAction;
  var fetchCitationDownloadFailedAction;
  switch (action.type) {
...
var Reducer = function Reducer(state, action) {
  ...
  var fprdStartedAction;
  var fprdFailedAction;
  var fprdSucceededAction;
  ...
  var fcdStartedAction;
  var fcdFailedAction;
  var fcdSucceededAction;
  var fcdResetAction;
  switch (action.type) {
    ...
    case _Actions.ActionTypes.FETCH_PRODUCT_RELEASE_DOI_SUCCEEDED:
      ...
      if ((0, _typeUtil.exists)(productReleaseDoiStatus) && (0, _typeUtil.exists)(productReleaseDoiStatus.status)) {
        newState.data.productReleaseDois[release] = productReleaseDoiStatus;
      } else {
        newState.data.productReleaseDois[release] = null;
      }
      return _Service.default.calculateAppStatus(_Service.default.applyDoiStatusReleaseGlobally(newState, productReleaseDoiStatus));
