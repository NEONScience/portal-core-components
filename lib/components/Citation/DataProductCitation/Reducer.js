...
const Reducer = (state, action)=>{
    ...
    let productReleaseDoiStatusArray;
    let release;
    let bundleParent;
        ...
        case ActionTypes.FETCH_PRODUCT_RELEASE_DOI_SUCCEEDED:
            ...
            if (!exists(productReleaseDoiStatus)) {
                ...
                if (existsNonEmpty(productReleaseDoiStatus)) {
                    productReleaseDoiStatusArray = productReleaseDoiStatus;
                    newState.data.productReleaseDois[release] = productReleaseDoiStatusArray.filter((dpds)=>exists(dpds) && exists(dpds.status));
                } else {
                    newState.data.productReleaseDois[release] = null;
                }
