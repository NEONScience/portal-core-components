 ...
 * @param {*} limited
 */ const buildFetchProductObservable = (action, takeUntilOperator, limited)=>{
    const fetchProductAction = action;
    let productObs;
    if (limited) {
        productObs = NeonGraphQL.getDemoDataProductByCode(fetchProductAction.productCode, true);
    } else {
        productObs = NeonGraphQL.getDataProductByCode(fetchProductAction.productCode, fetchProductAction.release, true);
    }
    return productObs.pipe(mergeMap((response)=>{
 ...
 */ const buildDataApiObservable = (action, takeUntilOperator, limited, sessionHeaders)=>{
    // Get query params to send with request
    const queryParams = getApiDataQueryParams(action);
    const url = getDataApiRequest(queryParams.productCode, queryParams.release, queryParams.site, queryParams.month, limited);
    // XHR for API data endpoint to retrieve available files
    const headers = {
        ...sessionHeaders
    };
    return NeonApi.getJsonAjaxObservable(url, headers).pipe(mergeMap((response)=>buildFilesAjaxObservable(action, response, takeUntilOperator)), catchError((error)=>{
 ...
 */ const createFetchProductEpic = (limited)=>(action$)=>action$.pipe(filter((action)=>action.type === "FETCH_PRODUCT"), switchMap((action)=>concat(of(ActionCreator.fetchProductStarted()), buildFetchProductObservable(action, takeUntil(cancelProduct$), limited))));
/**
 * Epic for fetching wind rose data
 * @param {*} sessionHeaders Action stream
 */ const createFetchWindRoseDataEpic = (limited, sessionHeaders)=>(action$)=>action$.pipe(filter((action)=>action.type === "FETCH_WIND_ROSE"), switchMap((action)=>concat(of(ActionCreator.fetchWindRoseWorking()), buildDataApiObservable(action, takeUntil(cancelWindRose$), limited, sessionHeaders))));
...
const Provider = (inProps)=>{
    ...
    const sessionHeadersStringified = JSON.stringify(sessionHeaders);
    useEffect(()=>{
        ...
        const subscriptions = [
            createFetchProductEpic(isViewerLimited)(actionStream$).subscribe(enhancedDispatch),
            createFetchWindRoseDataEpic(isViewerLimited, appliedSessionHeaders)(actionStream$).subscribe(enhancedDispatch)
        ];
