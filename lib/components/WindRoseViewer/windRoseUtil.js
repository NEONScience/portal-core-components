...
exports.putWindRoseBin = exports.parseWindRoseDataFiles = exports.buildWindRoseData = exports.WIND_ROSE_CHANGE_TYPE = exports.VELOCITY_BINS = exports.DIRECTION_BIN_LOOKUP = void 0;
...
const VELOCITY_BINS = exports.VELOCITY_BINS = ['0-.25', '.25-.5', '.5-.75', '.75-1', '1-1.5', '1.5-2', '2-3', '3-5', '>5'];
...
const DIRECTION_BIN_LOOKUP = exports.DIRECTION_BIN_LOOKUP = {
  0: {
    angle: 0,
    direction: 'N'
  },
 ...
 */
const WIND_ROSE_CHANGE_TYPE = exports.WIND_ROSE_CHANGE_TYPE = {
  SITE: 'SITE_CHANGED',
  MONTH: 'MONTH_CHANGED',
 ...
 * @param {*} query
 */
const parseWindRoseDataFiles = (data, query) => (0, _dataUtil.parseDataFiles)(data, query, FILE_RESOLUTION_MATCHERS);
exports.parseWindRoseDataFiles = parseWindRoseDataFiles;
const putWindRoseBin = (windRoseBins, dirBin, speedValue) => {
 ...
 * @param {*} dataFilesInfo
 */
exports.putWindRoseBin = putWindRoseBin;
const buildWindRoseData = (data, dataFilesInfo) => {
  const builtData = {
  ...
  data.forEach(fileData => {
    ...
    fileData.data.forEach(rowData => {
      ...
      if (seriesTimeValue !== undefined && seriesTimeValue !== null) {
        seriesTimeValue = (0, _dataUtil.convertDateToUTC)(seriesTimeValue);
        // Build for each field in row
        rowFieldKeys.forEach(fieldName => {
          if (typeof rowData[fieldName] !== 'undefined' && rowData[fieldName] !== null && rowData[fieldName].length > 0) {
      ...
      if (seriesTimeValue === undefined || seriesTimeValue === null) {
        ...
        const windRoseDayKey = `${seriesTimeDate.getUTCFullYear().toString()}` + `-${(seriesTimeDate.getUTCMonth() + 1).toString()}` + `-${seriesTimeDate.getUTCDate().toString()}`;
        let windRoseBins = {};
...
exports.buildWindRoseData = buildWindRoseData;