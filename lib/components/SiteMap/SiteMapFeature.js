...
var SiteMapFeature = function SiteMapFeature(props) {
  ...
  */
  ...
  var selectionActive = state.selection.active === featureType || state.selection.active === _SiteMapUtils.FEATURE_TYPES.SITES && [_SiteMapUtils.FEATURES.DOMAINS.KEY, _SiteMapUtils.FEATURES.STATES.KEY].includes(featureKey); // Jump-To function to afford map navigation where appropriate
  ...
  */
  ...
  var renderSitePopup = function renderSitePopup(siteCode) {
    ...
    var renderActions = function renderActions() {
      if (selectionActive) {
        var isSelectable = !validItems || validItems.has(site.siteCode);
        ...
        var action = 'This site cannot be selected';
  ...
  var renderShape = function renderShape(primaryId) {
    ...
    if (featureShape === 'Marker' && isPoint(shapeData)) {
      ...
      if (state.map.zoomedIcons[featureKey] !== null) {
        ...
        interaction = selectionActive ? {
          ...
          onClick: function onClick() {
            if (featureType === _SiteMapUtils.FEATURE_TYPES.SITES && shapeData.siteCode && isSelectable) {
              dispatch({
    ...
    switch (featureShape) {
      ...
      case 'Polygon':
        ...
        if (!positionsArrayIsValid(positions, featureType === _SiteMapUtils.FEATURE_TYPES.SAMPLING_POINTS)) {
          return null;
        }
  ...
  */
  ...
  return /*#__PURE__*/_react.default.createElement(_reactLeaflet.FeatureGroup, null, Object.keys(featureData) // Valid items should render above unselecatbles
  .sort(function (a) {
    if (!validItems) {
      return 0;
    }
