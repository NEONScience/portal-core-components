  ...
  var renderShape = function renderShape(siteCode) {
    ...
    if (shapeData.geometry && shapeData.geometry.coordinates) {
      ...
      if (shape === 'Polygon') {
        ...
        if (siteCode === state.focusLocation.current && !location || location === state.focusLocation.current) {
    ...
    if (isPoint(shapeData)) {
      ...
      if (state.map.zoomedIcons[featureKey] !== null) {
        ...
        var initialHighlight = (location || siteCode) === focusLocation && featureKey !== _SiteMapUtils.FEATURES.POUR_POINTS.KEY ? _SiteMapUtils.HIGHLIGHT_STATUS.HIGHLIGHT : _SiteMapUtils.HIGHLIGHT_STATUS.NONE;
