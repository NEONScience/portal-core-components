...
var SiteMapFeature = function SiteMapFeature(props) {
  ...
  var polygonRef = (0, _react.useRef)(null);
  ...
  */
  ...
  var selectedItems = selectionActive && state.selection[featureType] ? state.selection[featureType] : new Set();

  if (state.selection.active === _SiteMapUtils.FEATURE_TYPES.SITES && [_SiteMapUtils.FEATURES.DOMAINS.KEY, _SiteMapUtils.FEATURES.STATES.KEY].includes(featureKey)) {
    selectionActive = true;
    selectedItems = state.selection[_SiteMapUtils.FEATURE_TYPES.SITES];
  ...
  */
  ...
  var renderBoundarySelectionAction = function renderBoundarySelectionAction(boundaryFeatureKey, boundaryKey) {
    if (!selectionActive || !state.selection.derived[boundaryFeatureKey]) {
      return null;
    }
    ...
    var action = boundarySites.size === 1 ? 'add this one site' : "add all ".concat(boundarySites.size, " sites");
    var preposition = 'to';
    var snackbarClass = classes.addToSelectionSnackbar;
    var snackbarIconClass = classes.addToSelectionSnackbarIcon;

    if (selectionPortion === _SiteMapUtils.SELECTION_PORTIONS.PARTIAL) {
      var intersection = new Set(_toConsumableArray(boundarySites).filter(function (x) {
        return selectedItems.has(x);
      }));
      var remaining = boundarySites.size - intersection.size;
      action = "add remaining ".concat(remaining, " site").concat(remaining === 1 ? '' : 's');
    }

    if (selectionPortion === _SiteMapUtils.SELECTION_PORTIONS.TOTAL) {
      action = boundarySites.size === 1 ? 'remove this one site' : "remove all ".concat(boundarySites.size, " sites");
      preposition = 'from';
    ...
    return /*#__PURE__*/_react.default.createElement("div", {
      ...
      message: /*#__PURE__*/_react.default.createElement("div", {
        ...
        variant: "body2"
      }, "Click to ", /*#__PURE__*/_react.default.createElement("b", null, action), " ", preposition, " selection")))
    }));
  };
  /**
     Render: Site Popup
  ...
  var renderSitePopup = function renderSitePopup(siteCode) {
    ...
    var renderActions = function renderActions() {
      if (selectionActive) {
        ...
        var snackbarClass = isSelected ? classes.removeFromSelectionSnackbar : classes.addToSelectionSnackbar;
        var snackbarIconClass = isSelected ? classes.removeFromSelectionSnackbarIcon : classes.addToSelectionSnackbarIcon;
  ...
  */
  ...
  var renderPopupFunctions = {
    ...
    DOMAINS: function DOMAINS(domainCode) {
      ...
      return renderBoundaryPopup(domainCode, title, [jumpLink, renderChildSites, renderBoundarySelectionAction(_SiteMapUtils.FEATURES.DOMAINS.KEY, domainCode)]);
    },
    ...
    STATES: function STATES(stateCode) {
      ...
      return renderBoundaryPopup(stateCode, featureData[stateCode] ? featureData[stateCode].name : stateCode, [jumpLink, renderChildSites, renderBoundarySelectionAction(_SiteMapUtils.FEATURES.STATES.KEY, stateCode)]);
    },
  ...
  */
  ...
  var renderShape = function renderShape(primaryId) {
    ...
    if (shapeData.geometry && shapeData.geometry.coordinates) {
      positions = shapeData.geometry.coordinates; // Polyline
    ...
    if (featureShape === 'Marker' && isPoint(shapeData)) {
      ...
      if (state.map.zoomedIcons[featureKey] !== null) {
        ...
        interaction = selectionActive ? {
          ...
          onClick: function onClick() {
            if (featureType === _SiteMapUtils.FEATURE_TYPES.SITES && shapeData.siteCode) {
              dispatch({
                type: 'toggleSiteSelected',
                site: shapeData.siteCode
              });
            }
          }
        } : {
