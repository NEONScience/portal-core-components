...
const calculateZoomState = function (zoom, newState) {
  ...
  appliedState.map.status = (0, _SiteMapUtils.determineMapStatus)(newState);
  appliedState = updateMapTileWithZoom(appliedState);
  if (!init && appliedState.map.status === _SiteMapUtils.MAP_STATE_STATUS_TYPE.READY) {
...
const reducer = (state, action) => {
  ...
  switch (action.type) {
    ...
    // Map
    case 'setMapZoom':
      if (!(0, _SiteMapUtils.zoomIsValid)(action.zoom)) {
        return state;
      }
      newState.map.zoom = action.zoom;
      if ((0, _SiteMapUtils.centerIsValid)(action.center)) {
        newState.map.center = action.center;
      }
      if ((0, _SiteMapUtils.boundsAreValid)(action.bounds)) {
        newState.map.bounds = action.bounds;
      }
      newState.map.status = (0, _SiteMapUtils.determineMapStatus)(newState);
      if (newState.map.status === _SiteMapUtils.MAP_STATE_STATUS_TYPE.READY) {
    ...
    case 'setMapBounds':
      ...
      newState.map.bounds = action.bounds;
      newState.map.status = (0, _SiteMapUtils.determineMapStatus)(newState);
      if (currentMapBoundsMapStatus === _SiteMapUtils.MAP_STATE_STATUS_TYPE.INIT && newState.map.status === _SiteMapUtils.MAP_STATE_STATUS_TYPE.READY) {
        newState = calculateZoomState(newState.map.zoom, newState);
      } else {
    ...
    case 'setMapCenter':
      ...
      newState.map.center = [...action.center];
      newState.map.status = (0, _SiteMapUtils.determineMapStatus)(newState);
      if (currentMapCenterMapStatus === _SiteMapUtils.MAP_STATE_STATUS_TYPE.INIT && newState.map.status === _SiteMapUtils.MAP_STATE_STATUS_TYPE.READY) {
        newState = calculateZoomState(newState.map.zoom, newState);
      } else {
        newState = calculateFeatureDataFetches(newState);
      }
    ...
    case 'showFullObservatory':
      ...
      newState.map.zoom = (0, _SiteMapUtils.deriveFullObservatoryZoomLevel)(action.mapRef.current);
      return newState;
...
const Provider = inProps => {
  const props = (0, _defaultProps.resolveProps)(_SiteMapUtils.SITE_MAP_DEFAULT_PROPS, inProps);
  const {
    view,
    aspectRatio,
    fullscreen,
  ...
  */
  ...
  const hasInitialZoom = typeof mapZoom === 'number' && (0, _SiteMapUtils.zoomIsValid)(mapZoom);
...
Provider.propTypes = {
  ..._SiteMapUtils.SITE_MAP_PROP_TYPES,
...
const getTestableItems = () => process.env.NODE_ENV !== 'test' ? {} : {
  ...
  validateSelection,
  isBasePlot,
  zoomIsValid: _SiteMapUtils.zoomIsValid,
  centerIsValid: _SiteMapUtils.centerIsValid,
