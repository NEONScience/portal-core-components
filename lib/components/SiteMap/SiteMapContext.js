...
var _FetchLocations = _interopRequireDefault(require("./FetchLocations.worker"));

var _FetchLocationHierarchy = _interopRequireDefault(require("./FetchLocationHierarchy.worker"));
  ...
  var setFetchStatusFromAction = function setFetchStatusFromAction(status) {
    ...
    if (dataSource === _SiteMapUtils.FEATURE_DATA_SOURCES.REST_LOCATIONS_API) {
      ...
      if (status === _SiteMapUtils.FETCH_STATUS.SUCCESS && _data) {
        var parsedData = (0, _SiteMapWorkerSafeUtils.parseLocationData)(_data);
        ...
        newState.featureData[_featureType][_featureKey][_siteCode][location] = _extends(_extends({}, parsedData), {}, {
          siteCode: _siteCode,
    ...
    if (dataSource === _SiteMapUtils.FEATURE_DATA_SOURCES.GRAPHQL_LOCATIONS_API) {
      ...
      if (status === _SiteMapUtils.FETCH_STATUS.SUCCESS && _data2) {
        ...
        Object.keys(_data2).forEach(function (locName) {
          ...
          newState.featureData[featureType][featureKey][_siteCode2][locName] = _extends(_extends({}, _data2[locName]), {}, {
...
var Provider = function Provider(props) {
  ...
  */
  ...
  (0, _react.useEffect)(function () {
    ...
    var worker = new _FetchLocations.default();
    worker.addEventListener('message', function (message) {
      var _message$data = message.data,
          status = _message$data.status,
  ...
  */
  ...
  (0, _react.useEffect)(function () {
    ...
    Object.keys(state.featureDataFetches[hierarchiesSource][hierarchiesType]).forEach(function (domainCode) {
      ...
      var worker = new _FetchLocationHierarchy.default();
      worker.addEventListener('message', function (message) {
        var _message$data2 = message.data,
            status = _message$data2.status,
            data = _message$data2.data,
            error = _message$data2.error;
        ...
        Object.keys(gqlLocFetches[minZoom][siteCode].fetches).filter(function (fetchId) {
          ...
          worker.addEventListener('message', function (message) {
            var _message$data3 = message.data,
                status = _message$data3.status,
                data = _message$data3.data,
                error = _message$data3.error;
