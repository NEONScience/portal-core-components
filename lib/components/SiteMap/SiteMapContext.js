...
var deriveRegionSelections = function deriveRegionSelections(state) {
  ...
  var derive = function derive(featureKey) {
    ...
    Object.keys(state.featureData[featureType][featureKey]).forEach(function (boundaryCode) {
      var boundarySitesSet = state.featureData[featureType][featureKey][boundaryCode].sites || new Set();
...
var getSelectableSet = function getSelectableSet(setArg) {
  var validSet = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : null;
  if (!(setArg instanceof Set) && !Array.isArray(setArg)) {
    return new Set();
  }
  var set = setArg instanceof Set ? setArg : new Set(setArg);
  return validSet instanceof Set ? new Set(_toConsumableArray(set).filter(function (item) {
...
var calculateFeatureDataFetches = function calculateFeatureDataFetches(state) {
  ...
  var fetchablePrototypeSites = (state.manualLocationData || []).filter(function (manualLocation) {
    return manualLocation.manualLocationType === _SiteMapUtils.MANUAL_LOCATION_TYPES.PROTOTYPE_SITE && state.sites[manualLocation.siteCode];
  });
  var sitesToConsider = state.sites;
  if (state.manualLocationData && fetchablePrototypeSites.length) {
    sitesToConsider = {};
    fetchablePrototypeSites.forEach(function (manualLocation) {
      sitesToConsider[manualLocation.siteCode] = state.sites[manualLocation.siteCode];
    });
  }
  var sitesInMap = (0, _SiteMapUtils.calculateLocationsInBounds)(sitesToConsider, state.map.bounds, true, 0.06);
  ...
  if (state.map.zoom >= _SiteMapUtils.SITE_LOCATION_HIERARCHIES_MIN_ZOOM && state.map.bounds) {
    ...
    var hierarchiesType = _SiteMapUtils.FEATURE_TYPES.SITE_LOCATION_HIERARCHIES.KEY;
  ...
  .filter(function (featureKey) {
    ...
    var _FEATURES$featureKey = _SiteMapUtils.FEATURES[featureKey],
      ...
      matchLocationType = _FEATURES$featureKey.matchLocationType;
  ...
  Object.keys(_SiteMapUtils.GRAPHQL_LOCATIONS_API_CONSTANTS.MINZOOM_TO_FEATURES_MAP).filter(function (minZoom) {
    ...
    _SiteMapUtils.GRAPHQL_LOCATIONS_API_CONSTANTS.MINZOOM_TO_FEATURES_MAP[minZoom].forEach(function (featureKey) {
      ...
      var companionMinZoom = companionFeatureKey ? _SiteMapUtils.GRAPHQL_LOCATIONS_API_CONSTANTS.FEATURES_TO_MINZOOM_MAP[companionFeatureKey] : null;
      ...
      .filter(function (siteCode) {
        ...
        var getInitialSiteCodeFetchObject = function getInitialSiteCodeFetchObject(fetchMinZoom) {
          return {
            features: Object.fromEntries(_SiteMapUtils.GRAPHQL_LOCATIONS_API_CONSTANTS.MINZOOM_TO_FEATURES_MAP[fetchMinZoom].filter(function (fKey) {
        ...
        if (isBasePlot(featureKey) && companionFeatureKey && companionMinZoom) {
          if (!newState.featureDataFetches[dataSource][companionMinZoom][siteCode]) {
            ...
            var companionFeatureObject = newState.featureDataFetches[dataSource][companionMinZoom][siteCode].features[companionFeatureKey];
        ...
        var _newState$featureData = newState.featureDataFetches[dataSource][minZoom][siteCode],
          ...
          fetches = _newState$featureData.fetches;
...
var updateMapTileWithZoom = function updateMapTileWithZoom(state) {
  var newState = _extends({}, state);
 ...
 */
var calculateZoomState = function calculateZoomState(zoom, newState) {
  var init = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;
  var appliedState = newState;
...
var completeOverallFetch = function completeOverallFetch(state) {
  var newState = _extends({}, state);
...
var applyFeatureVisibilityToChildren = function applyFeatureVisibilityToChildren(state, feature, visible) {
...
var applyFeatureVisibilityToParents = function applyFeatureVisibilityToParents(state, feature) {
  var newState = _extends({}, state);
...
var setFetchStatusFromAction = function setFetchStatusFromAction(state, action, status) {
  ...
  if (dataSource === _SiteMapUtils.FEATURE_DATA_SOURCES.REST_LOCATIONS_API) {
    ...
    if (!newState.featureDataFetches[dataSource] || !newState.featureDataFetches[dataSource][_featureKey] || !newState.featureDataFetches[dataSource][_featureKey][_siteCode] || !newState.featureDataFetches[dataSource][_featureKey][_siteCode][location]) {
      return newState;
    }
    newState.featureDataFetches[dataSource][_featureKey][_siteCode][location] = status;
    // If the status is SUCCESS and the action has data, also commit the data
  ...
  if (dataSource === _SiteMapUtils.FEATURE_DATA_SOURCES.GRAPHQL_LOCATIONS_API) {
    ...
    newState.featureDataFetches[dataSource][minZoom][_siteCode2].fetches[fetchId].status = status;
    // If the status is SUCCESS and the action has data, also commit the data
    if (status === _SiteMapUtils.FETCH_STATUS.SUCCESS && _data2) {
      ...
      Object.keys(_data2).forEach(function (locName) {
        ...
        var _FEATURES$featureKey3 = _SiteMapUtils.FEATURES[featureKey],
          ...
          matchLocationCoordinateMap = _FEATURES$featureKey3.matchLocationCoordinateMap;
        ...
        if (isBasePlot(featureKey)) {
          var hierarchy = newState.featureData[_SiteMapUtils.FEATURE_TYPES.SITE_LOCATION_HIERARCHIES.KEY][_siteCode2]; // eslint-disable-line max-len
          ...
          newState.featureData[featureType][featureKey][_siteCode2][locName].samplingModules = Object.keys(hierarchy).reduce(function (acc, cur) {
            ...
            return k !== 'all' && k !== 'mfb' && _SiteMapUtils.PLOT_SAMPLING_MODULES[k];
          }).sort(function (a, b) {
            return _SiteMapUtils.PLOT_SAMPLING_MODULES[a] > _SiteMapUtils.PLOT_SAMPLING_MODULES[b] ? 1 : -1;
          });
        }
      });
    }
...
var reducer = function reducer(state, action) {
  ...
  switch (action.type) {
    ...
    case 'setFocusLocationFetchSucceeded':
      ...
      newState.focusLocation.map.center = _toConsumableArray(newState.map.center);
    ...
    case 'toggleSitesSelectedForState':
      ...
      getSelectableSet(newState.featureData[_SiteMapUtils.FEATURE_TYPES.STATES.KEY][_SiteMapUtils.FEATURES.STATES.KEY][action.stateCode].sites, validSet).forEach(function (siteCode) {
    ...
    case 'toggleSitesSelectedForDomain':
      ...
      getSelectableSet(newState.featureData[_SiteMapUtils.FEATURE_TYPES.DOMAINS.KEY][_SiteMapUtils.FEATURES.DOMAINS.KEY][action.domainCode].sites, validSet).forEach(function (siteCode) {
        newState.selection.set[setMethod](siteCode);
      });
      /* eslint-enable max-len */
      newState.selection.changed = true;
...
var Provider = function Provider(props) {
  var view = props.view,
    ...
    tableFullHeight = props.tableFullHeight,
    manualLocationData = props.manualLocationData,
  ...
  */
  ...
  if (Object.keys(_SiteMapUtils.FEATURE_TYPES).filter(function (k) {
    return _SiteMapUtils.FEATURE_TYPES[k].selectable;
  }).includes(selection)) {
  ...
  var stateKey = "siteMapContextState-".concat(mapUniqueId);
  ...
  var hasInitialZoom = typeof mapZoom === 'number' && zoomIsValid(mapZoom);
  ...
  if (savedState && shouldRestoreState) {
    ...
    var restoredState = (0, _StateStorageConverter.convertStateFromStorage)(savedState, initialState);
  ...
  */
  (0, _react.useEffect)(function () {
    ...
    var _state$featureData$FE = state.featureData[_SiteMapUtils.FEATURE_TYPES.STATES.KEY][_SiteMapUtils.FEATURES.STATES.KEY],
      statesData = _state$featureData$FE === void 0 ? {} : _state$featureData$FE;
    var _state$featureData$FE2 = state.featureData[_SiteMapUtils.FEATURE_TYPES.DOMAINS.KEY][_SiteMapUtils.FEATURES.DOMAINS.KEY],
      domainsData = _state$featureData$FE2 === void 0 ? {} : _state$featureData$FE2;
    ...
    if (state.manualLocationData) {
      var manualSite = state.manualLocationData.find(function (ml) {
        return ml.siteCode === current;
      });
      if (manualSite) {
        var _latitude3 = manualSite.latitude,
          _longitude3 = manualSite.longitude;
        var _timeout3 = window.setTimeout(function () {
          dispatch({
            type: 'setFocusLocationFetchSucceeded',
            data: {
              type: 'SITE',
        ...
        return function () {
          return window.clearTimeout(_timeout3);
        };
      }
    }
  ...
  (0, _react.useEffect)(function () {
    ...
    Object.keys(state.featureDataFetches[arcgisSource]).forEach(function (featureKey) {
      Object.keys(state.featureDataFetches[arcgisSource][featureKey]).forEach(function (siteCode) {
        ...
        _NeonApi.default.getArcgisAssetObservable(featureKey, siteCode).pipe((0, _rxjs.map)(function (response) {
          if (response) {
            dispatch({
    ...
    Object.keys(restLocFetches).filter(function (featureKey) {
      return featureKey !== _SiteMapUtils.FEATURE_TYPES.SITE_LOCATION_HIERARCHIES.KEY;
    }).forEach(function (featureKey) {
...
var _default = SiteMapContext; // Additional items exported for unit testing
exports.default = _default;
var getTestableItems = function getTestableItems() {
  return process.env.NODE_ENV !== 'test' ? {} : {
    deriveRegionSelections: deriveRegionSelections,
    isSelectable: isSelectable,
    getSelectableSet: getSelectableSet,
    validateSelection: validateSelection,
    isBasePlot: isBasePlot,
    zoomIsValid: zoomIsValid,
    centerIsValid: centerIsValid,
    calculateFeatureDataFetches: calculateFeatureDataFetches,
    updateMapTileWithZoom: updateMapTileWithZoom,
    completeOverallFetch: completeOverallFetch,
    applyFeatureVisibilityToChildren: applyFeatureVisibilityToChildren,
    applyFeatureVisibilityToParents: applyFeatureVisibilityToParents,
    setFetchStatusFromAction: setFetchStatusFromAction,
    reducer: reducer
  };
};
