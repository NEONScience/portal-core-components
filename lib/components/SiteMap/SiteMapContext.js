...
var calculateFeatureDataFetches = function calculateFeatureDataFetches(state) {
  ...
  .filter(function (featureKey) {
    ...
    var _FEATURES$featureKey2 = _SiteMapUtils.FEATURES[featureKey],
        ...
        matchLocationPattern = _FEATURES$featureKey2.matchLocationPattern;
    ...
    locationsInMap.forEach(function (locationCode) {
      ...
      parentLocations[locationCode].children.filter(function (childLocation) {
        return !newState.featureDataFetches[featureType][featureKey][siteCode][childLocation] && (!matchLocationPattern || matchLocationPattern.test(childLocation));
...
var reducer = function reducer(state, action) {
  ...
  var setFetchStatusFromAction = function setFetchStatusFromAction(status) {
    ...
    var _FEATURES$featureKey3 = _SiteMapUtils.FEATURES[featureKey],
        ...
        _FEATURES$featureKey4 = _FEATURES$featureKey3.matchLocationCoordinateMap,
        matchLocationCoordinateMap = _FEATURES$featureKey4 === void 0 ? [] : _FEATURES$featureKey4;
    ...
    if (status === _SiteMapUtils.FETCH_STATUS.SUCCESS && data) {
      ...
      if (featureType === _SiteMapUtils.FEATURE_TYPES.SAMPLING_POINTS && parentDataFeatureKey) {
        var parentDataFeatureType = _SiteMapUtils.FEATURES[parentDataFeatureKey].type;
        var locationParent = data.locationParent;
        var parentData = newState.featureData[parentDataFeatureType][parentDataFeatureKey];
        var coordIdx = matchLocationCoordinateMap.findIndex(function (match) {
          return location.endsWith(match);
        });
        ...
        if (!parentData[siteCode]) {
          parentData[siteCode] = {};
        }

        if (!parentData[siteCode][locationParent]) {
          parentData[siteCode][locationParent] = {};
        ...
        if (!parentData[siteCode][locationParent].geometry) {
          parentData[siteCode][locationParent].geometry = {
            coordinates: []
          };
        ...
        parentData[siteCode][locationParent].geometry.coordinates[coordIdx] = [parsedData.latitude, parsedData.longitude];
        return true;
