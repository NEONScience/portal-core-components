...
var reducer = function reducer(state, action) {
  ...
  switch (action.type) {
    ...
    case 'setDomainLocationHierarchyFetchSucceeded':
      ...
      if ((0, _SiteMapUtils.mapIsAtFocusLocation)(state) && state.focusLocation.data && state.focusLocation.data.siteCode) {
    ...
    case 'toggleItemSelected':
      ...
      if (newState.selection.limit === 1) {
        if (newState.selection.set.has(action.item) && newState.selection.set.size > 1) {
          ...
          newState.selection.changed = true;
        } else if (!newState.selection.set.has(action.item) && isSelectable(action.item, validSet)) {
