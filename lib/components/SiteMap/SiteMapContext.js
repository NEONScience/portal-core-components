...
import { fetchManyLocationsGraphQL, fetchSingleLocationREST, fetchDomainHierarchy } from './FetchLocationUtils';
import { SORT_DIRECTIONS, BASE_LAYERS, OVERLAYS, VIEWS, FEATURES, FETCH_STATUS, FEATURE_TYPES, FEATURE_DATA_SOURCES, SELECTION_PORTIONS, SITE_LOCATION_HIERARCHIES_MIN_ZOOM, MAP_ZOOM_RANGE, MAP_MOUSE_MODES, OBSERVATORY_CENTER, PLOT_SAMPLING_MODULES, SITE_MAP_PROP_TYPES, SITE_MAP_DEFAULT_PROPS, MIN_TABLE_MAX_BODY_HEIGHT, MANUAL_LOCATION_TYPES, GRAPHQL_LOCATIONS_API_CONSTANTS, MAP_STATE_STATUS_TYPE, getDefaultState, getZoomedIcons, mapIsAtFocusLocation, getMapStateForFocusLocation, hydrateNeonContextData, calculateFeatureAvailability, boundsAreValid, calculateLocationsInBounds, zoomIsValid, centerIsValid, deriveFullObservatoryZoomLevel, determineMapStatus } from './SiteMapUtils';
...
const deriveRegionSelections = (state)=>{
    const derive = (featureKey)=>{
        ...
        Object.keys(state.featureData[featureType][featureKey]).forEach((boundaryCode)=>{
            ...
            const selectableSites = !validSet ? boundarySitesSet : new Set([
                ...boundarySitesSet
            ].filter((siteCode)=>validSet.has(siteCode)));
            const intersection = [
                ...selectableSites
            ...
            selectedBoundaries[boundaryCode] = intersection.length === selectableSites.size ? SELECTION_PORTIONS.TOTAL : SELECTION_PORTIONS.PARTIAL;
        });
...
const getSelectableSet = (setArg, validSet = null)=>{
    ...
    return validSet instanceof Set ? new Set([
        ...set
    ].filter((item)=>isSelectable(item, validSet))) : set;
};
...
const validateSelection = (state)=>{
    ...
    if (set.size > 0 && (!validSet || [
        ...set
    ].every((item)=>validSet.has(item)))) {
        valid = true;
...
const calculateFeatureDataFetches = (state, requiredSites = [])=>{
    ...
    const fetchablePrototypeSites = (state.manualLocationData || []).filter((manualLocation)=>manualLocation.manualLocationType === MANUAL_LOCATION_TYPES.PROTOTYPE_SITE && state.sites[manualLocation.siteCode]);
    ...
    if (requiredSites && requiredSites.length) {
        requiredSitesArray = (Array.isArray(requiredSites) ? requiredSites : [
            requiredSites
    ...
    Object.keys(FEATURES).filter((featureKey)=>FEATURES[featureKey].dataSource === FEATURE_DATA_SOURCES.ARCGIS_ASSETS_API && state.filters.features.available[featureKey] && state.filters.features.visible[featureKey]).forEach((featureKey)=>{
    ...
    Object.keys(FEATURES)// Only look at available+visible features that get fetched and have a location type match
    .filter((featureKey)=>FEATURES[featureKey].dataSource === FEATURE_DATA_SOURCES.REST_LOCATIONS_API && FEATURES[featureKey].matchLocationType && state.filters.features.available[featureKey] && state.filters.features.visible[featureKey]).forEach((featureKey)=>{
    ...
    /* eslint-disable max-len */ // Start by looping through all minZoom levels associated with any GraphQL Locations API features
    ...
    Object.keys(GRAPHQL_LOCATIONS_API_CONSTANTS.MINZOOM_TO_FEATURES_MAP).filter((minZoom)=>state.map.zoom >= minZoom).forEach((minZoom)=>{
        ...
        GRAPHQL_LOCATIONS_API_CONSTANTS.MINZOOM_TO_FEATURES_MAP[minZoom].forEach((featureKey)=>{
            ...
            const companionMinZoom = companionFeatureKey ? GRAPHQL_LOCATIONS_API_CONSTANTS.FEATURES_TO_MINZOOM_MAP[companionFeatureKey] : null;
            ...
            .filter((siteCode)=>state.featureData.SITE_LOCATION_HIERARCHIES[siteCode])// Site must have meaningful features to fetch at this minZoom level
            .filter((siteCode)=>state.sites[siteCode].terrain === FEATURES[featureKey].siteTerrain).forEach((siteCode)=>{
                const getInitialSiteCodeFetchObject = (fetchMinZoom)=>({
                        features: Object.fromEntries(GRAPHQL_LOCATIONS_API_CONSTANTS.MINZOOM_TO_FEATURES_MAP[fetchMinZoom].filter((fKey)=>state.sites[siteCode].terrain === FEATURES[fKey].siteTerrain).map((fKey)=>[
                                fKey,
                                {
                                    fetchId: null,
                                    locations: []
                ...
                if (isBasePlot(featureKey) && companionFeatureKey && companionMinZoom) {
                    if (!newState.featureDataFetches[dataSource][companionMinZoom][siteCode]) {
                        ...
                        newState.featureDataFetches[dataSource][minZoom][siteCode].features[featureKey] = {
                            ...companionFeatureObject
                        };
                    }
                }
                ...
                let awaitingFetchKey = Object.keys(fetches).find((fetchKey)=>fetches[fetchKey].status === FETCH_STATUS.AWAITING_CALL);
                if (!awaitingFetchKey) {
    ...
    /* eslint-enable max-len */ return newState;
};
...
const updateMapTileWithZoom = (state)=>{
    ...
    if (newState.map.zoom <= 17 && state.map.baseLayer !== BASE_LAYERS.NATGEO_WORLD_MAP.KEY && state.map.baseLayerAutoChangedAbove17) {
...
const applyFeatureVisibilityToChildren = (state, feature, visible)=>{
    ...
    if (FEATURES[feature].type === FEATURE_TYPES.GROUP.KEY) {
        Object.keys(FEATURES).filter((f)=>FEATURES[f].parent === feature).forEach((f)=>{
...
const applyFeatureVisibilityToParents = (state, feature)=>{
    ...
    if (FEATURES[feature].parent) {
        const parentVisible = Object.keys(FEATURES).filter((f)=>FEATURES[f].parent === FEATURES[feature].parent) // Of all children...
...
const setFetchStatusFromAction = (state, action, status)=>{
    ...
    if (!FEATURE_DATA_SOURCES[dataSource]) {
        return state;
    }
    const newState = {
        ...state
    ...
    if (dataSource === FEATURE_DATA_SOURCES.REST_LOCATIONS_API) {
        const { featureKey, siteCode, location, data } = action;
        if (!FEATURES[featureKey]) {
            return newState;
        }
        const { type: featureType } = FEATURES[featureKey];
    ...
    if (dataSource === FEATURE_DATA_SOURCES.GRAPHQL_LOCATIONS_API) {
        const { minZoom, siteCode, fetchId, data } = action;
        ...
        if (status === FETCH_STATUS.SUCCESS && data) {
            ...
            Object.keys(data).forEach((locName)=>{
                ...
                const { type: featureType, parentDataFeatureKey, matchLocationCoordinateMap } = FEATURES[featureKey];
                ...
                if (parentDataFeatureKey) {
                    ...
                    parentSiteData[parentLocName].geometry.coordinates[coordIdx] = [
                        latitude,
                        longitude
                    ];
                ...
                const geometry = !newState.featureData[featureType][featureKey][siteCode][locName] || !newState.featureData[featureType][featureKey][siteCode][locName].geometry ? null : {
                ...
                if (isBasePlot(featureKey)) {
                    ...
                    newState.featureData[featureType][featureKey][siteCode][locName].samplingModules = Object.keys(hierarchy).reduce((acc, cur)=>{
                        ...
                        return acc;
                    }, []).filter((k)=>k !== 'all' && k !== 'mfb' && PLOT_SAMPLING_MODULES[k]).sort((a, b)=>PLOT_SAMPLING_MODULES[a] > PLOT_SAMPLING_MODULES[b] ? 1 : -1);
                }
            });
...
*/ const reducer = (state, action)=>{
    ...
    switch(action.type){
        case 'setView':
            if (!Object.keys(VIEWS).includes(action.view)) {
                return state;
            }
            newState.view.current = action.view;
            return newState;
        case 'setViewInitialized':
            if (!Object.keys(VIEWS).includes(state.view.current)) {
                return state;
            }
        ...
        case 'setMapZoom':
            ...
            if (boundsAreValid(action.bounds)) {
        ...
        case 'setMapBounds':
            ...
            newState.map.bounds = action.bounds;
            newState.map.status = determineMapStatus(newState);
            if (currentMapBoundsMapStatus === MAP_STATE_STATUS_TYPE.INIT && newState.map.status === MAP_STATE_STATUS_TYPE.READY) {
                newState = calculateZoomState(newState.map.zoom, newState);
            } else {
        ...
        case 'setMapCenter':
            ...
            newState.map.status = determineMapStatus(newState);
            if (currentMapCenterMapStatus === MAP_STATE_STATUS_TYPE.INIT && newState.map.status === MAP_STATE_STATUS_TYPE.READY) {
                newState = calculateZoomState(newState.map.zoom, newState);
            } else {
                newState = calculateFeatureDataFetches(newState);
            }
        ...
        case 'setMapMouseMode':
            if (!Object.keys(MAP_MOUSE_MODES).includes(action.mouseMode)) {
                return state;
            }
        ...
        case 'setLegendFeatureOptionVisibility':
            if (!Object.keys(FEATURES).includes(action.feature) || typeof action.visible !== 'boolean') {
                return state;
            }
        ...
        case 'setLegendFeatureOptionCollapsed':
            if (!Object.keys(FEATURES).includes(action.feature)) {
                return state;
            }
        ...
        case 'setLegendOverlayOptionCollapsed':
            if (!Object.keys(OVERLAYS).includes(action.overlay)) {
                return state;
            }
            newState.filters.overlays.expanded.delete(action.overlay);
            return newState;
        ...
        case 'setNewFocusLocation':
            ...
            if (newState.view.current === VIEWS.TABLE) {
                newState.view.current = VIEWS.MAP;
            }
            return newState;
        ...
        case 'setFocusLocationFetchSucceeded':
            ...
            newState.focusLocation.map.center = [
                ...newState.map.center
            ];
            newState = updateMapTileWithZoom(newState);
            newState = calculateFeatureAvailability(newState);
            newState = calculateFeatureDataFetches(newState, newState.focusLocation.data && newState.focusLocation.data.siteCode ? [
        ...
        case 'setDomainLocationHierarchyFetchStarted':
            /* eslint-disable max-len */ if (!newState.featureDataFetches[hierarchiesSource][hierarchiesType][action.domainCode]) {
                return state;
            }
        ...
        case 'setDomainLocationHierarchyFetchSucceeded':
            ...
            /* eslint-disable max-len */ newState.featureDataFetches[hierarchiesSource][hierarchiesType][action.domainCode] = FETCH_STATUS.SUCCESS;
            Object.keys(action.data).forEach((siteCode)=>{
                newState.featureData.SITE_LOCATION_HIERARCHIES[siteCode] = action.data[siteCode];
            });
            /* eslint-enable max-len */ newState.overallFetch.pendingHierarchy -= 1;
            newState = completeOverallFetch(newState);
            newState = calculateFeatureDataFetches(newState, mapIsAtFocusLocation(state) && newState.focusLocation.data && newState.focusLocation.data.siteCode ? [
                newState.focusLocation.data.siteCode
            ] : []);
            return newState;
        ...
        case 'toggleSitesSelectedForState':
            ...
            setMethod = state.selection.derived[FEATURES.STATES.KEY][action.stateCode] === SELECTION_PORTIONS.TOTAL ? 'delete' : 'add';
            getSelectableSet(newState.featureData[FEATURE_TYPES.STATES.KEY][FEATURES.STATES.KEY][action.stateCode].sites, validSet).forEach((siteCode)=>{
        ...
        case 'toggleSitesSelectedForDomain':
            ...
            /* eslint-disable max-len */ setMethod = state.selection.derived[FEATURES.DOMAINS.KEY][action.domainCode] === SELECTION_PORTIONS.TOTAL ? 'delete' : 'add';
            getSelectableSet(newState.featureData[FEATURE_TYPES.DOMAINS.KEY][FEATURES.DOMAINS.KEY][action.domainCode].sites, validSet).forEach((siteCode)=>{
                newState.selection.set[setMethod](siteCode);
            });
            /* eslint-enable max-len */ newState.selection.changed = true;
            newState = validateSelection(newState);
            newState = deriveRegionSelections(newState);
            return newState;
        // Default
...
/** Context and Hook */ const Context = /*#__PURE__*/ createContext(getDefaultState());
...
/** Context Provider */ const Provider = (inProps)=>{
    const props = resolveProps(SITE_MAP_DEFAULT_PROPS, inProps);
    const { view, aspectRatio, fullscreen, mapUniqueId, mapZoom, mapCenter, mapBaseLayer, location: locationProp, selection, validItems, selectedItems, selectionLimit, onSelectionChange, tableFullHeight, manualLocationData, children } = props;
    // Neon Context State
  ...
  */ const initialMapZoom = mapZoom === null ? null : Math.max(Math.min(mapZoom, MAP_ZOOM_RANGE[1]), MAP_ZOOM_RANGE[0]);
    let initialState = getDefaultState();
    initialState.view.current = Object.keys(VIEWS).includes(view.toUpperCase()) ? view.toUpperCase() : VIEWS.MAP;
    ...
    const canFetchFeatureData = state.neonContextHydrated && !(state.focusLocation.current && state.focusLocation.fetch.status !== FETCH_STATUS.SUCCESS);
    /**
  ...
  */ useEffect(()=>{
        ...
        Object.keys(state.featureDataFetches[hierarchiesSource][hierarchiesType]).forEach((domainCode)=>{
            ...
            fetchDomainHierarchy(domainCode).then((response)=>{
                dispatch({
        ...
        Object.keys(state.featureDataFetches[arcgisSource]).forEach((featureKey)=>{
            Object.keys(state.featureDataFetches[arcgisSource][featureKey]).forEach((siteCode)=>{
                ...
                if (status !== FETCH_STATUS.AWAITING_CALL) {
                    return;
                }
                dispatch({
                ...
                NeonApi.getArcgisAssetObservable(featureKey, siteCode).pipe(map((response)=>{
                    if (response) {
                        dispatch({
        ...
        Object.keys(restLocFetches).filter((featureKey)=>featureKey !== FEATURE_TYPES.SITE_LOCATION_HIERARCHIES.KEY).forEach((featureKey)=>{
            Object.keys(state.featureDataFetches[restLocSource][featureKey]).forEach((siteCode)=>{
                ...
                Object.keys(featureSite).forEach((location)=>{
                    if (featureSite[location] !== FETCH_STATUS.AWAITING_CALL) {
                        return;
                    }
                    dispatch({
                        type: 'setFeatureDataFetchStarted',
                    ...
                    fetchSingleLocationREST(location).then((data)=>{
                        dispatch({
                            type: 'setFeatureDataFetchSucceeded',
                            dataSource: restLocSource,
                            data,
        ...
        Object.keys(gqlLocFetches).forEach((minZoom)=>{
            Object.keys(gqlLocFetches[minZoom]).forEach((siteCode)=>{
                Object.keys(gqlLocFetches[minZoom][siteCode].fetches).filter((fetchId)=>gqlLocFetches[minZoom][siteCode].fetches[fetchId].status === FETCH_STATUS.AWAITING_CALL).forEach((fetchId)=>{
                    dispatch({
                        type: 'setFeatureDataFetchStarted',
                    ...
                    fetchManyLocationsGraphQL(gqlLocFetches[minZoom][siteCode].fetches[fetchId].locations).then((response)=>{
                        dispatch({
                            type: 'setFeatureDataFetchSucceeded',
                            dataSource: gqlLocSource,
                            minZoom,
        ...
        canFetchFeatureData,
        state.featureDataFetchesHasAwaiting,
        state.featureDataFetches
    ]);
    /**
