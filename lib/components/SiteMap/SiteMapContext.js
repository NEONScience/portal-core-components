...
var setFetchStatusFromAction = function setFetchStatusFromAction(state, action, status) {
  ...
  var newState = _extends({}, state); // ARCGIS_ASSETS_API
...
var reducer = function reducer(state, action) {
  ...
  var validSet = state.selection.validSet; // Shortcuts for deailing with hierarchies
  ...
  switch (action.type) {
    ...
    case 'setNewFocusLocation':
      ...
      newState.overallFetch.expected += 1; // Switch view to MAP if we're on TABLE (and not on SPLIT)
...
var Provider = function Provider(props) {
  ...
  */
  ...
  if (Object.keys(_SiteMapUtils.FEATURE_TYPES).filter(function (k) {
    ...
    initialState = validateSelection(initialState); // Automatically set feature visibility, table view, etc. based on selection type
