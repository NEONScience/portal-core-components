...
/* eslint-disable max-len, @stylistic/max-len */ // Creates fetch objects with an AWAITING_CALL status based on current state.
...
/* eslint-enable max-len, @stylistic/max-len */ // NATGEO_WORLD_MAP has no data at zoom 17 or higher so go to WORLD_IMAGERY (satellite)
    ...
    if (dataSource === FEATURE_DATA_SOURCES.GRAPHQL_LOCATIONS_API) {
        ...
        if (status === FETCH_STATUS.SUCCESS && data) {
            ...
            Object.keys(data).forEach((locName)=>{
                ...
                let geometry;
                if (!newState.featureData[featureType][featureKey][siteCode][locName] || !newState.featureData[featureType][featureKey][siteCode][locName].geometry) {
                    geometry = null;
                } else {
                    geometry = {
...
*/ const reducer = (state, action)=>{
    ...
    switch(action.type){
        ...
        case 'toggleSitesSelectedForDomain':
            ...
            /* eslint-disable max-len, @stylistic/max-len */ setMethod = state.selection.derived[FEATURES.DOMAINS.KEY][action.domainCode] === SELECTION_PORTIONS.TOTAL ? 'delete' : 'add';
            ...
            /* eslint-enable max-len, @stylistic/max-len */ newState.selection.changed = true;
            newState = validateSelection(newState);
            newState = deriveRegionSelections(newState);
            return newState;
        // Default
