...
var SiteMapLeaflet = function SiteMapLeaflet() {
  ...
  */
  ...
  (0, _react.useEffect)(function () {
    ...
    mapRef.current.container.onmouseup = function (event) {
      ...
      var selectionSites = (0, _SiteMapUtils.calculateLocationsInBounds)(state.sites, selectionBounds);
      var newSelectionSet = new Set([].concat(_toConsumableArray(selectionSites), _toConsumableArray(shiftPressed ? Array.from(state.selection.set) : [])));
      areaSelectionDispatch({
        type: 'end',
        x: event.offsetX,
        y: event.offsetY
  ...
  */
  ...
  var renderMouseModeToggleButtonGroup = function renderMouseModeToggleButtonGroup() {
    ...
    var units = state.selection.active === _SiteMapUtils.FEATURE_TYPES.SITES ? 'sites' : 'locations';
