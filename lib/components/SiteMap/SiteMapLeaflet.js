...
/* eslint-disable no-underscore-dangle */ import React, { useId, useRef, useEffect, useReducer, useCallback, useLayoutEffect } from 'react';
import ReactDOMServer from 'react-dom/server';
import L from 'leaflet';
import { LayerGroup, MapContainer, ScaleControl, TileLayer, WMSTileLayer, useMap, useMapEvents } from 'react-leaflet';
...
import { VIEWS, BASE_LAYERS, OBSERVATORY_CENTER, OVERLAYS, OVERLAY_GROUPS, MAP_ZOOM_RANGE, FEATURES, FEATURE_TYPES, FETCH_STATUS, MAP_MOUSE_MODES, UNSELECTABLE_MARKER_FILTER, LEAFLET_ATTR_PREFIX, MAP_STATE_STATUS_TYPE, mapIsAtFocusLocation, calculateLocationsInBounds, deriveFullObservatoryZoomLevel } from './SiteMapUtils';
 ...
 * @param {*} mouseMode
 * @returns
 */ const getMouseModeProps = (mouseMode)=>mouseMode === MAP_MOUSE_MODES.PAN ? {
        boxZoom: true,
        dragging: true
    } : {
        boxZoom: false,
...
const handleOnMouseUpAreaSelection = (event, areaSelectionDispatch, dispatch, areaSelection, state, map)=>{
    ...
    ].includes(state.selection.active)) {
        const selectableFeatureKey = Object.keys(FEATURES).find((k)=>FEATURES[k].type === state.selection.active);
...
const LeafletAreaSelectionManager = ()=>{
    ...
    const map = useMap();
    const [areaSelection, areaSelectionDispatch] = useReducer(areaSelectionReducer, areaSelectionDefaultState);
    const { isDragging: areaSelectionDragging } = areaSelection;
    /**
     Effect
     Area Selection - event listeners to handle area selection mouse events
  */ useEffect(()=>{
        if (!map || !map.getContainer() || mapMouseMode !== MAP_MOUSE_MODES.AREA_SELECT) {
            return ()=>{};
        }
    ...
    useEffect(()=>{
        ...
        map,
        areaSelectionDragging
    ]);
    /**
     Render: AreaSelection
  */ const renderAreaSelection = ()=>{
        const { isDragging, center, reach } = areaSelection;
        if (state.map.mouseMode !== MAP_MOUSE_MODES.AREA_SELECT || !isDragging || !Number.isInteger(center.x) || !Number.isInteger(center.y) || !Number.isInteger(reach.x) || !Number.isInteger(reach.y)) {
            return null;
        }
        const style = {
...
const LeafletPanesManager = ()=>{
    const [state, dispatch] = SiteMapContext.useSiteMapContext();
    const { map: { mouseMode }, filters: { features: { visible: visibleFeatures } } } = state;
    const map = useMap();
    /**
    Effect
  ...
  */ useLayoutEffect(()=>{
        ...
        window.setTimeout(()=>{
            ...
            paths.filter((path)=>path.attributes.class && path.attributes.class.value.includes('#mask')).forEach((path)=>{
                ...
                path.setAttributeNS(null, 'style', mouseMode === MAP_MOUSE_MODES.AREA_SELECT ? 'cursor: crosshair; pointer-events: none' : '');
            });
            if (defCount === 0) {
                return;
            }
  ...
  */ useLayoutEffect(()=>{
        const timeout = window.setTimeout(()=>{
            if (state.view.current !== VIEWS.MAP || !state.selection.active || !state.selection.validSet || state.selection.active !== FEATURE_TYPES.SITES.KEY) {
                return;
            }
            const markerPane = map.getPane('markerPane');
            if (markerPane && markerPane.children && markerPane.children.length) {
                ...
                [
                    ...markerPane.children
                ].filter((marker)=>!state.selection.validSet.has(marker.title)).forEach((marker)=>{
                    // eslint-disable-next-line no-param-reassign
...
const LeafletMapManager = ()=>{
    const [state, dispatch] = SiteMapContext.useSiteMapContext();
    const { aspectRatio: { currentValue: aspectRatioValue }, map: { mouseMode, center, zoom: mapZoom, bounds: mapBounds } } = state;
            ...
            const debouncedSetCenter = debounce(()=>{
                if (isCenterUpdated()) {
                    dispatch({
                        ...
                        bounds: {
                            lat: [
                                targetBounds._southWest.lat,
                                targetBounds._northEast.lat
                            ],
                            lng: [
                                targetBounds._southWest.lng,
                                targetBounds._northEast.lng
                            ]
                        }
                    });
                }
  ...
  */ useEffect(()=>{
        ...
        dispatch({
            ...
            bounds: {
                lat: [
                    nextMapBounds._southWest.lat,
                    nextMapBounds._northEast.lat
                ],
                lng: [
                    nextMapBounds._southWest.lng,
                    nextMapBounds._northEast.lng
                ]
            }
        });
    }, [
  ...
  */ useEffect(()=>{
    ...
    useEffect(()=>{
        ...
        // Disabling react hooks linter here as this is modifying the leaflet
        // map instance directly which is outside the context of React
...
*/ const SiteMapLeaflet = ()=>{
    ...
    if (state.focusLocation.current && state.focusLocation.fetch.status !== FETCH_STATUS.SUCCESS) {
        canRender = false;
    }
    /**
     Callback
  ...
  */ useEffect(()=>{
        if (!mapRefExists() || state.view.current !== VIEWS.MAP) {
            return;
        }
        mapRef.current.invalidateSize();
  ...
  */ const renderShowFullObservatoryButton = ()=>/*#__PURE__*/ _jsx(Tooltip, {
            placement: "right",
            ...
            children: /*#__PURE__*/ _jsx("div", {
                ...
                children: /*#__PURE__*/ _jsx(IconButton, {
                    ...
                    children: /*#__PURE__*/ _jsx(ObservatoryIcon, {
                        fontSize: "small"
                    })
                })
            })
    ...
    /**
     Render - Zoom to Observatory Button
  */ const renderReturnToFocusLocationButton = ()=>!state.focusLocation.current ? null : /*#__PURE__*/ _jsx(Tooltip, {
            placement: "right",
            ...
            children: /*#__PURE__*/ _jsx("div", {
                ...
                children: /*#__PURE__*/ _jsx(IconButton, {
                    ...
                    children: /*#__PURE__*/ _jsx(FocusLocationIcon, {
                        fontSize: "small"
                    })
                })
            })
   ...
   */ const renderMouseModeToggleButtonGroup = ()=>{
        ...
        const panTooltip = 'Click and drag on map to move the map center; ' + 'shift+drag to zoom to an area';
        const areaSelectTooltip = `Click and drag on map to select ${units} ` + 'in an area; shift+drag to add onto selection';
        ...
        return /*#__PURE__*/ _jsx(ToggleButtonGroup, {
            ...
            children: Object.keys(MAP_MOUSE_MODES).map((key)=>{
  ...
  */ const renderBaseLayer = ()=>{
        ...
        if (!state.map.baseLayer) {
            return /*#__PURE__*/ _jsx(TileLayer, {
                url: "",
                attribution: "",
                "data-id": id
            });
        ...
        const attributionNode = /*#__PURE__*/ _jsx("div", {
  ...
  */ const renderOverlay = (key)=>{
        ...
        return /*#__PURE__*/ _jsx(LayerGroup, {
    ...
    const renderGroupedLayerControl = ()=>{
        ...
        const groupedLayerControlOverlays = Object.keys(OVERLAYS).map((key)=>{
            ...
            let groupTitle = null;
            if (group && OVERLAY_GROUPS[group]) {
                groupTitle = OVERLAY_GROUPS[group].title;
            }
            return {
                name,
                title,
        ...
        return /*#__PURE__*/ _jsx(LeafletGroupedLayerControl, {
            position: "topleft",
  ...
  */ const renderProgress = ()=>{
    ...
    const renderFeatures = ()=>{
        ...
        return Object.keys(FEATURES).filter((key)=>state.filters.features.available[key]).filter((key)=>state.filters.features.visible[key]).map((key)=>/*#__PURE__*/ _jsx(SiteMapFeature, {
                featureKey: key
            }, key));
    };
    /**
  ...
  */ const mouseModeProps = getMouseModeProps(state.map.mouseMode);
    ...
    const appliedCenter = exists(state.map.center) ? state.map.center : state.map.initialCenter;
    const appliedZoom = exists(state.map.zoom) ? state.map.zoom : state.map.initialZoom;
    return /*#__PURE__*/ _jsxs(_Fragment, {
        children: [
            /*#__PURE__*/ _jsxs(MapContainer, {
                ...
                ...mouseModeProps,
                children: [
                    /*#__PURE__*/ _jsx(LeafletMapManager, {}),
                    /*#__PURE__*/ _jsx(LeafletPanesManager, {}),
                    /*#__PURE__*/ _jsx(LeafletAreaSelectionManager, {}),
                    /*#__PURE__*/ _jsx(ScaleControl, {
                        imperial: true,
                        metric: true,
                        updateWhenIdle: true
                    }),
