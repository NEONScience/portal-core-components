...
/**
   Main Component
*/
var States = function States(props) {
  ...
  var featureData = state.featureData[_SiteMapUtils.FEATURE_TYPES.BOUNDARIES][_SiteMapUtils.FEATURES.STATES.KEY];
  var _state$selection = state.selection,
      selectedStates = _state$selection.derived[_SiteMapUtils.FEATURES.STATES.KEY],
      selectionActive = _state$selection.active;
  /**
     Render Method: Popup
  */

  var renderPopup = function renderPopup(stateCode) {
    if (!featureData[stateCode]) {
      return null;
    }

    var usState = featureData[stateCode];

    var renderActions = function renderActions() {
      var count = usState.sites.size;

      if (!selectionActive || !count) {
        return null;
      }

      var isTotalSelected = selectedStates[stateCode] === 'total';
    ...
    return /*#__PURE__*/_react.default.createElement(_reactLeaflet.Popup, {
      ...
      variant: "h6",
      gutterBottom: true
    }, "".concat(usState.name, " (").concat(stateCode, ")")), renderPopupSitesList(usState.sites), renderActions());
  };
  /**
     Main Render
  */
  ...
  var sortedStates = Object.keys(featureData).sort(function (a) {
    return a === state.focusLocation.current ? 1 : -1;
  });
  return /*#__PURE__*/_react.default.createElement(_reactLeaflet.FeatureGroup, null, sortedStates.map(function (stateCode) {
    var usState = featureData[stateCode];
    var baseColor = state.focusLocation.current === stateCode ? "#".concat((0, _tinycolor.default)(_SiteMapUtils.FEATURES.STATES.style.color).darken(20).toHex()) : _SiteMapUtils.FEATURES.STATES.style.color;
    var overlayColor = selectionActive && selectedStates[stateCode] ? _SiteMapUtils.BOUNDARY_COLORS["".concat(selectedStates[stateCode], "Selected")] : baseColor;
    var interactionProps = selectionActive ? {
      ...
      onClick: function onClick() {
        if (usState.sites.size) {
          // Not all states has NEON sites
          dispatch({
    ...
    }, interactionProps), renderPopup(stateCode));
  }));
};
