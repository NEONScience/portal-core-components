...
/* eslint-disable jsx-a11y/anchor-is-valid */ import React, { useRef, useEffect, useLayoutEffect } from 'react';
...
import { getHref, VIEWS, FEATURES, NLCD_CLASSES, FEATURE_TYPES, MANUAL_LOCATION_TYPES, MIN_TABLE_MAX_BODY_HEIGHT, PLOT_SAMPLING_MODULES, UNSELECTABLE_MARKER_FILTER, calculateLocationsInBounds } from './SiteMapUtils';
...
const calculateMaxBodyHeight = (tableRef)=>{
    if (!tableRef || !tableRef.current) {
        return MIN_TABLE_MAX_BODY_HEIGHT;
    }
...
const SiteMapTable = ()=>{
    ...
    const { manualLocationData, view: { current: view, initialized: viewsInitialized } } = state;
    const { focus, fullHeight, maxBodyHeight, maxBodyHeightUpdateFromAspectRatio } = state.table;
    const { set: selection, validSet: selectableItems, hideUnselectable } = state.selection;
    const selectionActive = state.selection.active === focus;
    /**
    Effect - Initialize table if this is the first time we're seeing it
  */ useEffect(()=>{
        if (!tableRef || !tableRef.current || view !== VIEWS.TABLE || viewsInitialized[VIEWS.TABLE]) {
            return;
        }
        dispatch({
  ...
  */ useEffect(()=>{
        if (view === VIEWS.TABLE && viewsInitialized[VIEWS.TABLE] && maxBodyHeightUpdateFromAspectRatio) {
            dispatch({
                type: 'setTableMaxBodyHeight',
                height: calculateMaxBodyHeight(tableRef)
            });
  ...
  */ useLayoutEffect(()=>{
    ...
    const renderFeatureIcon = (featureKey, unselectable = false)=>{
        if (!FEATURES[featureKey] || !FEATURES[featureKey].iconSvg) {
            return null;
        }
        const { iconSvg } = FEATURES[featureKey];
        const style = unselectable ? {
            filter: UNSELECTABLE_MARKER_FILTER
        } : {};
    ...
    const renderCaptionString = (str = '--', ariaLabel = null)=>/*#__PURE__*/ _jsx(Typography, {
            variant: "caption",
  ...
  */ const sitesInMap = Array.from(rows.reduce((acc, cur)=>{
    ...
    const commonColumns = {
        site: {
            ...
            defaultSort: 'desc',
            searchable: true,
            lookup: Object.fromEntries(Array.from(sitesInMap).sort().map((siteCode)=>[
                    siteCode,
                    siteCode
                ])),
            customFilterAndSearch: (input, row)=>{
                if (typeof input === 'string' && input.length) {
                    return searchOnAttribs(input, [
                        row.siteCode,
                        row.description
                    ]);
                }
            ...
            render: (row)=>{
                ...
                const featureKey = isDecommissioned ? FEATURES.DECOMMISSIONED_SITES.KEY : `${site.terrain.toUpperCase()}_${site.type.toUpperCase()}_SITES`;
                    ...
                    children: [
                        /*#__PURE__*/ _jsxs(Link, {
                            ...
                            children: [
                                ...
                                /*#__PURE__*/ _jsx("span", {
                                    children: `${description || 'Unnamed Site'} (${siteCode})`
                                })
                            ]
                        }),
                        /*#__PURE__*/ _jsxs("div", {
        ...
        domain: {
            ...
            defaultSort: 'desc',
            searchable: true,
            lookup: Object.fromEntries(Array.from(domainsInMap).sort().map((domainCode)=>[
                    domainCode,
                    domainCode
                ])),
            customSort: (rowA, rowB)=>{
        ...
        state: {
            ...
            defaultSort: 'desc',
            searchable: true,
            lookup: Object.fromEntries(Array.from(statesInMap).sort().map((stateCode)=>[
                    stateCode,
            ...
            render: (row)=>{
                ...
                const { stateCode } = row;
                return !usstate ? null : /*#__PURE__*/ _jsx("div", {
                    children: /*#__PURE__*/ _jsx(Link, {
                        component: "button",
                        className: classes.linkButton,
  ...
  */ let columns = [];
    if (focus === FEATURE_TYPES.SITES.KEY) {
        columns = [
            commonColumns.site,
            {
                ...
                defaultSort: 'desc',
                searchable: true,
                lookup: Object.fromEntries(Array.from(new Set(rows.map((row)=>row.type))).sort().map((k)=>[
                        k,
                        ucWord(k)
                    ])),
            ...
            },
            {
                ...
                sorting: true,
                defaultSort: 'desc',
                lookup: Object.fromEntries(Array.from(new Set(rows.map((row)=>row.terrain))).sort().map((k)=>[
                        k,
                        ucWord(k)
                    ])),
    ...
    if (focus === FEATURE_TYPES.LOCATIONS.KEY) {
        columns = [
            ...
            },
            {
                ...
                defaultSort: 'desc',
                searchable: true,
                lookup: Object.fromEntries(Array.from(new Set(rows.map((row)=>row.featureKey))).sort((a, b)=>getFeatureName(a) > getFeatureName(b) ? 1 : -1).map((featureKey)=>[
                        featureKey,
            ...
            },
            {
                ...
                defaultSort: 'desc',
                filtering: false,
                render: (row)=>renderCaptionString(Number.isFinite(row.elevation) ? `${row.elevation.toFixed(2)}m` : '--', 'Elevation'),
            ...
            },
            {
                ...
                deafultSort: 'asc',
                searchable: true,
                lookup: Object.fromEntries(Object.keys(NLCD_CLASSES).filter((classKey)=>rows.some((row)=>row.nlcdClass === classKey)).sort().map((classKey)=>[
                        classKey,
                        NLCD_CLASSES[classKey].name
                    ])),
                render: (row)=>{
                    const { nlcdClass } = row;
                    if (!nlcdClass) {
                ...
                render: (row)=>{
                    ...
                    return /*#__PURE__*/ _jsxs(_Fragment, {
                        children: [
                            ...
                            Number.isFinite(plotSize) ? /*#__PURE__*/ _jsxs(_Fragment, {
                                children: [
                                    /*#__PURE__*/ _jsx("br", {}),
            ...
            },
            {
                ...
                deafultSort: 'asc',
                filtering: false,
                render: (row)=>renderCaptionString(Number.isFinite(row.slopeAspect) ? `${row.slopeAspect.toFixed(2)}\u00b0` : '--', 'Slope Aspect'),
            ...
            },
            {
                ...
                deafultSort: 'asc',
                filtering: false,
                render: (row)=>renderCaptionString(Number.isFinite(row.slopeGradient) ? `${row.slopeGradient.toFixed(2)}%` : '--', 'Slope Gradient'),
            ...
            },
            {
                ...
                render: (row)=>{
                    ...
                    return /*#__PURE__*/ _jsx(Tooltip, {
                        placement: "left",
                        title: samplingModules.length ? /*#__PURE__*/ _jsx("ul", {
                            style: {
                                marginLeft: Theme.spacing(-1)
                            },
                            children: samplingModules.map((m)=>/*#__PURE__*/ _jsx("li", {
                                    children: PLOT_SAMPLING_MODULES[m]
                                }, m))
                        }) : /*#__PURE__*/ _jsx("i", {
                            children: "none"
                        }),
    ...
    const toolbarClassName = view === VIEWS.SPLIT ? classes.toolbarContainer : `${classes.toolbarContainer} ${classes.toolbarContainerNoSplit}`;
    const components = {
        Container: Box,
        // eslint-disable-next-line react/no-unstable-nested-components
        Toolbar: (toolbarProps)=>/*#__PURE__*/ _jsx("div", {
  ...
  */ const tableOptions = {
        ...
        rowStyle: (row)=>{
            if (selectionActive) {
                ...
                if (rowIsSelected(row)) {
                    return {
                        backgroundColor: COLORS.LIGHT_BLUE[50]
                    };
                }
            }
            return {};
    ...
    return /*#__PURE__*/ _jsx("div", {
        ...
        children: /*#__PURE__*/ _jsx(LegacyNeonThemeProvider, {
