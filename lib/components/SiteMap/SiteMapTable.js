...
var SiteMapTable = function SiteMapTable() {
  ...
  */
  ...
  if (selectionActive && selection.size > 0) {
    if (focus === _SiteMapUtils.FEATURE_TYPES.SITES) {
      selection.forEach(function (siteCode) {
        var idx = rows.findIndex(function (row) {
          return row.siteCode === siteCode;
        });
  ...
  */
  ...
  var renderSelectionTitle = function renderSelectionTitle() {
    if (!selectionActive) {
      return null;
    }

    var unit = focus === _SiteMapUtils.FEATURE_TYPES.SITES ? 'site' : 'location';
    var s = selection.size === 1 ? '' : 's';
    var title = "".concat(selection.size ? selection.size.toString() : 'No', " ").concat(unit).concat(s, " selected");
    var style = {
      color: selection.size ? _Theme.default.palette.primary.dark : _Theme.default.palette.grey[300],
