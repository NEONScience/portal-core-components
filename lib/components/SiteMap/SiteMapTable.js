  ...
  */
  ...
  var commonColumns = {
    ...
    coordinates: {
      ...
      searchable: false,
      render: function render(row) {
        var latitude = Number.isFinite(row.latitude) ? row.latitude.toFixed(5) : null;
        var longitude = Number.isFinite(row.longitude) ? row.longitude.toFixed(5) : null;
  ...
  if (focus === _SiteMapUtils.FEATURE_TYPES.LOCATIONS.KEY) {
    columns = [{
      ...
      csvRender: function csvRender(row) {
        ...
        if (!_SiteMapUtils.NLCD_CLASSES[row.nlcdClass]) {
          return row.nlcdClass;
        }

        return _SiteMapUtils.NLCD_CLASSES[row.nlcdClass].name;
      }
    }, {
      // Plot Size
      field: 'plotSize',
      ...
      csvRender: function csvRender(row) {
        return [row.plotDimensions || null, Number.isFinite(row.plotSize) ? row.plotSize.toFixed(0) : null];
      }
    }, {
      // Plot Slope Aspect
      field: 'slopeAspect',
