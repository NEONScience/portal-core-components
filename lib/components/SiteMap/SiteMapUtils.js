...
export const isCoord = (c)=>Array.isArray(c) && c.length === 2 && c.every((x)=>Number.isFinite(x));
export const MAP_ZOOM_RANGE = [
    1,
    19
];
export const OBSERVATORY_CENTER = [
    52.68,
    -110.75
];
...
export const SELECTION_PORTIONS = {
    PARTIAL: 'PARTIAL',
    TOTAL: 'TOTAL'
};
...
/* eslint-disable max-len, @stylistic/max-len */ // Source: https://www.mrlc.gov/data/legends/national-land-cover-database-2001-nlcd2001-legend
    ...
    const newState = {
        ...
        filters: {
            ...
            features: {
                ...
                available: Object.fromEntries(Object.entries(FEATURES).map((entry)=>[
                        entry[0],
                        featureIsAvailable(entry[1])
                    ]))
            }
        }
    };
...
*/ export const getHref = (key, arg = null)=>{
    ...
    switch(key){
        case 'EXPLORE_SAMPLE_PRODUCTS_BY_SITE':
            return `${EXPLORE_SAMPLE_PRODUCTS_BASE}?datasetid=${biorepoDatasetsJSON.datasets.find((item)=>item.name === String(arg))?.datasetID || ''}`;
...
const DEFAULT_STATE = {
    ...
    featureDataFetches: Object.fromEntries(Object.keys(FEATURE_DATA_SOURCES).filter((dataSource)=>![
        ...
        ].includes(dataSource)).map((dataSource)=>[
            dataSource,
            {}
        ])),
    featureData: Object.fromEntries(Object.keys(FEATURE_TYPES).filter((featureType)=>featureType !== FEATURE_TYPES.SAMPLING_POINTS.KEY).map((featureType)=>[
            featureType,
            {}
        ])),
    sites: {},
    filters: {
        ...
        features: {
            available: {},
            visible: Object.fromEntries(Object.keys(FEATURES).map((key)=>[
                    key,
                    !featureIsHiddenByDefault(key)
                ])),
            collapsed: new Set()
        },
        overlays: {
...
export const zoomIsValid = (zoom)=>Number.isInteger(zoom) && zoom >= MAP_ZOOM_RANGE[0] && zoom <= MAP_ZOOM_RANGE[1];
export const centerIsValid = (center)=>Array.isArray(center) && center.length === 2 && center.every((v)=>typeof v === 'number' && !Number.isNaN(v));
export const boundsAreValid = (bounds)=>typeof bounds === 'object' && bounds !== null && Object.keys(bounds).every((key)=>[
            'lat',
            'lng'
        ].includes(key) && Array.isArray(bounds[key]) && bounds[key].length === 2 && bounds[key].every((v)=>typeof v === 'number') && bounds[key][1] > bounds[key][0]);
...
export const getZoomedIcon = (featureKey = null, zoom = 3, highlight = HIGHLIGHT_STATUS.NONE, selection = SELECTION_STATUS.UNSELECTED)=>{
    const feature = FEATURES[featureKey] || {};
...
export const getPhantomLeafletMap = (state)=>{
    const { aspectRatio: { currentValue: aspectRatio, widthReference }, map: { center, zoom } } = state;
...
export const getMapStateForFocusLocation = (state = {})=>{
    ...
    if (newState.map.zoom !== null) {
        ...
        newState.map.bounds = !newBounds ? null : {
            /* eslint-disable no-underscore-dangle */ lat: [
                round(newBounds._southWest.lat),
                round(newBounds._northEast.lat)
            ],
            lng: [
                round(newBounds._southWest.lng),
                round(newBounds._northEast.lng)
            ]
        };
        phantomMap.remove();
    }
...
export const findCentroid = (coords = [])=>{
    ...
    return [
        round(cLat * (180 / Math.PI)),
        round(cLng * (180 / Math.PI))
    ];
};
...
const isInBounds = (loc, extendedBounds, extendPoints = 0)=>{
    if (Number.isFinite(loc.latitude) && Number.isFinite(loc.longitude)) {
        if (extendPoints > 0) {
            const lats = [
                loc.latitude - extendPoints,
                loc.latitude + extendPoints
            ];
            const lngs = [
                loc.longitude - extendPoints,
                loc.longitude + extendPoints
            ];
...
export const calculateLocationsInBounds = (locations, bounds = null, extendMap = false, extendPoints = 0)=>{
...
export const deriveFullObservatoryZoomLevel = (map)=>{
    ...
    if (typeof map !== 'object' || map === null || !map || !map._container || !map._container.parentElement) {
