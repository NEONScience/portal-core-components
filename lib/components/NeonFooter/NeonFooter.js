...
var NeonFooter = function NeonFooter(props) {
  ...
  var _NeonContext$useNeonC = _NeonContext.default.useNeonContextState(),
      ...
      footerFetch = _NeonContext$useNeonC3.fetches.footer,
      footerHTML = _NeonContext$useNeonC3.html.footer;

  var renderMode = 'legacy';

  if (isActive) {
    if (footerFetch.status === _NeonContext.FETCH_STATUS.SUCCESS && footerHTML && drupalCssLoaded) {
      renderMode = 'drupal';
    }

    if ([_NeonContext.FETCH_STATUS.AWAITING_CALL, _NeonContext.FETCH_STATUS.FETCHING].includes(footerFetch.status)) {
      renderMode = 'loading';
    }
  }
