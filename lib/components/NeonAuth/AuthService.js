/* eslint-disable @next/next/no-location-assign-relative-destination */ import { Subject, of, map, catchError, delay, tap } from 'rxjs';
...
const AuthService = {
    ...
    handleLoginMessageFromBroadcastChannel: (dispatch)=>{
    ...
    watchAuth0: (dispatch, onConnectCbs)=>{
        ...
        const topicEndpoint = NeonEnvironment.authTopics.getAuth0();
        const watchSubscription$ = client.watch(topicEndpoint).pipe(map((message)=>{
            try {
