 ...
 */
const getJsonObservable = (url, headers = undefined, includeToken = true, withCredentials = undefined) => {
...
const NeonApi = {
   ...
   */
  getJson: (url, callback, errorCallback, cancellationSubject$, headers = undefined) => (0, _rxUtil.getJson)(url, callback, errorCallback, cancellationSubject$, getApiTokenHeader(headers)),
  /**
  ...
  getProductBundlesObservable: (release = null) => {
    ...
    const path = (0, _typeUtil.isStringNonEmpty)(release) ? `${root}?release=${release}` : `${root}`;
    return getJsonObservable(path);
  },
  /**
   ...
   * @return The RxJS Ajax Observable
   */
  getPrototypeDatasetsObservable: () => getJsonObservable(`${_NeonEnvironment.default.getFullApiPath('prototype')}/datasets`),
  getPrototypeDatasetObservable: uuid => getJsonObservable(`${_NeonEnvironment.default.getFullApiPath('prototype')}/datasets/${uuid}`),
  getPrototypeManifestRollupObservable: uuid => getJsonObservable(`${_NeonEnvironment.default.getFullDownloadApiPath('prototypeManifestRollup')}?uuid=${uuid}`),
  getPrototypeDataFileObservable: (uuid, fileName) => getJsonObservable(`${_NeonEnvironment.default.getFullApiPath('prototype')}/data/${uuid}/${fileName}`),
  /**
   ...
   * @return The RxJS Ajax Observable
   */
  getReleaseObservable: release => getJsonObservable(`${_NeonEnvironment.default.getFullApiPath('releases')}/${release}`),
  /**
   ...
   * @return The RxJS Ajax Observable
   */
  getSiteJsonObservable: siteCode => getJsonObservable(`${_NeonEnvironment.default.getFullApiPath('sites')}/${siteCode}`),
  /**
   ...
   * @return The RxJS Ajax Observable
   */
  getSiteLocationHierarchyObservable: siteCode => getJsonObservable(`${_NeonEnvironment.default.getFullApiPath('locations')}/${siteCode}?hierarchy=true`),
  /**
   ...
   * @return The RxJS Ajax Observable
   */
  getLocationObservable: location => getJsonObservable(`${_NeonEnvironment.default.getFullApiPath('locations')}/${location}`),
  /**
   ...
   * @return The RxJS Ajax Observable
   */
  getArcgisAssetObservable: (feature, siteCode) => getJsonObservable(`${_NeonEnvironment.default.getFullApiPath('arcgisAssets')}/${feature}/${siteCode}`, undefined, true, false),
  /**
   * Gets the RxJS Observable for the document HEAD endpoint for a given name
   * @param {string} name The document name
   * @return The RxJS Ajax Observable
   */
  headDocumentObservable: name => headJsonObservable(`${_NeonEnvironment.default.getFullApiPath('documents')}/${name}`),
  /**
   ...
   * @return The RxJS Ajax Observable
   */
  getQuickStartGuideDetailObservable: (name, version) => getJsonObservable(`${_NeonEnvironment.default.getFullApiPath('quickStartGuides')}/details/${name}/${version}`),
  /**
   * Gets the RxJS Observable for the quick start guides HEAD endpoint for a given name
   * @param {string} name The document name
   * @return The RxJS Ajax Observable
   */
  headQuickStartGuideObservable: name => headJsonObservable(`${_NeonEnvironment.default.getFullApiPath('quickStartGuides')}/${name}`)
};
