...
var getAvailableReleaseClause = function getAvailableReleaseClause(args) {
  if (!args) return '';
  var hasRelease = (0, _typeUtil.isStringNonEmpty)(args.release);
  ...
  switch (type) {
    case TYPES.DATA_PRODUCTS:
      if (dimensionality === DIMENSIONALITIES.ONE) {
        ...
        var availableReleases = getAvailableReleaseClause(args);
        ...
        var _availableReleases = getAvailableReleaseClause(args);
...
var getAjaxRequest = function getAjaxRequest(body) {
  ...
  var withCredentials = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : undefined;
...
var NeonGraphQL = {
  getDataProductByCode: function getDataProductByCode(productCode, release) {
    var includeAvailableReleases = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;
    return getObservableWith(TYPES.DATA_PRODUCTS, DIMENSIONALITIES.ONE, {
      productCode: productCode,
      release: release,
      includeAvailableReleases: includeAvailableReleases
    });
  },
  getAllDataProducts: function getAllDataProducts(release) {
    var includeAvailableReleases = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
