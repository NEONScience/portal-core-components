...
const renderExternalHostLink = (href = '', text = '', host = '', productCode = 'n/a') => /*#__PURE__*/(0, _jsxRuntime.jsx)(_Link.default, {
  href: href,
  target: "_blank",
  ...
  BOLD: {
    ...
    getProductLinks: (productCode = '') => {
      ...
      return externalProducts[productCode].searches.map(search => ({
        key: search.query,
        node: renderExternalHostLink(`https://portal.boldsystems.org/recordset/${search.query}`, search.title, 'BOLD', productCode)
      }));
    }
  },
  NPN: {
...
Object.keys(externalHosts).forEach(hostId => {
  externalHosts[hostId].renderLink = (productCode = 'n/a') => renderExternalHostLink(externalHosts[hostId].url, externalHosts[hostId].projectTitle, hostId, productCode);
  externalHosts[hostId].renderShortLink = (productCode = 'n/a') => renderExternalHostLink(externalHosts[hostId].url, externalHosts[hostId].name, hostId, productCode);
});
const getByHostId = (hostId = '') => externalHosts[hostId] || null;
const getByProductCode = (productCode = '') => externalProducts[productCode] ? externalHosts[externalProducts[productCode].host] : null;
const getProductSpecificInfo = (productCode = '') => externalProducts[productCode] ? externalProducts[productCode] : null;
const ExternalHost = {
  HOST_TYPES,
  LINK_TYPES,
  getByHostId,
