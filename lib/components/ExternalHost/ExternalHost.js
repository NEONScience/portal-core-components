...
const externalHosts = {
  AERONET: {
    ...
    getSiteLink: function () {
      ...
      const hrefSite = Object.keys(nonStandardSites).includes(siteCode) ? nonStandardSites[siteCode] : `NEON_${siteCode}`;
      const description = Object.keys(allSites).length ? ` - ${allSites[siteCode].description}` : '';
  ...
  BOLD: {
    ...
    getProductLinks: function () {
      ...
      return externalProducts[productCode].searches.map(search => ({
        key: search.query,
        node: renderExternalHostLink(`http://www.boldsystems.org/index.php/Public_SearchTerms?query=${search.query}`, search.title, 'BOLD', productCode)
      }));
    }
  },
  NPN: {
