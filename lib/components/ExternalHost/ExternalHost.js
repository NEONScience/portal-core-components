...
var externalHosts = {
  AERONET: {
    ...
    getSiteLink: function getSiteLink() {
      ...
      var text = siteCode + ' - ' + _sites2.default[siteCode].description;
  ...
  MGRAST: {
    ...
    getProductLinks: function getProductLinks() {
      ...
      return externalProducts[productCode].projects.map(function (project) {
        return {
          key: project.id,
          node: renderExternalHostLink('https://www.mg-rast.org/mgmain.html?mgpage=project&project=' + project.id, project.title, 'MGRAST', productCode)
        };
      });
    }
  },
