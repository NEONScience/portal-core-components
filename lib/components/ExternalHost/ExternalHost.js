...
const HOST_TYPES = {
  ...
  // NEON and host offer different / complementary data
  REFORMATTED_DATA: 'REFORMATTED_DATA',
  // NEON and host offer same data in different formats
  EXCLUSIVE_DATA: 'EXCLUSIVE_DATA' // NEON does not offer any data, only host has it accessible
};
const LINK_TYPES = {
  BY_SITE: 'BY_SITE',
  // Show links to external host site page(s)
...
const externalProducts = {
  ...
  'DP1.10020.001': {
    ...
    searches: [{
      query: 'BETN',
      title: 'Ground beetle sequences DNA barcode'
    }, {
      query: 'BETP',
      title: 'Ground beetle sequences DNA barcode (Prototype Data)'
    }]
  },
  'DP1.10038.001': {
    host: 'BOLD',
    searches: [{
      query: 'MOSN',
      title: 'Mosquito sequences DNA barcode'
    }, {
      query: 'MOSP',
      title: 'Mosquito sequences DNA barcode (Prototype Data)'
    }]
  },
  'DP1.10055.001': {
    host: 'NPN'
  ...
  'DP1.10076.001': {
    ...
    searches: [{
      query: 'MAMN',
      title: 'Small mammal sequences DNA barcode'
    }, {
      query: 'MAMPR',
      title: 'Small mammal sequences DNA barcode (Prototype Data)'
    }]
  },
  'DP1.20002.001': {
    host: 'PHENOCAM'
...
const renderExternalHostLink = function () {
  let href = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : '';
  let text = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : '';
  let host = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : '';
  let productCode = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : 'n/a';
...
const externalHosts = {
  AERONET: {
    ...
    getSiteLink: function () {
      let allSites = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
      let siteCode = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : '';
      let productCode = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 'n/a';
      ...
      const hrefSite = Object.keys(nonStandardSites).includes(siteCode) ? nonStandardSites[siteCode] : "NEON_".concat(siteCode);
      const description = Object.keys(allSites).length ? " - ".concat(allSites[siteCode].description) : '';
      const text = "".concat(siteCode).concat(description);
      return renderExternalHostLink("".concat(hrefBase).concat(hrefSite), text, 'AERONET', productCode);
    }
  },
  AMERIFLUX: {
    id: 'AMERIFLUX',
    ...
    getSiteLink: function () {
      ...
      const description = Object.keys(allSites).length ? " - ".concat(allSites[siteCode].description) : '';
      const text = "".concat(siteCode, " - ").concat(description);
      return renderExternalHostLink("".concat(hrefBase, "/").concat(hrefSite, "/"), text, 'AMERIFLUX', productCode);
    }
  },
  BOLD: {
    id: 'BOLD',
    ...
    getProductLinks: function () {
      let productCode = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : '';
  ...
  PHENOCAM: {
    ...
    getSiteLink: function () {
      let allSites = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
      let siteCode = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : '';
      let productCode = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : '';
      if (!allSites[siteCode]) {
        return null;
      }
      const hrefBase = 'https://phenocam.nau.edu/webcam/sites';
      const hrefSite = "".concat(allSites[siteCode].domainCode, ".").concat(siteCode, ".").concat(productCode.split('.').slice(0, 2).join('.'));
      const text = "".concat(siteCode, " - ").concat(allSites[siteCode].description);
...
Object.keys(externalHosts).forEach(hostId => {
  externalHosts[hostId].renderLink = function () {
    let productCode = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 'n/a';
    return renderExternalHostLink(externalHosts[hostId].url, externalHosts[hostId].projectTitle, hostId, productCode);
  };
  externalHosts[hostId].renderShortLink = function () {
    let productCode = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 'n/a';
    return renderExternalHostLink(externalHosts[hostId].url, externalHosts[hostId].name, hostId, productCode);
  };
});
const getByHostId = function () {
  let hostId = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : '';
...
const getProductSpecificInfo = function () {
  let productCode = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : '';
...
var _default = exports.default = ExternalHost;