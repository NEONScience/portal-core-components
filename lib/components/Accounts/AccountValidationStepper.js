...
const getStepContents = (theme, activeStep, stepCompleted, customSteps)=>{
    const hasCustomStep = exists(customSteps) && exists(customSteps[activeStep]);
    if (hasCustomStep) {
        const coercedStep = customSteps[activeStep];
...
const AccountValidationStepper = (inProps)=>{
    ...
    const renderVerticalStepContents = ()=>{
        ...
        const stepContents = hasStep(activeStep, accountValidationStepDisplay) ? getStepContents(theme, activeStep, stepCompleted, accountValidationStepDisplay) : activeStep;
