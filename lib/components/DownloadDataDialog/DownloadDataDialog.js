...
export default function DownloadDataDialog() {
  ...
  */ const [{ downloadStatus, dialogOpen, productData, manifest, requiredSteps, allStepsComplete, fromManifest, fromAOPManifest, documentation, s3Files, release, latestRelease, sites, dateRange, packageType, provisionalData }, dispatch] = DownloadDataContext.useDownloadDataState();
  ...
  */ const renderSizeEstimate = ()=>{
    ...
    const renderDownloadButtonStepNote = ()=>{
        ...
        if (!allStepsComplete) {
            const completeStepsMessage = 'Complete all steps to enable download. ' + `${completedSteps.length} of ${completableSteps.length} completed.`;
            return /*#__PURE__*/ _jsx(Typography, {
                variant: "body2",
                style: {
            ...
            return /*#__PURE__*/ _jsxs("div", {
                children: [
                    ...
                    /*#__PURE__*/ _jsx("div", {
                        ...
                        children: renderDownloadButtonStepNote()
                    })
                ]
            });
        }
    ...
    if (release.value === null) {
        releaseTooltip = 'You are downloading only the latest released and provisional data ' + `(release: ${latestRelease || 'unknown'}).`;
    } else {
        releaseTooltip = 'You are downloading product data only from the ' + `${release.value} release (no provisional data will be included).`;
    }
