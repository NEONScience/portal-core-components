...
var useStyles = (0, _styles.makeStyles)(function (theme) {
  return {
    ...
    releaseChip: {
      color: _Theme.default.colors.BROWN[500],
      border: "1px solid ".concat(_Theme.default.colors.BROWN[500]),
      backgroundColor: _Theme.default.colors.BROWN[100],
      fontWeight: 600,
...
function TimeSeriesViewerSummary() {
  ...
  // Release
  var useReleaseChip = false;
  var latestRelease = null;
  if (state.releases && state.releases.length) {
    var sortedReleases = _toConsumableArray(state.releases).sort(function (a, b) {
      return a.generationDate > b.generationDate ? -1 : 1;
    });
    latestRelease = sortedReleases[0].release;
  }
  var latestReleaseClause = useReleaseChip ? '' : " (release: ".concat(latestRelease || 'unknown', ")");
  var releaseTooltip = state.release === null ? "You are viewing only the latest released and provisional data (release: ".concat(latestRelease || 'unknown', ").") : "You are viewing product data only from the ".concat(state.release, " release (no provisional data will be included).");
  var releaseChipLabel = state.release === null ? "Latest released and provisional data".concat(latestReleaseClause) : "".concat(state.release);
  var releaseSummary = !useReleaseChip ? /*#__PURE__*/_react.default.createElement(_Typography.default, {
    variant: "body2",
    key: releaseChipLabel
  }, releaseChipLabel) : /*#__PURE__*/_react.default.createElement("div", null, /*#__PURE__*/_react.default.createElement(_Tooltip.default, {
    ...
    }, releaseChipLabel, /*#__PURE__*/_react.default.createElement(_InfoOutlined.default, {
      fontSize: "small"
    })),
...
function TimeSeriesViewerContainer() {
  ...
  var _useState3 = (0, _react.useState)(state.product.productCode),
    ...
    setLoadedProductCode = _useState4[1];
  ...
  var renderGraphOverlay = function renderGraphOverlay() {
    var isError = state.status === _constants.TIME_SERIES_VIEWER_STATUS.ERROR;
    var isWarning = state.status === _constants.TIME_SERIES_VIEWER_STATUS.WARNING;
    var isLoading = !isError && state.status !== _constants.TIME_SERIES_VIEWER_STATUS.READY;
    ...
    var isLoadingData = isLoading && state.status === _constants.TIME_SERIES_VIEWER_STATUS.LOADING_DATA;
    if (isLoading) {
