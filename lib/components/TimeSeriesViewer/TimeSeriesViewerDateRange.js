...
var TimeSeriesViewerDateRange = function TimeSeriesViewerDateRange(props) {
  ...
  selectedSites.forEach(function (siteCode) {
    ...
    var avaReleases = state.product.sites[siteCode].availableReleases;
    if (Array.isArray(avaReleases)) {
      var provRelease = avaReleases.find(function (value) {
  ...
  var svgHeight = _AvailabilityUtils.SVG.CELL_PADDING + (_AvailabilityUtils.SVG.CELL_HEIGHT + _AvailabilityUtils.SVG.CELL_PADDING) * (selectedSites.length + 1);
