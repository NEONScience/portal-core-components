...
import React, { useCallback, useEffect, useLayoutEffect, useState, useRef, useMemo, useReducer } from 'react';
...
const getYearMonthMoment = (yearMonth, day = 15)=>moment(`${yearMonth}-${day.toString().padStart(2, '0')}`);
const svgMinWidth = (SVG.CELL_WIDTH + SVG.CELL_PADDING) * SVG.MIN_CELLS + Math.floor(SVG.MIN_CELLS / 12) * SVG.YEAR_PADDING;
...
const TimeSeriesViewerDateRange = (props)=>{
    ...
    const { dateRange: currentRange, sites: stateSelectionSites } = state.selection;
    let selectableRange = state.product.dateRange;
    const { sites: stateProductSites, continuousDateRange: displayRange } = state.product;
    const displayMin = 0;
    ...
    const sliderValue = dateRangeState.activelySelectingDateRange.map((v, i)=>displayRange.indexOf(dateRangeState.activelySelectingDateRange[i] || currentRange[i]));
    useEffect(()=>{
        if ((currentRange[0] !== dateRangeState.activelySelectingDateRange[0] || currentRange[1] !== dateRangeState.activelySelectingDateRange[1]) && !activelySelecting) {
            const action = {
    ...
    const pointsAvailable = POINTS_PERFORMANCE_LIMIT - TimeSeriesViewerContext.calcPredictedPointsByDateRange(state, currentRange[0], currentRange[1]);
    // Determine a good enough estimation for how many
    // points per month we should consider given a current month.
    const pointsPerMonth = TimeSeriesViewerContext.calcPredictedPointsByDateRange(state, currentRange[0], currentRange[0]);
    ...
    if (monthsAvailable < displayRange.length) {
        ...
        selectableRange = [
            displayRange[sliderMin],
            displayRange[sliderMax]
        ];
    }
    ...
    const svgHeight = SVG.CELL_PADDING + (SVG.CELL_HEIGHT + SVG.CELL_PADDING) * (selectedSites.length + 1);
    ...
    const handleChangeDatePicker = (rangeIndex, value)=>{
        ...
        if (!formattedValue || ![
            0,
            1
        ].includes(rangeIndex) || newSliderValue === -1) {
            return;
        }
    ...
    const renderedSlider = uniqueSliderMarks <= 1 ? null : /*#__PURE__*/ _jsx(Slider, {
            ...
            /*#__PURE__*/ _jsxs("div", {
                ...
                children: [
                    ...
                    /*#__PURE__*/ _jsx(Typography, {
                        variant: "h6",
                        gutterBottom: true,
                        children: "Select by Data Product Availability"
                    }),
                    /*#__PURE__*/ _jsx(FullWidthVisualization, {
                        vizRef: svgRef,
                    ...
                    /*#__PURE__*/ _jsx(BasicAvailabilityKey, {
                        style: {
                            flexGrow: 1
                        },
