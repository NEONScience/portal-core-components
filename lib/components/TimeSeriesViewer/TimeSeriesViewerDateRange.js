...
var TimeSeriesViewerDateRange = function TimeSeriesViewerDateRange(props) {
  ...
  var applySliderValues = (0, _react.useCallback)(function (values) {
    ...
    var newLefts = displayMax ? ["".concat(limited[0] / displayMax * 100, "%"), "".concat(limited[1] / displayMax * 100, "%")] : ['0%', '100%']; // Apply values to Slider DOM hidden input

    dateRangeSliderRef.current.querySelector('input[type="hidden"]').setAttribute('value', limited.join(',')); // Apply values to slider drag handles

    [0, 1].forEach(function (idx) {
      dateRangeSliderRef.current.querySelector("span[role=\"slider\"][data-index=\"".concat(idx, "\"]")).setAttribute('aria-valuenow', limited[idx].toString());
      dateRangeSliderRef.current.querySelector("span[role=\"slider\"][data-index=\"".concat(idx, "\"]")).style.left = newLefts[idx];
      dateRangeSliderRef.current.querySelector("span[role=\"slider\"][data-index=\"".concat(idx, "\"] > span > span > span")).innerText = displayRange[limited[idx]];
    }); // Apply values to slider track between drag handles

    var newTrackWidth = "".concat((limited[1] - limited[0]) / displayMax * 100, "%");
