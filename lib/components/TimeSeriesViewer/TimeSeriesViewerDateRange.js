...
var TimeSeriesViewerDateRange = function TimeSeriesViewerDateRange(props) {
  ...
  var getBoundedRange = function getBoundedRange(values) {
    return [Math.max(values[0], sliderMin), Math.min(values[1], sliderMax)];
  ...
  var applySliderValues = (0, _react.useCallback)(function (values) {
    ...
    var limited = getBoundedRange(values); // Derive new percentage values for left and width styles of slider DOM elements
  ...
  return _react.default.createElement("div", null, _react.default.createElement(DateRangeSlider, {
    ...
    onChangeCommitted: function onChangeCommitted(event, values) {
      dispatch({
        type: 'selectDateRange',
        dateRange: getBoundedRange(values).map(function (x) {
          return displayRange[x];
        })
      });
    }
