...
*/ function PositionHistoryButton(inProps) {
    ...
    const dialogDescription = `${positionsDescription} The table below shows changes to the physical ` + 'location of this position since its creation.';
                                            ...
                                            children: history.map((row, idx)=>{
                                                ...
                                                if (!Number.isNaN(parsedReferenceElevation)) {
                                                    if (!Number.isNaN(parsedZOffset)) {
                                                        const calcOffsetElevation = (parsedReferenceElevation + parsedZOffset).toFixed(2).toString();
...
*/ function SiteOption(inProps) {
    ...
    if (stateCode) {
        ...
        const optionSubtitle = `${terrainTypeTitle} ` + `- Domain ${domainCode} (${domainName}) ` + `- Lat/Lon: ${latitude.toFixed(6)}, ${longitude.toFixed(6)}`;
