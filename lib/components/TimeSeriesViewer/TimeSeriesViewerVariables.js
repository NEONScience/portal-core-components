...
function Control(props) {
  ...
  const numPoints = _TimeSeriesViewerContext.default.calcPredictedPointsForNewVariable(state);
  const labelText = numPoints > _TimeSeriesViewerContext.POINTS_PERFORMANCE_LIMIT ? 'Add Variables (disabled)' : 'Add Variables';
...
function Option(inProps) {
  const props = (0, _defaultProps.resolveProps)(optionDefaultProps, inProps);
...
function TimeSeriesViewerVariables() {
  ...
  const isDisabled = _TimeSeriesViewerContext.default.calcPredictedPointsForNewVariable(state) > _TimeSeriesViewerContext.POINTS_PERFORMANCE_LIMIT;
  return /*#__PURE__*/(0, _jsxRuntime.jsxs)("div", {
