...
var parseSeriesData = function parseSeriesData(csv) {
  var series = parseCSV(csv);
  /*
    GOAL: go from this:
...
var reducer = function reducer(state, action) {
  ...
  switch (action.type) {
    ...
    case 'fetchDataFileSucceeded':
      ...
      newState.product.sites[action.siteCode].positions[action.position].data[action.month][action.downloadPkg][action.timeScale].series = parseSeriesData(action.csv);
      return newState;
    // Selection Actions
      ...
      state.selection.variables.forEach(function (variable) {
        ...
        positions.forEach(function (position) {
          continuousDateRange.forEach(function (month) {
            ...
            var _state$product$sites$ = state.product.sites[siteCode].positions[position].data[month][downloadPkg][timeScale],
                url = _state$product$sites$.url,
                status = _state$product$sites$.status; // If the file isn't awaiting a fetch call then don't fetch it
