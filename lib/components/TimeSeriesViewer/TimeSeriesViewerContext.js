...
var COMP_STATUS = {
  ...
  LOADING: 'LOADING',
  // Actively loading site and/or series data
  ERROR: 'ERROR',
...
var getSeriesVariables = function getSeriesVariables(variables) {
  var forDefault = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
  return Object.keys(variables).filter(function (v) {
    if (['startDateTime', 'endDateTime'].includes(v) || variables[v].units === 'NA' || /QF$/.test(v)) {
      return false;
    }

    if (forDefault && (variables[v].downloadPkg === 'expanded' || /QM$/.test(v))) {
      return false;
    }
...
var getContinuousDatesArray = function getContinuousDatesArray(dateRange) {
  ...
  var startMoment = (0, _moment.default)("".concat(dateRange[0], "-10"));
  var endMoment = (0, _moment.default)("".concat(dateRange[1], "-20")).add(1, 'months');

  if (roundToYears) {
    startMoment = (0, _moment.default)("".concat(dateRange[0], "-10")).startOf('year');
    endMoment = (0, _moment.default)("".concat(dateRange[1], "-20")).endOf('year').add(1, 'months');
  }

  var contionuousRange = [];
  var months = 0;
...
var parseSiteVariables = function parseSiteVariables(stateVariables, siteCode, csv) {
  var newStateVariables = _extends({}, stateVariables);

  var variables = (0, _papaparse.parse)(csv, CSV_CONFIG);
...
var parseSitePositions = function parseSitePositions(site, csv) {
  ...
  var positions = (0, _papaparse.parse)(csv, CSV_CONFIG);
...
var applyDefaultsToSelection = function applyDefaultsToSelection(state) {
  ...
  if (!selection.variables.length && Object.keys(variables).length) {
    var seriesVars = getSeriesVariables(variables, true);

    if (seriesVars.length) {
      selection.variables.push(seriesVars[0]);
    }
...
var reducer = function reducer(state, action) {
  ...
  var calcStatus = function calcStatus(fetches) {
    return Object.keys(fetches).length ? COMP_STATUS.LOADING : COMP_STATUS.READY;
  };
  ...
  switch (action.type) {
    ...
    case 'fetchSiteMonth':
      newState.fetches["fetchSiteMonth.".concat(action.siteCode, ".").concat(action.month)] = true;
    ...
    case 'fetchSiteMonthFailed':
      delete newState.fetches["fetchSiteMonth.".concat(action.siteCode, ".").concat(action.month)];
    ...
    case 'fetchSiteVariables':
      newState.fetches["fetchSiteVariables.".concat(action.siteCode)] = true;
      newState.product.sites[action.siteCode].fetches.variables.status = FETCH_STATUS.FETCHING;
      return newState;

    case 'fetchSiteVariablesFailed':
      delete newState.fetches["fetchSiteVariables.".concat(action.siteCode)];
    ...
    case 'fetchSitePositions':
      newState.fetches["fetchSitePositions.".concat(action.siteCode)] = true;
      newState.product.sites[action.siteCode].fetches.positions.status = FETCH_STATUS.FETCHING;
      return newState;

    case 'fetchSitePositionsFailed':
      delete newState.fetches["fetchSitePositions.".concat(action.siteCode)];
  ...
  (0, _react.useEffect)(function () {
    ...
    state.selection.sites.forEach(function (site) {
      ...
      if (fetches.variables.status === FETCH_STATUS.AWAITING_CALL && fetches.variables.url) {
        ...
        getCSV(fetches.variables.url).pipe((0, _operators.map)(function (response) {
          dispatch({
      ...
      if (fetches.positions.status === FETCH_STATUS.AWAITING_CALL && fetches.positions.url) {
        ...
        getCSV(fetches.positions.url).pipe((0, _operators.map)(function (response) {
          dispatch({
