...
var parseSiteVariables = function parseSiteVariables(previousVariables, siteCode, csv) {
  ...
  var ignoreTables = new Set(['sensor_positions', 'science_review_flags']); // Build the set of variables using only the valid tables
...
var applyDefaultsToSelection = function applyDefaultsToSelection(state) {
  ...
  if (status === _constants.TIME_SERIES_VIEWER_STATUS.READY_FOR_SERIES && !hasVariablesSelected && selection.variables.length && selection.yAxes.y1.dataRange.every(function (x) {
    return x === null;
  })) {
...
var Provider = function Provider(props) {
  ...
  */
  ...
  (0, _react.useEffect)(function () {
    ...
    _NeonGraphQL.default.getDataProductByCode(state.product.productCode, state.release).pipe((0, _rxjs.map)(function (response) {
