...
exports.summarizeTimeSteps = exports.getTestableItems = exports.defaultProps = exports.default = exports.Y_AXIS_RANGE_MODE_DETAILS = exports.Y_AXIS_RANGE_MODES = exports.TabComponentPropTypes = exports.TIME_STEPS = exports.TIME_SERIES_VIEWER_STATUS_TITLES = exports.POINTS_PERFORMANCE_LIMIT = exports.MAX_NUM_SITES_SELECTABLE = exports.DEFAULT_STATE = void 0;
...
const MAX_NUM_SITES_SELECTABLE = exports.MAX_NUM_SITES_SELECTABLE = 5;
...
const summarizeTimeSteps = (steps, timeStep = null, pluralize = true) => {
  if (steps === 1) {
    return 'none';
  }
...
const getContinuousDatesArray = (dateRange, roundToYears = false) => {
  ...
  let startMoment = (0, _moment.default)(`${dateRange[0]}-20`);
  let endMoment = (0, _moment.default)(`${dateRange[1]}-10`).add(1, 'months');
  if (roundToYears) {
    startMoment = (0, _moment.default)(`${dateRange[0]}-20`).startOf('year');
    endMoment = (0, _moment.default)(`${dateRange[1]}-10`).endOf('year').add(1, 'months');
  }
      ...
      selection.sites.forEach(site => {
        ...
        positions.forEach(position => {
          selection.continuousDateRange.forEach(month => {
            ...
            const timeStepTables = (0, _get.default)(product.sites[siteCode].positions[position], `data['${month}']['${pkg}']['${dataTimeStep}']`, {});
...
const Provider = inProps => {
  ...
  (0, _react.useEffect)(() => {
    ...
    if (state.status === _constants.TIME_SERIES_VIEWER_STATUS.READY_FOR_DATA && !metaFetchTriggered) {
      if (!dataFetches.length) {
        ...
        const masterFetchToken = `fetchDataFiles.${(0, _uniqueId.default)()}`;
        dispatch({
          type: 'fetchDataFiles',
          token: masterFetchToken,
          fetches: dataActions
        });
        (0, _rxUtil.forkJoinWithProgress)(dataFetches).pipe((0, _rxjs.mergeMap)(([finalResult, progress]) => (0, _rxjs.merge)(progress.pipe((0, _rxjs.tap)(value => dispatch({
          type: 'fetchDataFilesProgress',
          token: masterFetchToken,
          value
        })), (0, _rxjs.ignoreElements)()), finalResult))).subscribe(value => dispatch({
  ...
  */
  return (
    ...
    (0, _jsxRuntime.jsx)(Context.Provider, {
