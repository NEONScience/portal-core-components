...
*/ const Provider = (inProps)=>{
    ...
    const { timeSeriesUniqueId, mode: modeProp, productCode: productCodeProp, productData: productDataProp, release: releaseProp, children } = props;
    /**
  ...
  */ const initialState = cloneDeep(DEFAULT_STATE);
    ...
    const stateStorage = useMemo(()=>makeStateStorage(stateKey), [
        stateKey
    ]);
    ...
    const appliedInitialState = initState.restoredStateFromStorage ? initState.initialState : initialState;
  ...
  */ useEffect(()=>{
        ...
        NeonGraphQL.getDataProductByCode(state.product.productCode, state.release, true).pipe(map((response)=>{
  ...
  */ useEffect(()=>{
        const getSiteMonthDataURL = (siteCode, month)=>{
            ...
            const releaseParam = hasRelease ? `?release=${state.release}` : '';
        ...
        const fetchNeededSiteMonths = (siteCode, fetches)=>{
            continuousDateRange.filter((month)=>!fetches.siteMonths[month]).forEach((month)=>{
                ...
                NeonApi.getJsonObservable(getSiteMonthDataURL(siteCode, month), NeonApi.getApiTokenHeader()).pipe(map((response)=>{
