...
exports.getTestableItems = exports.default = exports.summarizeTimeSteps = exports.TIME_STEPS = exports.DEFAULT_STATE = exports.TabComponentPropTypes = exports.Y_AXIS_RANGE_MODE_DETAILS = exports.Y_AXIS_RANGE_MODES = exports.TIME_SERIES_VIEWER_STATUS_TITLES = void 0;
...
var PREFERRED_DATETIME_VARIABLES = ['startDateTime', 'endDateTime', 'startDate', 'endDate']; // Keys, details, and supporting functions for all possible Y-axis range modes
...
var getTimeStepForTableName = function getTimeStepForTableName() {
  ...
  return Object.keys(TIME_STEPS).find(function (key) {
    return tableName.endsWith("_".concat(key));
  }) || null;
};
...
var parseSiteVariables = function parseSiteVariables(previousVariables, siteCode, csv) {
  ...
  var variables = parseCSV(csv); // Build a list of tables we care about. Some products include maintenance tables that should
  ...
  variables.data.filter(function (variable) {
    return validTables.has(variable.table);
    ...
    var timeStep = getTimeStepForTableName(table);
...
var applyDefaultsToSelection = function applyDefaultsToSelection(state) {
  ...
  if (Object.keys(variables).length) {
    ...
    if (!selection.variables.length) {
      var defaultVar = Object.keys(variables).find(function (v) {
        return variables[v].canBeDefault && !selection.invalidDefaultVariables.has(v);
      });
  ...
  if (status === _constants.TIME_SERIES_VIEWER_STATUS.READY_FOR_SERIES && selection.isDefault && selection.variables.length && selection.yAxes.y1.dataRange.every(function (x) {
    return x === null;
  })) {
...
var reducer = function reducer(state, action) {
  ...
  switch (action.type) {
    ...
    case 'fetchDataFileSucceeded':
      ...
      newState.product.sites[action.siteCode].positions[action.position].data[action.month][action.downloadPkg][action.timeStep].series = action.series;
      return newState;
    ...
    */
    ...
    case 'updateSelectedSites':
      ...
      newState.selection.isDefault = false; // Remove any sites that are no longer in the selected set
...
var Provider = function Provider(props) {
  ...
  */
  ...
  initialState.selection = applyDefaultsToSelection(initialState); // get the state from storage if present
  ...
  var viewerStatus = state.viewerStatus; // The current sign in process uses a separate domain. This function
  ...
  */
  ...
  (0, _react.useEffect)(function () {
    ...
    _NeonGraphQL.default.getDataProductByCode(state.product.productCode, state.release).pipe((0, _operators.map)(function (response) {
  ...
  */
  ...
  (0, _react.useEffect)(function () {
    ...
    _NeonApi.default.getReleasesObservable().pipe((0, _operators.map)(function (response) {
  ...
  (0, _react.useEffect)(function () {
    ...
    var fetchNeededSiteMonths = function fetchNeededSiteMonths(siteCode, fetches) {
      continuousDateRange.filter(function (month) {
        ...
        _NeonApi.default.getJsonObservable(getSiteMonthDataURL(siteCode, month), _NeonApi.default.getApiTokenHeader()).pipe((0, _operators.map)(function (response) {
      ...
      fetchVariables.forEach(function (variable) {
        ...
        positions.forEach(function (position) {
          continuousDateRange.forEach(function (month) {
            ...
            }), (0, _operators.switchMap)(function (csv) {
