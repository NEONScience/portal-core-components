...
var parseSiteMonthData = function parseSiteMonthData(site, month, files) {
  var newSite = _extends({}, site);

  var availableTimeSteps = new Set();
  var metaFetchQueued = false;
  files.forEach(function (file) {
    ...
    if (parts[DATA_FILE_PARTS.MONTH.offset] !== month) {
      return;
    } // Extract parts
  ...
  return {
    ...
    metaFetchQueued: metaFetchQueued
  };
};
/**
...
var parseSitePositions = function parseSitePositions(site, month, csv) {
  var newSite = _extends({}, site);
...
var reducer = function reducer(state, action) {
  ...
  var calcStatus = function calcStatus() {
    var metaFetchQueued = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : false;
  ...
  switch (action.type) {
    ...
    case 'fetchSiteMonthSucceeded':
      ...
      parsedContent = parseSiteMonthData(newState.product.sites[action.siteCode], action.month, action.files);
      ...
      calcSelection(); // parseSiteMonthData will "queue" fetches for meta files like positions and variables but
    ...
    case 'fetchSitePositions':
      newState.metaFetches["fetchSitePositions.".concat(action.siteCode, ".").concat(action.month)] = true;
      newState.product.sites[action.siteCode].fetches.positions[action.month].status = FETCH_STATUS.FETCHING;
      calcStatus();
      return newState;

    case 'fetchSitePositionsFailed':
      delete newState.metaFetches["fetchSitePositions.".concat(action.siteCode, ".").concat(action.month)];
      newState.product.sites[action.siteCode].fetches.positions[action.month].status = FETCH_STATUS.ERROR;
    ...
    case 'fetchSitePositionsSucceeded':
      delete newState.metaFetches["fetchSitePositions.".concat(action.siteCode, ".").concat(action.month)];
      newState.product.sites[action.siteCode].fetches.positions[action.month].status = FETCH_STATUS.SUCCESS;
      newState.product.sites[action.siteCode] = parseSitePositions(newState.product.sites[action.siteCode], action.month, action.csv);
      calcSelection();
      calcStatus();
      return newState;
...
var Provider = function Provider(props) {
  ...
  */
  ...
  (0, _react.useEffect)(function () {
    ...
    var fetchSitePositions = function fetchSitePositions(siteCode, month, url) {
      dispatch({
    ...
    state.selection.sites.forEach(function (site) {
      ...
      continuousDateRange.filter(function (month) {
        return Object.keys(fetches.positions).includes(month) && fetches.positions[month].status === FETCH_STATUS.AWAITING_CALL && fetches.positions[month].url;
      }).forEach(function (month) {
