...
const PREFERRED_DATETIME_VARIABLES = ['startDateTime', 'endDateTime', 'startDate', 'endDate', 'date'];
...
const DEFAULT_STATE = exports.DEFAULT_STATE = {
  ...
  status: _constants.TIME_SERIES_VIEWER_STATUS.AWAITING_PRECONDITIONS,
  displayError: null,
  fetchProduct: {
...
const matchTimeStepForTableName = function (key) {
  ...
  return tableName.endsWith("_".concat(key)) || TIME_STEPS[key].matchFileTableSuffix.some(suffix => tableName.endsWith("_".concat(suffix))) || (0, _typeUtil.existsNonEmpty)(TIME_STEPS[key].matchFileTableIncludes) && TIME_STEPS[key].matchFileTableIncludes.some(testStr => tableName.includes(testStr)) || allowDefaultFallthrough;
};
const getTimeStepForTableName = function () {
  ...
  return Object.keys(TIME_STEPS).find(key => matchTimeStepForTableName(key, tableName)) || (useDefault ? DEFAULT_FALLBACK_TMI : null);
};
...
const checkDateTimeVariable = dateTimeVariable => {
  if (!PREFERRED_DATETIME_VARIABLES.includes(dateTimeVariable)) {
    // eslint-disable-next-line no-console
    console.debug("Determined datetime variable does not match known preferred: ".concat(dateTimeVariable));
  }
};
...
const determineAutoTimeStep = (availableTimeSteps, timeStepVariables, variables) => {
  ...
  let dateTimeVariableTimeSteps = Array.from(availableTimeSteps).filter(timeStep => (0, _typeUtil.exists)(timeStepVariables[timeStep]) && timeStepVariables[timeStep].dateTimeVariables.has(dateTimeVariable));
  if (dateTimeVariableTimeSteps.length <= 0) {
    ...
    // tables
    const hasDefault = (0, _typeUtil.exists)(timeStepVariables[DEFAULT_FALLBACK_TMI]) && timeStepVariables[DEFAULT_FALLBACK_TMI].dateTimeVariables.has(dateTimeVariable);
    if (hasDefault) {
...
const reducer = (state, action) => {
  ...
  switch (action.type) {
    ...
    case 'setLimitedReleaseState':
      newState.status = _constants.TIME_SERIES_VIEWER_STATUS.LOGIN_REQUIRED;
    ...
    case 'fetchSiteMonthSucceeded':
      ...
      newState.timeStep.availableTimeSteps = new Set([...state.timeStep.availableTimeSteps, ...parsedContent.availableTimeSteps]);
    ...
    case 'fetchSiteVariablesSucceeded':
      ...
      newState.timeStep = _extends({}, state.timeStep, {
        variables: _extends({}, state.timeStep.variables, parsedContent.timeStepVariables)
      });
...
const Provider = props => {
  ...
  */
  ...
  (0, _react.useEffect)(() => {
    ...
    if (viewerStatus !== _constants.TIME_SERIES_VIEWER_STATUS.AWAITING_PRECONDITIONS) {
      return;
    }
    dispatch({
      type: 'initialize',
  ...
  }, [dispatch, preconditionsSatisfied, viewerStatus, isViewerLimited, productCodeProp, productDataProp, releaseProp]);
  ...
  (0, _react.useEffect)(() => {
    ...
    if ((0, _typeUtil.isStringNonEmpty)((_state$product = state.product) === null || _state$product === void 0 ? void 0 : _state$product.productCode)) {
      reinitProductCode = state.product.productCode;
    } else if ((0, _typeUtil.isStringNonEmpty)(productCodeProp)) {
    ...
    } else if ((0, _typeUtil.isStringNonEmpty)(releaseProp)) {
    ...
    if ((0, _typeUtil.isStringNonEmpty)(reinitProductCode)) {
      dispatch({
        type: 'reinitializeFromLimited',
        productCode: reinitProductCode,
  ...
  }, [dispatch, preconditionsSatisfied, viewerStatus, isViewerLimited, stateIsViewerLimitedMode, modeProp, productCodeProp, productDataProp, releaseProp, state.product.productCode, state.release]);
  ...
  }, [state.mode, productCodeProp, releaseProp, state.product.productCode, state.release, viewerStatus, preconditionsSatisfied, isViewerLimited, stateIsViewerLimitedMode, dispatch]);
  ...
  */
  (0, _react.useEffect)(() => {
    ...
    if (isViewerLimited && (0, _typeUtil.isStringNonEmpty)(state.release)) {
    ...
    if (isViewerLimited) {
      graphQLObservable = _NeonGraphQL.default.getDemoDataProductByCode(state.product.productCode, true);
    } else {
      graphQLObservable = _NeonGraphQL.default.getDataProductByCode(state.product.productCode, state.release, true);
    }
    dispatch({
      type: 'initFetchProductCalled'
    });
    graphQLObservable.pipe((0, _rxjs.map)(response => {
  ...
  }, [state.mode, state.status, state.fetchProduct.status, state.product.productCode, state.release, isViewerLimited]);
  ...
  */
  (0, _react.useEffect)(() => {
    ...
    const getSiteMonthDataURL = (siteCode, month) => {
      const root = isViewerLimited ? _NeonEnvironment.default.getFullApiPath('demoData') : _NeonEnvironment.default.getFullApiPath('data');
      const hasRelease = state.release && typeof state.release === 'string' && state.release.length > 0;
      const releaseParam = !isViewerLimited && hasRelease ? "?release=".concat(state.release) : '';
    ...
    const fetchNeededSiteMonths = (siteCode, fetches) => {
      continuousDateRange.filter(month => !fetches.siteMonths[month]).forEach(month => {
        ...
        const headers = _extends({}, neonContextSessionState.sessionHeaders);
        _NeonApi.default.getJsonObservable(getSiteMonthDataURL(siteCode, month), headers).pipe((0, _rxjs.map)(response => {
  ...
  }, [state.mode, state.status, state.selection, state.selection.digest, state.variables, state.product, state.release, preconditionsSatisfied, isViewerLimited, neonContextSessionState]);
