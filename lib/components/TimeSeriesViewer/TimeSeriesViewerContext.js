...
import { of, merge, map, tap, mergeMap, switchMap, catchError, ignoreElements } from 'rxjs';
...
const generateYAxisRange = (axis = {})=>{
    const { rangeMode, dataRange = [
        0,
        0
    ], standardDeviation = 0, precision = 0, axisRange } = axis;
...
const Context = /*#__PURE__*/ createContext(cloneDeep(DEFAULT_STATE));
...
const getTimeStep = (input = '')=>Object.keys(TIME_STEPS).find((key)=>TIME_STEPS[key].tmi === input) || null;
const matchTimeStepForTableName = (key, tableName = '', allowDefaultFallthrough = false)=>tableName.endsWith(`_${key}`) || TIME_STEPS[key].matchFileTableSuffix.some((suffix)=>tableName.endsWith(`_${suffix}`)) || existsNonEmpty(TIME_STEPS[key].matchFileTableIncludes) && TIME_STEPS[key].matchFileTableIncludes.some((testStr)=>tableName.includes(testStr)) || allowDefaultFallthrough;
const getTimeStepForTableName = (tableName = '', useDefault = false)=>Object.keys(TIME_STEPS).find((key)=>matchTimeStepForTableName(key, tableName)) || (useDefault ? DEFAULT_FALLBACK_TMI : null);
...
*/ const getUpdatedValueRange = (existingRange, newValue)=>{
    const arrayVal = Array.isArray(newValue) ? newValue : [
        newValue
    ];
 ...
 */ const determineDateTimeVariable = (variables, timeStep)=>{
    ...
    let dateTimeVars = Object.keys(variables).filter((v)=>variables[v].isDateTime && variables[v].timeSteps.has(timeStep));
    if (dateTimeVars.length <= 0) {
        ...
        dateTimeVars = Object.keys(variables).filter((v)=>variables[v].isDateTime && variables[v].timeSteps.has(DEFAULT_FALLBACK_TMI));
    }
    if (dateTimeVars.length > 0) {
 ...
 * @param {Object} variables
 * @returns
 */ const determineAutoTimeStep = (availableTimeSteps, timeStepVariables, variables)=>{
    ...
    let dateTimeVariableTimeSteps = Array.from(availableTimeSteps).filter((timeStep)=>exists(timeStepVariables[timeStep]) && timeStepVariables[timeStep].dateTimeVariables.has(dateTimeVariable));
    if (dateTimeVariableTimeSteps.length <= 0) {
        ...
        // tables
        const hasDefault = exists(timeStepVariables[DEFAULT_FALLBACK_TMI]) && timeStepVariables[DEFAULT_FALLBACK_TMI].dateTimeVariables.has(dateTimeVariable);
        if (hasDefault) {
    ...
    return Array.from(dateTimeVariableTimeSteps).reduce((acc, cur)=>{
        if (cur === 'auto') {
            return acc;
        }
        if (acc === null) {
 ...
 */ const parseSiteVariables = (previousVariables, siteCode, csv)=>{
    ...
    variables.data.filter((variable)=>validTables.has(variable.table) && !ignoreTables.has(variable.table)).forEach((variable, idx)=>{
        const { table, fieldName, description, dataType, units, downloadPkg } = variable;
 ...
 */ const applyDefaultsToSelection = (state, invalidDefaultVariables = new Set())=>{
    const { status, product, variables, selection, timeStep: stateTimeStep } = state;
    ...
    if (isAutoTimeStep && selection.autoTimeStep === null) {
        selection.autoTimeStep = determineAutoTimeStep(stateTimeStep.availableTimeSteps, stateTimeStep.variables, variables);
    }
            ...
            selection.sites.forEach((site)=>{
                ...
                positions.forEach((position)=>{
                    selection.continuousDateRange.forEach((month)=>{
                        ...
                        const timeStepTables = lodashGet(product.sites[siteCode].positions[position], `data['${month}']['${pkg}']['${dataTimeStep}']`, {});
    ...
    // month, or position.
    if (status === TIME_SERIES_VIEWER_STATUS.READY_FOR_SERIES && (!hasVariablesSelected || !foundVarWithData && selection.isDefault) && selection.variables.length && selection.yAxes.y1.dataRange.every((x)=>x === null)) {
        ...
        if (allowedDefaultVars.length > invalidDefaultVariables.size) {
            ...
            selection.variables = [];
            return applyDefaultsToSelection({
                ...state,
                selection
...
*/ const reducer = (state, action)=>{
    ...
    switch(action.type){
        case 'initialize':
            ...
            if (newState.mode !== VIEWER_MODE.STATIC && action.isViewerLimited) {
                ...
                newState.product.productCode = action.productDataProp ? action.productDataProp.productCode : action.productCodeProp;
            } else {
        ...
        case 'fetchSiteMonthFailed':
            if (!action.siteCode || !action.month || !state.metaFetches[`fetchSiteMonth.${action.siteCode}.${action.month}`]) {
                return state;
            }
        ...
        case 'fetchSiteMonthSucceeded':
            ...
            if (newState.timeStep.availableTimeSteps.size === 1) {
                return fail('This data product is not compatible with the Time Series Viewer ' + '(no valid time step found)');
            }
            calcSelection();
            if (newState.product.sites[action.siteCode].fetches.variables.status !== FETCH_STATUS.SUCCESS || newState.product.sites[action.siteCode].fetches.positions.status !== FETCH_STATUS.SUCCESS) {
                newState.status = TIME_SERIES_VIEWER_STATUS.LOADING_META;
            } else {
                calcStatus();
            }
        ...
        case 'selectVariables':
            ...
            /* eslint-disable prefer-destructuring */ if (parsedContent.selectedUnits.length === 1) {
            ...
            /* eslint-enable prefer-destructuring */ calcSelection();
            calcStatus();
            return newState;
    ...
    */ // Option Selection Actions
        ...
        case 'selectTimeStep':
            newState.selection.isDefault = false;
            if (!state.timeStep.availableTimeSteps.has(action.timeStep)) {
                return state;
            }
        ...
        case 'selectSitePositions':
            ...
            newState.selection.isDefault = false;
            newState.selection.sites[selectedSiteIdx].positions = [
                ...action.positions
            ];
            calcSelection();
            calcStatus();
...
*/ const Provider = (inProps)=>{
    ...
    const { mode: modeProp, productCode: productCodeProp, productData: productDataProp, release: releaseProp, children } = props;
    /**
  ...
  */ const neonContextSessionState = NeonContext.useNeonContextSessionState();
    ...
    const { status: viewerStatus, isViewerLimitedMode: stateIsViewerLimitedMode } = state;
    // Initialize the viewer once when the preconditions are satisfied
    useEffect(()=>{
        if (!preconditionsSatisfied) {
            return;
        }
        if (viewerStatus !== TIME_SERIES_VIEWER_STATUS.AWAITING_PRECONDITIONS) {
            return;
        }
        dispatch({
            type: 'initialize',
    ...
    useEffect(()=>{
        ...
        if (isViewerLimited === stateIsViewerLimitedMode) {
            return;
        }
  ...
  */ useEffect(()=>{
        ...
        if (isViewerLimited && isStringNonEmpty(state.release)) {
            ...
            dispatch({
                type: 'setLimitedReleaseState',
                message: 'Login required to view release data'
            });
            return;
        }
        let graphQLObservable;
        if (isViewerLimited) {
            graphQLObservable = NeonGraphQL.getDemoDataProductByCode(state.product.productCode, true);
        } else {
            graphQLObservable = NeonGraphQL.getDataProductByCode(state.product.productCode, state.release, true);
        }
        dispatch({
            type: 'initFetchProductCalled'
        });
        graphQLObservable.pipe(map((response)=>{
  ...
  */ useEffect(()=>{
        ...
        NeonApi.getReleasesObservable().pipe(map((response)=>{
  ...
  */ useEffect(()=>{
        ...
        const getSiteMonthDataURL = (siteCode, month)=>{
            const root = isViewerLimited ? NeonEnvironment.getFullApiPath('demoData') : NeonEnvironment.getFullApiPath('data');
            const hasRelease = state.release && typeof state.release === 'string' && state.release.length > 0;
            const releaseParam = !isViewerLimited && hasRelease ? `?release=${state.release}` : '';
        ...
        const fetchSiteVariables = (siteCode, url)=>{
            ...
            fetchCSV(url).pipe(map((response)=>{
                dispatch({
        ...
        const fetchNeededSiteMonths = (siteCode, fetches)=>{
            continuousDateRange.filter((month)=>!fetches.siteMonths[month]).forEach((month)=>{
                ...
                NeonApi.getJsonObservable(getSiteMonthDataURL(siteCode, month), headers).pipe(map((response)=>{
                ...
                positions.forEach((position)=>{
                    continuousDateRange.forEach((month)=>{
                        ...
                        Object.keys(timeStepTables).forEach((tableName)=>{
                            ...
                            dataFetches.push(fetchCSV(url).pipe(map((response)=>response.response), switchMap((csv)=>parseTimeSeriesData({
                                    csv,
        ...
        if (state.status === TIME_SERIES_VIEWER_STATUS.READY_FOR_DATA && !metaFetchTriggered) {
            if (!dataFetches.length) {
                ...
                forkJoinWithProgress(dataFetches).pipe(mergeMap(([finalResult, progress])=>merge(progress.pipe(tap((value)=>dispatch({
                            type: 'fetchDataFilesProgress',
                            token: masterFetchToken,
                            value
                        })), ignoreElements()), finalResult))).subscribe((value)=>dispatch({
