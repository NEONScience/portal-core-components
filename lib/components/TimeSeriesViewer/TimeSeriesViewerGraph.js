...
function TimeSeriesViewerGraph() {
  ...
  var buildTimeData = function buildTimeData() {
    continuousDateRange.forEach(function (month, idx) {
      ...
      state.product.sites[sites[0].siteCode].positions[sites[0].positions[0]].data[month].basic[timeScale].series.startDateTime.forEach(function (d) {
        data.push([d]);
      });
    });
      ...
      positions.forEach(function (position) {
        continuousDateRange.forEach(function (month, monthIdx) {
          variables.forEach(function (variable) {
            ...
            state.product.sites[siteCode].positions[position].data[month][downloadPkg][timeScale].series[variable].forEach(function (d, datumIdx) {
  ...
  */
  ...
  var renderPre = function renderPre() {
    return state.status === _TimeSeriesViewerContext.TIME_SERIES_VIEWER_STATUS.READY ? _react.default.createElement("pre", {
