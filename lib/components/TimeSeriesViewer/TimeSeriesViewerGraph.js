...
function TimeSeriesViewerGraph() {
      ...
      positions.forEach(function (position) {
        ...
        continuousDateRange.forEach(function (month) {
          ...
          variables.forEach(function (variable) {
            ...
            if (!posData[month] || !posData[month][pkg] || !posData[month][pkg][timeStep] || !posData[month][pkg][timeStep].series[variable]) {
            ...
            if (seriesStepCount !== monthStepCount) {
              ...
              var setSeriesValueByTimestamp = function setSeriesValueByTimestamp(t) {
                var timestamp = _moment.default.utc(qualityData[t][0]).format('YYYY-MM-DD[T]HH:mm:ss[Z]');

                var dataIdx = posData[month][pkg][timeStep].series.startDateTime.findIndex(function (startDateTime) {
  ...
  }, [selectionDigest, state.status, data, graphOptions, dygraphRef, legendRef, axisCountChangedRef]); // Effect to register a click handler to the legend for sereis visibility toggling
