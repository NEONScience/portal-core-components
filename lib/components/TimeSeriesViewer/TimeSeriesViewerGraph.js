...
function TimeSeriesViewerGraph() {
  ...
  var timeStep = selectedTimeStep === 'auto' ? autoTimeStep : selectedTimeStep; // let data = cloneDeep(NULL_DATA);
  ...
  var buildTimeData = function buildTimeData() {
    ...
    var newQualityData = [];
    var newMonthOffsets = {};
    var newTimestampMap = {}; // Sanity check: must have a valid time step

    if (!_TimeSeriesViewerContext.TIME_STEPS[timeStep]) {
      return _extends(_extends({}, graphData), {}, {
    ...
    return _extends(_extends({}, graphData), {}, {
      ...
      data: newData.length === 0 ? (0, _cloneDeep.default)(NULL_DATA) : newData,
  ...
  var buildSeriesData = function buildSeriesData() {
    var graphData = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
    var newData = graphData.data,
        newQualityData = graphData.qualityData,
        newMonthOffsets = graphData.monthOffsets; // Reinitialize

    var newSeries = [];
    var newLabels = ['x'];
    var newQualityLabels = ['start', 'end']; // Sanity check: must have a valid dateTimeVariable
        ...
        continuousDateRange.forEach(function (month) {
          ...
          variables.forEach(function (variable) {
            ...
            var setSeriesValueByTimestamp = function setSeriesValueByTimestamp(t) {
              var isodate = _moment.default.utc(newData[t][0]).format('YYYY-MM-DD[T]HH:mm:ss[Z]');
          ...
          qualityFlags.forEach(function (qf, qfIdx) {
            ...
            if (seriesStepCount !== monthStepCount) {
              ...
              var setQualityValueByTimestamp = function setQualityValueByTimestamp(t) {
                var isodate = _moment.default.utc(newQualityData[t][0]).format('YYYY-MM-DD[T]HH:mm:ss[Z]');
                ...
                newQualityData[t][columnIdx] = qfIdx ? [].concat(_toConsumableArray(newQualityData[t][columnIdx]), [d]) : [d];
              };
  ...
  var legendFormatter = function legendFormatter(graphData) {
    var _state$graphData = state.graphData,
        series = _state$graphData.series,
        qualityData = _state$graphData.qualityData,
        timestampMap = _state$graphData.timestampMap,
        qualityLabels = _state$graphData.qualityLabels; // Series
  ...
  var renderQualityFlags = function renderQualityFlags(canvas, area, g) {
    var _state$graphData2 = state.graphData,
        ...
        qualityLabels = _state$graphData2.qualityLabels;
  ...
  (0, _react.useEffect)(function () {
    ...
    dispatch({
      ...
      graphData: buildSeriesData(buildTimeData(_extends({}, state.graphData)))
    });
  });
  ...
  if (state.status === _TimeSeriesViewerContext.TIME_SERIES_VIEWER_STATUS.READY) {
    ...
    var _state$graphData3 = state.graphData,
        _series = _state$graphData3.series,
        labels = _state$graphData3.labels;
    graphOptions = _extends(_extends({}, (0, _cloneDeep.default)(BASE_GRAPH_OPTIONS)), {}, {
      ...
      visibility: _series.map(function (s) {
