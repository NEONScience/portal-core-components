...
require('dygraphs/src/extras/shapes.js'); // eslint-disable-next-line import/extensions
...
function TimeSeriesViewerGraph() {
  ...
  }, [state.status, dygraphRef, legendRef, graphInnerContainerRef, downloadRef, graphState.pngDimensions, graphDispatch]); // Layout effect to keep the graph dimensions in-line with resize events
  ...
  (0, _react.useEffect)(function () {
    ...
    handleResize();
  }, [selectionDigest, state.status, state.graphData.data, graphOptions, dygraphRef, handleResize, axisCountChangedRef]); // Effect to register a click handler to the legend for series visibility toggling
  ...
  var buildGraphTitle = function buildGraphTitle() {
    var releaseTitle = (0, _typeUtil.isStringNonEmpty)(state.release) ? ", ".concat(state.release, " ") : '';
  ...
  var renderCitation = function renderCitation() {
