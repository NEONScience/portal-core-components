...
var YAxisRangeOption = function YAxisRangeOption(props) {
  ...
  var _state$selection2 = state.selection,
    ...
    standardDeviation = _state$selection2$yAx.standardDeviation;
  ...
  var customMax = Math.max(parseFloat(Math.max(axisRange[1], (dataRange[1] || 0) + customPad).toFixed(precision), 10), 0);
...
var RollPeriodOption = function RollPeriodOption() {
  ...
  var rollMin = 1;
  var rollMax = Math.floor(Math.max(dateRangeMonths * rollStepsPerMonth, currentRollPeriod) / 4);
