...
var YAxisRangeOption = function YAxisRangeOption(props) {
  ...
  var _state$selection$yAxe = state.selection.yAxes[axis],
      units = _state$selection$yAxe.units,
      dataRange = _state$selection$yAxe.dataRange,
      rangeMode = _state$selection$yAxe.rangeMode,
      axisRange = _state$selection$yAxe.axisRange,
      precision = _state$selection$yAxe.precision,
      standardDeviation = _state$selection$yAxe.standardDeviation;
  ...
  var customPad = Math.max((dataRange[1] || 0) * 0.25, standardDeviation * 2);
  var customMin = 0;
  var customMax = parseFloat(Math.max(axisRange[1], (dataRange[1] || 0) + customPad).toFixed(precision), 10); // Determine slider marks

  var marks = [customMin, (customMax - customMin) * 0.25, (customMax - customMin) * 0.5, (customMax - customMin) * 0.75, customMax].map(function (m) {
    return {
      value: m,
      label: m.toFixed(precision)
    };
  });
  var step = Math.pow(10, -1 * precision);
  return !render ? /*#__PURE__*/_react.default.createElement("div", {
    ...
    return /*#__PURE__*/_react.default.createElement(_ToggleButton.default, {
      ...
      title: _TimeSeriesViewerContext.Y_AXIS_RANGE_MODE_DETAILS[key].name
    ...
    onChange: function onChange(event) {
      var range = [axisRange[0], Math.min(event.target.value, customMax)];
      dispatch({
    ...
    className: classes.yAxisRangeTextField,
    onChange: function onChange(event) {
      var range = [Math.max(event.target.value, customMin), axisRange[1]];
      dispatch({
        type: 'selectYAxisCustomRange',
        axis: axis,
        range: range
    ...
    onChange: function onChange(event, values) {
      dispatch({
        type: 'selectYAxisCustomRange',
        axis: axis,
        range: [Math.min(Math.max(values[0], customMin), customMax), Math.min(Math.max(values[1], customMin), customMax)]
      });
    }
  })));
};
