...
const useNeonContextSessionState = ()=>{
    const [{ isFinal, auth: { isAuthenticated, userData } }] = useNeonContextState();
    ...
    if (appliedIsAuthenticated && exists(userData) && exists(userData.data)) {
        ...
        accountValidationSteps = existsNonEmpty(userData.data.accountValidationSteps) ? userData.data.accountValidationSteps : [];
    }
    if (!isFinal) {
        return {
            enabled: true,
...
*/ const reducer = (state, action)=>{
    ...
    const newState = {
        ...state,
        fetches: cloneDeep(state.fetches)
    };
...
*/ const Provider = (inProps)=>{
    ...
    const { children, fetchPartials, useCoreAuth, whenFinal } = props;
    ...
    const { isFinal, whenFinalCalled, auth: { isAuthenticated: stateIsAuthenticated } } = state;
    useEffect(()=>{
        ...
        return ()=>{
            BroadcastChannelService.removeAuthChannelMessageEventListener(authBroadcastChannelMessageHandler);
        };
    }, [
        dispatch,
    ...
    const fetchPartialHTML = (key)=>{
        if (!Object.keys(REMOTE_ASSETS).includes(key)) {
            return;
        }
    ...
    const fetchMethods = {
        sites: ()=>{
            NeonGraphQL.getAllSites().pipe(map((response)=>{
        ...
        bundles: ()=>{
            NeonApi.getProductBundlesObservable().pipe(map((response)=>{
        ...
        auth: ()=>{
            ...
            AuthService.fetchUserInfo((response)=>{
