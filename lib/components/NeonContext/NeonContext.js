...
var determineContextFetchFinal = function determineContextFetchFinal(state) {
  var authFinal = !state.auth.useCore || state.fetches.auth.status === FETCH_STATUS.SUCCESS || state.fetches.auth.status === FETCH_STATUS.ERROR;
  var sitesFinal = state.fetches.sites.status === FETCH_STATUS.SUCCESS || state.fetches.sites.status === FETCH_STATUS.ERROR;
  var bundlesFinal = state.fetches.bundles.status === FETCH_STATUS.SUCCESS || state.fetches.bundles.status === FETCH_STATUS.ERROR;
...
var Provider = function Provider(props) {
  ...
  var isFinal = state.isFinal,
    whenFinalCalled = state.whenFinalCalled;
  ...
  var fetchPartialHTML = function fetchPartialHTML(key) {
    ...
    (0, _ajax.ajax)({
      ...
      method: 'GET',
      responseType: 'text'
    }).pipe((0, _rxjs.map)(function (response) {
      dispatch({
  ...
  var fetchMethods = (_fetchMethods = {
    ...
    bundles: function bundles() {
      _NeonApi.default.getProductBundlesObservable().pipe((0, _rxjs.map)(function (response) {
        var bundles = _BundleParser.default.parseBundlesResponse(response);
        ...
        var context = _BundleParser.default.parseContext(bundles);
        dispatch({
