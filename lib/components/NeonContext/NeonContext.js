...
var _BroadcastChannelService = _interopRequireDefault(require("../../service/BroadcastChannelService"));
...
var _aopDataProducts = _interopRequireDefault(require("../../staticJSON/aopDataProducts.json"));
var _saeDataProducts = _interopRequireDefault(require("../../staticJSON/saeDataProducts.json"));
...
const DEFAULT_STATE = {
  data: {
    ...
    aopDataProducts: _aopDataProducts.default,
    saeDataProducts: _saeDataProducts.default,
    stateSites: {},
...
const useNeonContextSessionState = () => {
  ...
  if (_NeonEnvironment.default.sessionDisable) {
    return {
      enabled: false,
      ready: true,
      authenticated: true,
  ...
  if (appliedIsAuthenticated && (0, _typeUtil.exists)(userData) && (0, _typeUtil.exists)(userData.data)) {
    token = userData.token;
    canAccessData = userData.data.canAccessData === true;
    accountValidated = userData.data.accountValidated === true;
    accountValidationSteps = (0, _typeUtil.existsNonEmpty)(userData.data.accountValidationSteps) ? userData.data.accountValidationSteps : [];
  }
  if (!isFinal) {
    return {
      enabled: true,
  ...
  if ((0, _typeUtil.isStringNonEmpty)(appliedToken)) {
    const sessionHeaderName = _NeonEnvironment.default.getApiSessionTokenHeader();
    if ((0, _typeUtil.isStringNonEmpty)(sessionHeaderName)) {
      return {
        enabled: true,
        ready: true,
        authenticated: appliedIsAuthenticated,
  ...
  (0, _react.useEffect)(() => {
    const authBroadcastChannelMessageHandler = e => {
      if (stateIsAuthenticated) {
        if ((0, _typeUtil.exists)(e) && (0, _typeUtil.exists)(e.data) && (0, _typeUtil.isStringNonEmpty)(e.data.event)) {
          if (e.data.event === 'account-data-changed') {
            _AuthService.default.handleLoginMessageFromBroadcastChannel(dispatch);
          }
        }
        return;
      }
    ...
    _BroadcastChannelService.default.addAuthChannelMessageEventListener(authBroadcastChannelMessageHandler);
    return () => {
      _BroadcastChannelService.default.removeAuthChannelMessageEventListener(authBroadcastChannelMessageHandler);
    };
  ...
  const fetchMethods = {
    ...
    auth: () => {
      _AuthService.default.fetchUserInfo(response => {
        ...
        if (isAuthenticated) {
          _BroadcastChannelService.default.sendLoginMessage();
        }
