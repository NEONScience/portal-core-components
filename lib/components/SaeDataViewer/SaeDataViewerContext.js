    ...
    switch(action.type){
        ...
        case 'fetchSaeProductsSucceeded':
            ...
            action.products.forEach((productData)=>{
                newState.controlsState.saeDataProducts.forEach((saeDataProduct)=>{
        ...
        case 'setSelectedProduct':
            ...
            if (!exists(SAE_DATA_PRODUCT_MAP[coercedProductName])) {
                const defaultProduct = newState.controlsState.saeDataProducts.find((checkSaeDataProduct)=>checkSaeDataProduct.name.localeCompare(DEFAULT_SAE_PRODUCT.name) === 0);
