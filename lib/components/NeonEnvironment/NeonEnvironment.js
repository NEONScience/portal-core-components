/* eslint-disable prefer-regex-literals */ import { AuthSilentType } from '../../types/core';
// Default hosts
...
export const HostRegexService = {
    getApiHostRegex: ()=>new RegExp(/^(data|cert-data|int-data|local-data)[.]neonscience[.]org$/),
    getWebHostRegex: ()=>// eslint-disable-next-line max-len, @stylistic/max-len
...
const NeonEnvironment = {
    ...
    getVizApiPath: {
        saeBokehPlot: ()=>'/sae/bokeh-plot',
        saeDemoBokehPlot: ()=>'/sae/demo/bokeh-plot'
    },
    getApiLdPath: {
        repo: ()=>'/repository'
    },
    ...
    getAopGEEDesktopUrl: ()=>process.env.NEXT_PUBLIC_NEON_AOP_GEE_DATA_VIEWER_DESKTOP,
    getAopGEEMobileUrl: ()=>process.env.NEXT_PUBLIC_NEON_AOP_GEE_DATA_VIEWER_MOBILE,
    getAopGEEVideoUrl: ()=>process.env.NEXT_PUBLIC_NEON_AOP_GEE_DATA_VIEWER_VIDEO,
    getApiHostOverride: ()=>process.env.NEXT_PUBLIC_NEON_API_HOST_OVERRIDE || DEFAULT_API_HOST,
    getBioRepoHostOverride: ()=>process.env.NEXT_PUBLIC_BIOREPO_HOST_OVERRIDE || DEFAULT_BIOREPO_HOST,
    ...
    route: {
        ...
        buildAccountRoute: ()=>`${NeonEnvironment.getApiHost()}${NeonEnvironment.route.account()}`
    },
    /**
   ...
   */ getNeonServerData: ()=>{
    ...
    getNeonServerDataWebHost: ()=>{
        ...
        if (serverData && typeof serverData.NeonWebHost === 'string') {
            ...
            try {
                ...
                console.error('Failed to parse web host as URL', [
                    e
                ]);
            }
        }
    ...
    getNeonServerDataApiHost: ()=>{
        ...
        if (serverData && typeof serverData.NeonPublicAPIHost === 'string') {
            ...
            try {
                ...
                // eslint-disable-next-line no-console
                console.error('Failed to parse API host as URL', [
                    e
                ]);
            }
        }
    ...
    getNeonServerDataBioRepoHost: ()=>{
        ...
        if (serverData && typeof serverData.NeonBioRepoHost === 'string') {
            ...
            try {
                ...
                // eslint-disable-next-line no-console
                console.error('Failed to parse Biorepo host as URL', [
                    e
                ]);
            }
        }
    ...
    getWebHost: ()=>{
        ...
        /* eslint-enable */ if (NeonEnvironment.isWebHostValid(window.location.host)) {
    ...
    getApiHost: ()=>{
        ...
        /* eslint-enable */ if (NeonEnvironment.isApiHostValid(window.location.host)) {
            return `${window.location.protocol}//${window.location.host}`;
        }
        return DEFAULT_API_HOST;
    },
    getBioRepoHost: ()=>{
        ...
        /* eslint-enable */ if (NeonEnvironment.isBioRepoHostValid(window.location.host)) {
            return `${window.location.protocol}//${window.location.host}`;
        }
        return DEFAULT_BIOREPO_HOST;
    },
   ...
   */ getAuthSilentType: ()=>{
    ...
    getFullApiPath: (path = '')=>{
        ...
        const root = [
            'download',
            'manifest'
        ].includes(path) ? '' : NeonEnvironment.getRootApiPath();
    ...
    getFullVizApiPath: (path = '')=>{
        const host = NeonEnvironment.getApiHost();
        const root = NeonEnvironment.getRootVizApiPath();
        return NeonEnvironment.getVizApiPath[path] ? `${host}${root}${NeonEnvironment.getVizApiPath[path]()}` : `${host}${root}`;
    },
