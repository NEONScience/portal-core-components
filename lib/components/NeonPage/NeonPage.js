...
var NeonPage = function NeonPage(props) {
  var breadcrumbHomeHref = props.breadcrumbHomeHref,
    ...
    showHeaderSkeleton = props.showHeaderSkeleton,
    showFooterSkeleton = props.showFooterSkeleton,
    error = props.error,
  ...
  */
  ...
  var useSomeDrupalAssets = _NeonEnvironment.default.fetchDrupalAssets && !(customHeader && customFooter);
  ...
  */
  ...
  (0, _react.useEffect)(function () {
    ...
    fetch(_remoteAssetsMap.default[DRUPAL_THEME_CSS].url).then(function (response) {
      if (!response.ok) {
        throw new Error('Failed to fetch Drupal theme CSS');
      }
      return response.text();
    }).then(function (data) {
      var drupalStyle = document.createElement('style');
      var appliedData = _DrupalAssetService.default.cleanCss(data, true);
      drupalStyle.textContent = appliedData;
      document.head.appendChild(drupalStyle);
      try {
        var existingBlock = document.head.querySelector('link[data-meta="drupal-theme"]');
