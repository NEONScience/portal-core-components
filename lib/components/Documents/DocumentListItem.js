...
var useStyles = function useStyles(enableDownloadButton, atComponentXs) {
  var listItemPaddingRight = 'unset';

  if (!atComponentXs) {
    listItemPaddingRight = enableDownloadButton ? '192px' : 'unset';
  }
...
var DocumentListItem = function DocumentListItem(props) {
  ...
  var classes = useStyles(enableDownloadButton === true, atComponentXs)(_Theme.default); // eslint-disable-next-line max-len
  ...
  var handleResizeCb = (0, _react.useCallback)(function () {
    var container = containerRef.current; // Do nothing if either container or viz references fail ot point to a DOM node
