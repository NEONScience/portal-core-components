 ...
 */ const NeonJsonLd = {
   ...
   * @return The RxJS AJAX Observable.
   */ getRepoJsonLdObservable: ()=>NeonJsonLd.getJsonLdObservable(NeonEnvironment.getFullJsonLdApiPath('repo')),
    /**
   ...
   */ getProductJsonLdObservable: (productCode, release)=>{
        ...
        if (hasRelease) {
            return NeonJsonLd.getJsonLdObservable(`${NeonEnvironment.getFullJsonLdApiPath('products')}/${productCode}?release=${release}`);
        }
        return NeonJsonLd.getJsonLdObservable(`${NeonEnvironment.getFullJsonLdApiPath('products')}/${productCode}`);
    },
    /**
   ...
   */ getJsonLdWithInjection: (url, injectReleaseMeta = false)=>{
        const observable = NeonApi.getJsonObservable(url).pipe(map((response)=>NeonJsonLd.inject(response, injectReleaseMeta)), catchError((err)=>{
   ...
   */ injectRepo: ()=>NeonJsonLd.getJsonLdWithInjection(NeonEnvironment.getFullJsonLdApiPath('repo')),
    /**
   ...
   */ injectProduct: (productCode, release, injectReleaseMeta = false, onNotExistsOnly = false)=>{
        ...
        const hasRelease = typeof release !== 'undefined' && release !== null && typeof release === 'string' && release.length > 0;
        if (hasRelease) {
            return NeonJsonLd.getJsonLdWithInjection(`${NeonEnvironment.getFullJsonLdApiPath('products')}/${productCode}?release=${release}`, injectReleaseMeta);
        }
        NeonJsonLd.removeReleaseMeta();
        return NeonJsonLd.getJsonLdWithInjection(`${NeonEnvironment.getFullJsonLdApiPath('products')}/${productCode}`);
    },
    /**
   ...
   */ injectPrototypeDataset: (uuid, onNotExistsOnly = false)=>{
        ...
        return NeonJsonLd.getJsonLdWithInjection(`${NeonEnvironment.getFullJsonLdApiPath('prototype')}/datasets/${uuid}`, true);
    }
};
