...
var NeonJsonLd = {
  ...
  injectReleaseMeta: function injectReleaseMeta(data) {
    var metadataUseMultiParent = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
    if (!data) return;
    try {
      var metadataObjects = [];
      ...
      metadataObjects.forEach(function (metadataObject) {
        var doiUrl = new URL(metadataObject.identifier);
        ...
        if (typeof metadataObject.name === 'string' && metadataObject.name.length > 0) {
          if (metadataObject.name.indexOf(NeonJsonLd.CITATION_AUTHOR) >= 0) {
            var titleContent = metadataObject.name.replace(NeonJsonLd.CITATION_AUTHOR, '').trim();
   ...
   */
  removeAllMetadata: function removeAllMetadata() {
    NeonJsonLd.removeJsonLdScript();
    NeonJsonLd.removeReleaseMeta();
  },
  /**
   ...
   */
  removeJsonLdScript: function removeJsonLdScript() {
    try {
  ...
  removeReleaseMeta: function removeReleaseMeta() {
    try {
      var currentDoiBlocks = document.head.querySelectorAll('meta[name="citation_doi"]');
      if (typeof currentDoiBlocks !== 'undefined' && currentDoiBlocks !== null) {
        currentDoiBlocks.forEach(function (currentDoiBlock) {
          return currentDoiBlock === null || currentDoiBlock === void 0 ? void 0 : currentDoiBlock.remove();
        });
      }
      var currentTitleBlocks = document.head.querySelectorAll('meta[name="citation_title"]');
      if (typeof currentTitleBlocks !== 'undefined' && currentTitleBlocks !== null) {
        currentTitleBlocks.forEach(function (currentTitleBlock) {
          return currentTitleBlock === null || currentTitleBlock === void 0 ? void 0 : currentTitleBlock.remove();
        });
      }
      var currentAuthorBlocks = document.head.querySelectorAll('meta[name="citation_author"]');
      if (typeof currentAuthorBlocks !== 'undefined' && currentAuthorBlocks !== null) {
        currentAuthorBlocks.forEach(function (currentAuthorBlock) {
          return currentAuthorBlock === null || currentAuthorBlock === void 0 ? void 0 : currentAuthorBlock.remove();
        });
      }
    } catch (e) {
      console.error(e); // eslint-disable-line no-console
  ...
  getJsonLdWithInjection: function getJsonLdWithInjection(url) {
    var injectReleaseMeta = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
    var observable = _NeonApi.default.getJsonObservable(url).pipe((0, _rxjs.map)(function (response) {
      return NeonJsonLd.inject(response, injectReleaseMeta);
    }), (0, _rxjs.catchError)(function (err) {
   ...
   * @param {boolean} onNotExistsOnly Inject only if JSON-LD is not already injected
   */
  injectPrototypeDataset: function injectPrototypeDataset(uuid) {
    var onNotExistsOnly = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
    var shouldFetch = true;
    if (onNotExistsOnly) {
      try {
        var existingBlock = document.head.querySelector('script[type="application/ld+json"]');
