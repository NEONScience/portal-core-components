...
const NeonJsonLd = {
  ...
  getProductJsonLdObservable: (productCode, release) => {
    ...
    if (hasRelease) {
      return NeonJsonLd.getJsonLdObservable(`${_NeonEnvironment.default.getFullJsonLdApiPath('products')}/${productCode}?release=${release}`);
    }
    return NeonJsonLd.getJsonLdObservable(`${_NeonEnvironment.default.getFullJsonLdApiPath('products')}/${productCode}`);
  },
  /**
  ...
  injectProduct: function (productCode, release) {
    ...
    const hasRelease = typeof release !== 'undefined' && release !== null && typeof release === 'string' && release.length > 0;
    if (hasRelease) {
      return NeonJsonLd.getJsonLdWithInjection(`${_NeonEnvironment.default.getFullJsonLdApiPath('products')}/${productCode}?release=${release}`, injectReleaseMeta);
    }
    NeonJsonLd.removeReleaseMeta();
    return NeonJsonLd.getJsonLdWithInjection(`${_NeonEnvironment.default.getFullJsonLdApiPath('products')}/${productCode}`);
  },
  /**
  ...
  injectPrototypeDataset: function (uuid) {
    ...
    return NeonJsonLd.getJsonLdWithInjection(`${_NeonEnvironment.default.getFullJsonLdApiPath('prototype')}/datasets/${uuid}`, true);
  }
};
