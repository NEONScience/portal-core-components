...
exports.getJsonObservable = exports.getJson = exports.forkJoinWithProgress = exports.default = void 0;
...
const getJsonObservable = function (url) {
  ...
  let cors = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;
  const request = getRequest(url, headers, cors);
  const rxObs$ = (0, _ajax.ajax)(request).pipe((0, _rxjs.catchError)(error => {
    console.error(error); // eslint-disable-line no-console
    return (0, _rxjs.of)(error);
  }));
  return rxObs$;
};
exports.getJsonObservable = getJsonObservable;