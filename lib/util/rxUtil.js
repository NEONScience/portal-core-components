import { of, defer, forkJoin, map, tap, catchError, takeUntil, finalize, Subject } from 'rxjs';
 ...
 */ export const getJson = (url, callback, errorCallback, cancellationSubject$, headers = undefined, cors = false)=>{
    const request = getRequest(url, headers, cors);
    const rxObs$ = ajax(request).pipe(map((response)=>{
        const appliedResponse = exists(response) && exists(response.response) ? response.response : response;
 ...
 */ export const forkJoinWithProgress = (arrayOfObservables)=>defer(()=>{
        ...
        const modifiedObservablesList = arrayOfObservables.map((item)=>item.pipe(finalize(()=>{
                counter += 1;
                const percentValue = counter * 100 / arrayOfObservables.length;
                percent$.next(percentValue);
            })));
        const finalResult$ = forkJoin(modifiedObservablesList).pipe(tap(()=>{
            percent$.next(100);
            percent$.complete();
        }));
        return of([
 ...
 */ export const getJsonObservable = (url, headers = undefined, cors = false)=>{
    const request = getRequest(url, headers, cors);
    const rxObs$ = ajax(request).pipe(catchError((error)=>{
        console.error(error); // eslint-disable-line no-console
        return of(error);
    }));
    return rxObs$;
