...
var getJson = function getJson(url, callback, errorCallback, cancellationSubject$) {
  ...
  var rxObs$ = (0, _ajax.ajax)(request).pipe((0, _rxjs.map)(function (response) {
  ...
  }), (0, _rxjs.takeUntil)(cancellationSubject$)); // Placeholders for subscriber events, handled upstream in observable
...
var forkJoinWithProgress = function forkJoinWithProgress(arrayOfObservables) {
  return (0, _rxjs.defer)(function () {
    ...
    var modifiedObservablesList = arrayOfObservables.map(function (item) {
      return item.pipe((0, _rxjs.finalize)(function () {
        counter += 1;
        var percentValue = counter * 100 / arrayOfObservables.length;
        percent$.next(percentValue);
      }));
    });
    var finalResult$ = (0, _rxjs.forkJoin)(modifiedObservablesList).pipe((0, _rxjs.tap)(function () {
      percent$.next(100);
      percent$.complete();
    }));
