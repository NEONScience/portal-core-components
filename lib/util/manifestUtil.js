...
exports.default = exports.getSizeEstimateFromManifestRollupResponse = exports.getSizeEstimateFromManifestResponse = exports.formatBytes = exports.DOWNLOAD_SIZE_WARN = exports.MAX_POST_BODY_SIZE = exports.downloadAopManifest = exports.downloadManifest = exports.buildS3FilesRequestUrl = exports.buildManifestRequestBody = exports.buildManifestRequestUrl = exports.buildBundledProductsParams = exports.buildSiteCodesParams = exports.buildManifestConfig = void 0;
...
var buildBundledProductsParams = function buildBundledProductsParams() {
  var bundledProducts = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : new Array();

  if (bundledProducts.length <= 0) {
    return '';
  }

  return bundledProducts.reduce(function (bpString, bp, index) {
    return "".concat(bpString).concat(index === 0 ? '' : '&', "bp=").concat(bp);
  }, '');
};

exports.buildBundledProductsParams = buildBundledProductsParams;
...
var buildManifestRequestUrl = function buildManifestRequestUrl(config) {
  ...
  if (!useBody) {
    ...
    var bundledProductsParam = buildBundledProductsParams(bundledProducts);
    var productCodeParam = productCode.startsWith('NEON.DOM.SITE') ? productCode : "NEON.DOM.SITE.".concat(productCode);
    var params = ["dpcode=".concat(productCodeParam), "startdate=".concat(dateRange[0]), "enddate=".concat(dateRange[1]), "pkgtype=".concat(packageType), "includedocs=".concat(documentation ? 'true' : 'false'), siteCodesParam, bundledProductsParam];
    url = "".concat(url, "?").concat(params.filter(function (s) {
