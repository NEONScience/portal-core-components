...
var buildManifestRequestUrl = function buildManifestRequestUrl(config) {
  ...
  var productCode = config.productCode,
    ...
    provisionalData = config.provisionalData;
  ...
  if (!useBody) {
    ...
    var params = ["dpcode=".concat(productCodeParam), "startdate=".concat(dateRange[0]), "enddate=".concat(dateRange[1]), "pkgtype=".concat(packageType), "includedocs=".concat(documentation ? 'true' : 'false'), "includeProvisional=".concat(provisionalData ? 'true' : 'false'), siteCodesParam];
...
var downloadAopManifest = function downloadAopManifest(config, s3Files) {
  ...
  var provisionalData = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : 'exclude';
  var siteCodes = [];
  ...
  var includeProvisional = provisionalData === 'include';
