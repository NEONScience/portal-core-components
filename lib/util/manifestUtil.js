...
const buildManifestConfig = (selection, defaultPackageType = 'basic', isAop = false) => {
  const config = {
    productCode: '',
    release: '',
    sites: [],
...
const buildSiteCodesParams = (sites = new Array(), camelCase = false) => {
...
const buildManifestRequestUrl = (config, useBody = true) => {
  const {
    productCode,
    sites,
    dateRange,
  ...
  if (!useBody) {
    ...
    const params = [`dpcode=${productCodeParam}`, `startdate=${dateRange[0]}`, `enddate=${dateRange[1]}`, `pkgtype=${packageType}`, `includedocs=${documentation ? 'true' : 'false'}`, `includeProvisional=${provisionalData ? 'true' : 'false'}`, siteCodesParam];
    url = `${url}?${params.join('&')}`;
  }
  return url;
};
...
const buildS3FilesRequestUrl = (productCode, site, yearMonth, release) => {
  ...
  const root = `${_NeonEnvironment.default.getFullApiPath('data')}/${productCodeDir}/${site}/${yearMonth}`;
...
const downloadAopManifest = (config, s3Files, documentation = 'include', provisionalData = 'exclude') => {
  const siteCodes = [];
