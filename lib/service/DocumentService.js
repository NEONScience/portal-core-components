...
var _Tv = _interopRequireDefault(require("@mui/icons-material/Tv"));
...
const documentTypes = {
  ...
  image: {
    ...
    title: type => (0, _typeUtil.isStringNonEmpty)(type) ? `Image (${(type.match(/\/(.*)$/) || [])[1] || 'unknown type'})` : 'unknown type',
    Icon: _Photo.default
  },
  csv: {
...
const DocumentService = {
  ...
  downloadDocument: (document, onSuccessCb, onErrorCb) => {
    const apiPath = DocumentService.isQuickStartGuide(document) ? `${_NeonEnvironment.default.getFullApiPath('quickStartGuides')}/${document.name}` : `${_NeonEnvironment.default.getFullApiPath('documents')}/${document.name}`;
    fetch(apiPath, {
      method: 'HEAD'
    }).then(response => {
      if (!response.ok) {
