...
export var CitationDownloadType = /*#__PURE__*/ function(CitationDownloadType) {
...
ER  - `;
    ...
    getDataProductFormats: ()=>DataCiteService.getCitationFormats().filter((value)=>value.applicableDownloadtypes.includes("DATA_PRODUCT")),
    getPrototypeDatasetFormats: ()=>DataCiteService.getCitationFormats().filter((value)=>value.applicableDownloadtypes.includes("PROTOTYPE_DATASET")),
    downloadCitation: (formatShortName, type, target, doi, release, onSuccessCb, onErrorCb)=>{
        const useProvisional = release === PROVISIONAL_RELEASE;
        ...
        let fileName = '';
        const appliedRelease = isStringNonEmpty(release) ? release : PROVISIONAL_RELEASE;
        switch(type){
            case "PROTOTYPE_DATASET":
