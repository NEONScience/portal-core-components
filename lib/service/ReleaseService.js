...
var ReleaseService = {
  ...
  isNonRelease: function isNonRelease(releaseTag) {
    // eslint-disable-next-line prefer-regex-literals
    var regexLatestProv = new RegExp("^".concat(LATEST_AND_PROVISIONAL, "$"), 'i');
    // eslint-disable-next-line prefer-regex-literals
    var regexProv = new RegExp("^".concat(PROVISIONAL_RELEASE, "$"), 'i');
    var matchesLatestProv = regexLatestProv.exec(releaseTag);
    var matchesProv = regexProv.exec(releaseTag);
    var isLatestProv = (0, _typeUtil.exists)(matchesLatestProv) && matchesLatestProv.length > 0;
    var isProv = (0, _typeUtil.exists)(matchesProv) && matchesProv.length > 0;
    return isLatestProv || isProv;
  },
  isProv: function isProv(releaseTag) {
  ...
  transformDoiStatusRelease: function transformDoiStatusRelease(doiStatus) {
    if (!(0, _typeUtil.exists)(doiStatus)) {
      return null;
    }
    var appliedDoiStatus = doiStatus;
    var transformed = {
      url: '',
      productDoi: {
  ...
  determineDelineateAvaRelease: function determineDelineateAvaRelease(releaseTag) {
    return !(0, _typeUtil.isStringNonEmpty)(releaseTag) || releaseTag === LATEST_AND_PROVISIONAL || ReleaseService.isLatestNonProv(releaseTag);
  }
};
