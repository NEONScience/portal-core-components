...
const DataPackageParser = {
  parseSensorPosition: position => {
    if (!(0, _typeUtil.exists)(position)) {
      return null;
    }
    let result = null;
    const resultV2 = _neonDataPackage.sensorPositionV2Schema.safeParse(position);
    if (!resultV2.success) {
      // Check for v1 schema
      const resultV1 = _neonDataPackage.sensorPositionV1Schema.safeParse(position);
      if (!resultV1.success) {
...
var _default = exports.default = DataPackageParser;