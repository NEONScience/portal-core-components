...
*/ export default function parseLocationsArray(locationsArray = []) {
    ...
    return worker.spawn((inData)=>{
       ...
       becomes:
       { fooBar: 123 }
    */ const parseLocationProperties = (inProps = [], whiteList = DEFAULT_LOCATION_PROPERTIES_WHITELIST)=>{
            // Function to convert a prop key into camelCase or, if applicable, a defined alias
            const cleanPropKey = (inKey = '')=>{
                const words = inKey.slice(10).replace(/[^A-Za-z0-9_ -]/g, '').replace(/[_-]/g, ' ').toLowerCase().split(' ');
                const newPropKey = words.map((word, idx)=>idx === 0 ? word : `${word.slice(0, 1).toUpperCase()}${word.slice(1)}`).join('');
    ...
    */ const parseLocationData = (data = {})=>{
            const { siteCode = null, locationName: name = null, locationType: type = null, locationDescription: description = null, locationDecimalLatitude: latitude = null, locationDecimalLongitude: longitude = null, locationPolygon: polygon = null, locationProperties = {}, locationParent: parent = null, locationChildren: children } = data;
            ...
            if (![
                'latitude',
                'longitude'
            ].every((k)=>Object.keys(parsed).includes(k)) && parsed.geometry && parsed.geometry.coordinates) {
                [parsed.latitude, parsed.longitude] = parsed.geometry.coordinates.reduce((acc, cur)=>[
                        acc[0] + cur[0],
                        acc[1] + cur[1]
                    ], [
                    0,
