...
function generateTimeSeriesGraphData() {
  ...
  var worker = new _paralleljs.default(payload).require(getTimeSteps).require(monthIsValidFormat).require(monthToNumbers).require(monthIsValid).require(monthToTicker).require(tickerIsValid).require(tickerToMonth).require(tickerToIso).require(dateTimeToFloorSeconds).require(getNextMonth);
  return worker.spawn(function (inData) {
    ...
    */
    var product = inData.product,
      ...
      derivedVariableTable = _inData$selection.derivedVariableTable,
    ...
    var ALLOW_POSITIONAL_TS_MAPPING = false;
    ...
    // level.
    var ALLOW_POSITIONAL_TS_TRUNCATION = false;
    ...
    var findTimestampIdx = function findTimestampIdx(data, searchTime, startIdx) {
      var isodateS = tickerToIso(searchTime, true);
      var isodateM = tickerToIso(searchTime, false);
      var dateIdx = -1;
      for (var i = startIdx; i < data.length; i += 1) {
        var dateTimeVal = data[i];
        var dateTimeValZeroS = dateTimeToFloorSeconds(dateTimeVal);
        var matched = dateTimeVal === isodateS || dateTimeValZeroS === isodateS || dateTimeVal === isodateM;
        if (matched) {
          dateIdx = i;
          break;
        }
    ...
    */
    ...
    if (!Array.isArray(sites) || !sites.length || sites.every(function (site) {
    ...
    var productSitesAreValid = false;
    ...
    */
    ...
    var derivedVariableTables = [];
    Object.keys(derivedVariableTable).forEach(function (k) {
      var v = derivedVariableTable[k];
    ...
    */
    ...
    var newSeries = [];
    var newLabels = ['x'];
    var newQualityLabels = ['start', 'end'];
    ...
    sites.forEach(function (site) {
      ...
      positions.forEach(function (position) {
        ...
        continuousDateRange.forEach(function (month) {
          ...
          var monthStepCount = Object.keys(newMonthOffsets).includes(nextMonth) ? newMonthOffsets[nextMonth] - monthIdx : newData.length - monthIdx;
          ...
          selectedVariables.forEach(function (variable) {
            ...
            var derivedTableName = derivedVariableTable[variable];
            var posData = product.sites[siteCode].positions[position].data;
            ...
            var seriesStepCount = posData[month][pkg][timeStep][derivedTableName].series[variable].data.length;
            ...
            if (ALLOW_POSITIONAL_TS_MAPPING) {
              if (seriesStepCount === monthStepCount) {
                posData[month][pkg][timeStep][derivedTableName].series[variable].data.forEach(function (d, datumIdx) {
            ...
            var lastIdx = 0;
            // eslint-disable-next-line max-len
            var dtVarData = posData[month][pkg][timeStep][derivedTableName].series[dateTimeVariable].data;
            for (var _t = monthIdx; _t < monthIdx + monthStepCount; _t += 1) {
              var dataIdx = findTimestampIdx(dtVarData, newData[_t][0].getTime(), lastIdx);
              if (dataIdx === -1) {
                newData[_t][columnIdx] = null;
              } else {
                lastIdx = dataIdx + 1;
                newData[_t][columnIdx] = posData[month][pkg][timeStep][derivedTableName].series[variable].data[dataIdx];
              }
            }
          });
          ...
          qualityFlags.forEach(function (qf, qfIdx) {
            ...
            var _stateVariables$qf = stateVariables[qf],
              pkg = _stateVariables$qf.downloadPkg,
              qfTables = _stateVariables$qf.tables;
            var derivedTableName = !qfTables ? null : Array.from(qfTables).find(function (qfTable) {
              return derivedVariableTables.includes(qfTable);
            });
            var posData = product.sites[siteCode].positions[position].data;
            ...
            // eslint-disable-next-line max-len
            var seriesStepCount = posData[month][pkg][timeStep][derivedTableName].series[qf].data.length;
            // Series and month data lengths are identical as expected so we can stream
            // values directly in without matching timestamps
            if (ALLOW_POSITIONAL_TS_MAPPING) {
              if (seriesStepCount === monthStepCount) {
                posData[month][pkg][timeStep][derivedTableName].series[qf].data.forEach(function (d, datumIdx) {
            ...
            for (var _t2 = monthIdx; _t2 < monthIdx + monthStepCount; _t2 += 1) {
              var dataIdx = findTimestampIdx(dtVarData, newQualityData[_t2][0].getTime(), lastIdx);
              if (dataIdx === -1) {
                newQualityData[_t2][columnIdx] = getQFNullFill();
              } else {
                lastIdx = dataIdx + 1;
                var d = typeof posData[month][pkg][timeStep][derivedTableName].series[qf].data[dataIdx] !== 'undefined' ? posData[month][pkg][timeStep][derivedTableName].series[qf].data[dataIdx] : null;
                if (!Array.isArray(newQualityData[_t2][columnIdx])) {
                  newQualityData[_t2][columnIdx] = [];
                }
                newQualityData[_t2][columnIdx][qfIdx] = d;
              }
            }
          });
        });
