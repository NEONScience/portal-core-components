...
function tickerToIso(ticker, includeSeconds = true) {
    ...
    return includeSeconds ? `${YYYY}-${MM}-${DD}T${hh}:${mm}:${ss}Z` : `${YYYY}-${MM}-${DD}T${hh}:${mm}Z`;
}
...
*/ export default function generateTimeSeriesGraphData(payload = {}) {
    const worker = new Parallel(payload).require(getTimeSteps).require(monthIsValidFormat).require(monthToNumbers).require(monthIsValid).require(monthToTicker).require(tickerIsValid).require(tickerToMonth).require(tickerToIso).require(dateTimeToFloorSeconds).require(getNextMonth);
    return worker.spawn((inData)=>{
        /**
       Deconstruct necessary values from state
    */ const { product, variables: stateVariables, selection: { autoTimeStep, continuousDateRange, dateRange, dateTimeVariable, qualityFlags, sites, derivedVariableTable, timeStep: selectedTimeStep, variables: selectedVariables } } = inData;
        ...
        sites.forEach((site)=>{
            ...
            if (typeof product.sites[siteCode] === 'object' && product.sites[siteCode] !== null && typeof product.sites[siteCode].positions === 'object' && product.sites[siteCode].positions !== null) {
                ...
                positions.forEach((position)=>{
                    const invalidPosition = typeof product.sites[siteCode].positions[position] !== 'object';
                    const invalidPositionData = invalidPosition || product.sites[siteCode].positions[position] === null || typeof product.sites[siteCode].positions[position].data !== 'object' || product.sites[siteCode].positions[position].data === null || Object.keys(product.sites[siteCode].positions[position].data).length < 1;
                    ...
                    qualityFlags.forEach((qf, qfIdx)=>{
                        ...
                        for(let t = monthIdx; t < monthIdx + monthStepCount; t += 1){
                            ...
                            if (dataIdx === -1) {
                                ...
                                lastIdx = dataIdx + 1;
                                const datum = posData[month][pkg][timeStep][derivedTableName].series[qf].data[dataIdx];
                                const d = typeof datum !== 'undefined' ? datum : null;
                                if (!Array.isArray(newQualityData[t][columnIdx])) {
                                    newQualityData[t][columnIdx] = [];
                                }
                                newQualityData[t][columnIdx][qfIdx] = d;
