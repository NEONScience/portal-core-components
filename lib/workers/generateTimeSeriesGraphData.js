  ...
  return worker.spawn(function (inData) {
    ...
    */
    var product = inData.product,
        ...
        selectedVariables = _inData$selection.variables; // Optionally toggle to allow a purely positional mapping
    ...
    var ALLOW_POSITIONAL_TS_MAPPING = false; // Optionally toggle to allow truncating series values
    ...
    var TIME_STEPS = getTimeSteps(); // Function to match the searchTime in milliseconds since epoch,
      ...
      positions.forEach(function (position) {
        ...
        continuousDateRange.forEach(function (month) {
          ...
          selectedVariables.forEach(function (variable) {
            ...
            var lastIdx = 0;
            var dtVarData = posData[month][pkg][timeStep].series[dateTimeVariable].data;
            ...
            for (var _t = monthIdx; _t < monthIdx + monthStepCount; _t += 1) {
              ...
              if (dataIdx === -1) {
                ...
                lastIdx = dataIdx + 1;
                newData[_t][columnIdx] = posData[month][pkg][timeStep].series[variable].data[dataIdx];
              }
            }
          ...
          qualityFlags.forEach(function (qf, qfIdx) {
            ...
            var seriesStepCount = posData[month][pkg][timeStep].series[qf].data.length; // Series and month data lengths are identical as expected so we can stream
