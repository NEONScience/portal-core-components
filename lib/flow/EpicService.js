...
const EpicService = {
  createEpic: (ofTypeFilter, ajaxConfig, workingAction, successAction, errorAction, takeUntilTypeFilter, ajaxBodyCreator, ajaxRequestInjector, useForkJoin) => (action$, state$, {
    ajax
  }) => {
    ...
    return action$.pipe(_reduxObservable.ofType.apply(_reduxObservable.ofType, ofTypeFilters), (0, _rxjs.switchMap)(action => (0, _rxjs.concat)((0, _rxjs.of)(workingAction({
      action
  ...
  createMergeEpic: (ofTypeFilter, ajaxConfig, workingAction, successAction, errorAction, takeUntilTypeFilter, ajaxBodyCreator, ajaxRequestInjector, useForkJoin) => (action$, state$, {
    ajax
  }) => {
    const ofTypeFilters = !Array.isArray(ofTypeFilter) ? [ofTypeFilter] : ofTypeFilter;
    let takeUntilOperator;
    ...
    return action$.pipe(_reduxObservable.ofType.apply(_reduxObservable.ofType, ofTypeFilters), (0, _rxjs.mergeMap)(action => (0, _rxjs.concat)((0, _rxjs.of)(workingAction({
      action
    })), EpicService.createAjaxObservable(ajax, ajaxConfig, successAction, errorAction, action, takeUntilOperator, ajaxBodyCreator, ajaxRequestInjector, useForkJoin))));
  },
