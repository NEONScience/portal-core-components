...
program.usage('Usage: node sync-assets.js [options]').version(SCRIPT_VERSION).option('--cwd-relative-public-dir-path <value>', 'public folder path relative to current working directory').option('--cwd-relative-app-dir-path <value>', 'app folder path relative to current working directory').option('--prettify-index', 'option to prettify index file');
program.parse(process.argv);
const config = program.opts();
if (config.cwdRelativePublicDirPath === null || config.cwdRelativePublicDirPath === undefined || config.cwdRelativePublicDirPath.length <= 0) {
    console.error('Invalid command arguments. --cwd-relative-public-dir-path must be specified');
    process.exit(9);
}
if (config.cwdRelativeAppDirPath === null || config.cwdRelativeAppDirPath === undefined || config.cwdRelativeAppDirPath.length <= 0) {
    ...
    assetFiles.forEach((assetFileName)=>{
        Object.keys(SYNC_CSS_ASSET_PREFIX).forEach((assetKey)=>{
            ...
            if (assetFileName.startsWith(prefix)) {
                ...
                fs.writeFileSync(createFile, syncFileData, {
                    encoding: 'utf8'
                });
            }
        });
            ...
            if (assetFileName.startsWith(assetIndex.prefix)) {
                ...
                if (fs.existsSync(appFilePath)) {
                    ...
                    if (config.prettifyIndex) {
                        appFileOutput = await prettier.format(appFileOutput, {
                            parser: 'babel-ts',
                            printWidth: 100,
                            singleQuote: true
                        });
                    }
                    fs.writeFileSync(appFilePath, appFileOutput, {
                        encoding: 'utf8'
                    });
                }
            }
