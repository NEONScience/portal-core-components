...
const getDateRangeHandleMaskWidth = (dateRangeValue = [null, null]) => {
...
/**
   ...
   be visible.
*/
const getMinTimeOffset = svgWidth => 0 - (absMaxDataWidth - (svgWidth - SVG.LABEL_WIDTH));
const getYearStartX = (year) => {
  const intYear = parseInt(year, 10);
  return SVG.LABEL_WIDTH + SVG.END_PADDING
...
const getYearMonthsInView = (totalSvgWidth, dragOffset) => yearMonths.filter((yearMonth) => {
  ...
  const lower = 0;
  const upper = (totalSvgWidth - SVG.LABEL_WIDTH) + (8 * yearMonthWidth);
...
const getYearsInView = (totalSvgWidth, dragOffset) => years.filter((year) => {
  ...
  const upper = (totalSvgWidth - SVG.LABEL_WIDTH) + (2 * yearWidth);
...
export function AvailabilityGrid(config) {
  ...
  */
  ...
  const rowKeys = Object.keys(data.rows).sort().reverse();
  ...
  */
  ...
  const clipWidth = svgWidth - SVG.LABEL_WIDTH;
  ...
  */
  const lboundOffset = SVG.LABEL_WIDTH + halfCellPad - 1;
  ...
  */
  ...
  rowKeys.forEach((rowKey, rowIdx) => {
    ...
    const labelX = SVG.LABEL_WIDTH - SVG.CELL_PADDING;
  ...
  */
  ...
  const redrawData = () => {
    ...
    dataMasksG.selectAll('rect')
      ...
      .attr('width', svgWidth - SVG.LABEL_WIDTH)
      .attr('height', SVG.CELL_HEIGHT + SVG.CELL_PADDING)
    ...
    dateRangeMasksG.selectAll('rect')
      ...
      .attr('x', d => (
        getYearMonthGutterX(dateRange.value[d] || minYearMonth, d === 0 ? 'left' : 'right')
          - (getDateRangeHandleMaskWidth(dateRange.value) / 2)
          + getTimeOffset()
      ))
      .attr('y', 0)
      .attr('width', getDateRangeHandleMaskWidth(dateRange.value))
      .attr('height', svgHeight)
  ...
  if (selectionEnabled) {
    ...
    const dragDateRangeStart = drag()
      ...
      .on('drag', () => {
        ...
        const maskX = draggingDateRange[0].centerDragX
          - (getDateRangeHandleMaskWidth(dateRange.value) / 2);
        ...
        if (
          adjacentYearMonth !== dateRange.value[0]
            && distanceToAdjacent < distanceToCurrent
            && adjacentYearMonth <= dateRange.value[1]
        ) {
    ...
    const dragDateRangeEnd = drag()
      ...
      .on('drag', () => {
        ...
        const maskX = draggingDateRange[1].centerDragX
          - (getDateRangeHandleMaskWidth(dateRange.value) / 2);
