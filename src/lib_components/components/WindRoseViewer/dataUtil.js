 ...
 * @param {*} activeMatcher
 */
export const getLikeMatchers = (matchers, activeMatcher) => {
  if (!matchers || !activeMatcher) {
    return [];
  }
  return matchers.filter((matcher) => (
    (matcher.tmi === activeMatcher.tmi)
  ));
};
 ...
 */
export const getApiDataQueryParams = (action) => {
  const { product, release, query } = action;
  const hasRelease = isStringNonEmpty(release);
  const queryParams = {
 ...
 * @param {*} siteLookup
 */
export const getSiteOptions = (product, siteLookup) => {
 ...
 * @param {*} siteCodes
 */
export const getMonthOptions = (product, siteCodes) => {
  ...
  let months = [];
  if (site !== null) {
    months = reduce(site.availableMonths, (acc, month) => {
  ...
  months.sort((a, b) => (
    (-1) * a.value.toLowerCase().localeCompare(b.value.toLowerCase())
  ));
  return months;
};
 ...
 * @param {*} fileName
 */
export const extractDataFileNameInfo = (fileName) => {
  let info = null;
 ...
 * @param {*} vertical
 */
export const containsPosition = (positions, horizontal, vertical) => {
  if ((positions === null) || (positions.length <= 0)) {
    return false;
  }
  return some(positions, (position) => {
 ...
 */
export const tryFindActiveFiles = (files, query, fileResolutionMatchers) => {
  let info = null;
  ...
  some(fileResolutionMatchers, (matcher) => {
    reduce(files, (infoAcc, file) => {
 ...
 * @param {*} fileResolutionMatchers
 */
export const getActiveDataFilesInfo = (files, query, fileResolutionMatchers) => {
  const info = {
    dataFileUrls: [],
    dataFileMatcher: null,
    positions: [],
  ...
  while (((info.dataFileUrls === null) || (info.dataFileUrls.length <= 0)) && (i < maxTries)) {
    ...
    if ((activeFilesInfo !== null)
        && (activeFilesInfo.dataFileUrls !== null)
        && (activeFilesInfo.dataFileUrls.length > 0)) {
 ...
 * @param {*} fileResolutionMatchers
 */
export const parseDataFiles = (data, query, fileResolutionMatchers) => {
  const dataFilesInfo = {
    dataFileUrls: [],
    dataFileMatcher: null,
  ...
  if ((activeDataFilesInfo !== null)
      && (activeDataFilesInfo.dataFileUrls !== null)
      && (activeDataFilesInfo.dataFileUrls.length > 0)) {
  ...
  reduce(data.files, (acc, file) => {
    fileNameInfo = extractDataFileNameInfo(file.name);
    // Identify variables file
    if ((acc.variablesFileUrl === null)
        && (file.name.indexOf(FILE_TYPES.CSV) > -1)
        && (file.name.indexOf(FILE_TYPES.VARIABLES) > -1)) {
      acc.variablesFileUrl = file.url;
    }
    // Match against CSV, package type, and active file temporal resolution matcher
    if ((fileNameInfo !== null)
        && (dataFilesInfo.dataFileMatcher !== null)
        && (fileNameInfo.fileExtension === FILE_TYPES.CSV)
        && ((typeof fileNameInfo.packageType !== 'undefined')
          && (fileNameInfo.packageType !== null)
          && (fileNameInfo.packageType.toLowerCase() === PACKAGE_TYPE.BASIC))
        && ((fileNameInfo.temporalIndex === dataFilesInfo.dataFileMatcher.tmi)
          || (fileNameInfo.tableName.indexOf(dataFilesInfo.dataFileMatcher.fileName) > -1))) {
