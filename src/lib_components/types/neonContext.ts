...
export interface BundleContext {
   ...
   * that should provide the DOI(s) for the bundled product.
   * {
   *   "RELEASE": {
   *     "PRODUCT_CODE": "BUNDLE_PRODUCT_CODE" OR ["BUNDLE_PRODUCT_CODE", "BUNDLE_PRODUCT_CODE"],
   *   },
   * }
   * This lookup can also be utilized to determine whether or not
   * a product exists within a bundle.
   */
  bundleDoiLookup: Record<string, Record<string, string|string[]>>;
  /**
